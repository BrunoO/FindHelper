...
- **Saving Settings**: Application preferences are saved to a `findhelper_settings.json` file in the user's profile (`~/.FindHelper/`).
...
```powershell
cmake -S . -B build -A x64 -DFAST_LIBS_BOOST=ON -DBOOST_ROOT=C:\dev\boost_1_90_0 -DBoost_DIR=C:\dev\boost_1_90_0\lib64-msvc-14.1\cmake\Boost-1.90.0 -DENABLE_PGO=ON && cmake --build build --config Release
```
...
| `file_index_search_strategy_tests` | FileIndex parallel search (hybrid chunking) |
...
| `FAST_LIBS_BOOST` | `OFF` | Use Boost (unordered_map, regex, lockfree; requires Boost 1.81+) |
| `ENABLE_PGO` | `OFF` | Enable Profile-Guided Optimization (Windows only) |
| `PGO_PHASE` | `auto` | PGO phase when `ENABLE_PGO` is on: `auto` (detect `.pgd`), `instrument`, or `optimize` |
...
Case-insensitive matching folds `A–Z`/`a–z` only. File names containing non-ASCII characters are still searched and matched, but case variants beyond ASCII are not folded: e.g. searching `strasse` will not match `STRAßE` (`ß` never folds to `SS`), and locale-specific case rules such as Turkish dotted/dotless `I` are not applied. This follows the application's C-locale behavior and applies to all case-insensitive query types (substring, glob, fuzzy, pattern).
...
### Linux: Slow First Index Build (Cold Start)

Initial indexing on Linux can vary wildly — from a few seconds to several minutes for the same folder — depending on whether the filesystem tree is in the OS page cache.

**Why it happens:** Linux reads directory entries and inode metadata from disk. On first run after a reboot (or after a long idle), every `getdents64()` call requires a physical disk read. On HDDs with scattered inodes this causes thousands of random seeks; even on NVMe the cold-cache path is significantly slower. Once the tree has been read, the kernel caches the data in RAM and subsequent index builds are fast.

**Comparison:** Windows is largely unaffected because FindHelper reads the MFT sequentially (no per-file seeks). macOS is less affected because `getattrlistbulk()` batches metadata fetches.

**What to expect:**
- First run after boot on a large tree (≥1 M files, HDD): potentially minutes.
- Subsequent runs on the same session: typically seconds (warm cache).
- To force a cold run for benchmarking: `echo 3 | sudo tee /proc/sys/vm/drop_caches`

This is a Linux kernel/filesystem constraint, not a bug in FindHelper.
