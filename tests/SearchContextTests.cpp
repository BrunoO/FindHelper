...
TEST_SUITE("SearchContext - ValidateAndClamp") {  // NOLINT(readability-identifier-naming,cppcoreguidelines-avoid-non-const-global-variables) - doctest macro expands to a global test suite object with required name
  ...
  TEST_CASE("ValidateAndClamp - guided_scheduling_divisor clamped below minimum") {
    SearchContext ctx;
    ctx.guided_scheduling_divisor = 0;
    ctx.ValidateAndClamp();
    CHECK(ctx.guided_scheduling_divisor == 1);
  }

  TEST_CASE("ValidateAndClamp - guided_scheduling_divisor clamped above maximum") {
    SearchContext ctx;
    ctx.guided_scheduling_divisor = 9;
    ctx.ValidateAndClamp();
    CHECK(ctx.guided_scheduling_divisor == 8);
  }

  TEST_CASE("ValidateAndClamp - guided_scheduling_divisor boundary values unchanged") {
    SearchContext ctx;

    ctx.guided_scheduling_divisor = 1;
    ctx.ValidateAndClamp();
    CHECK(ctx.guided_scheduling_divisor == 1);

    ctx.guided_scheduling_divisor = 8;
    ctx.ValidateAndClamp();
    CHECK(ctx.guided_scheduling_divisor == 8);
  }

  TEST_CASE("ValidateAndClamp - default guided_scheduling_divisor unchanged") {
    SearchContext ctx;
    ctx.ValidateAndClamp();
    CHECK(ctx.guided_scheduling_divisor == 2);
  }
}
...
TEST_SUITE("SearchContext - PrepareExtensionSet") {  // NOLINT(readability-identifier-naming,cppcoreguidelines-avoid-non-const-global-variables) - doctest macro expands to a global test suite object with required name
  ...
  TEST_CASE("PrepareExtensionSet - case-insensitive lowercase") {
    ...
    CHECK(std::binary_search(ctx.extension_set.begin(), ctx.extension_set.end(), "txt"));  // NOLINT(llvm-use-ranges) - C++17; std::ranges requires C++20
    CHECK(std::binary_search(ctx.extension_set.begin(), ctx.extension_set.end(), "cpp"));  // NOLINT(llvm-use-ranges) - C++17; std::ranges requires C++20
    CHECK(std::binary_search(ctx.extension_set.begin(), ctx.extension_set.end(), "h"));  // NOLINT(llvm-use-ranges) - C++17; std::ranges requires C++20
  }
