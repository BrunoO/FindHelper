...
TEST_SUITE("StringSearch NEON") {
    ...
    TEST_SUITE("NEON Direct") {
        TEST_CASE("IsViewAsciiNEON accepts long ASCII buffer") {
            std::string text(kLongPad, 'a');
            text += "ASCII_ONLY";
            text += std::string(kLongPad, 'b');
            CHECK(string_search::neon::IsViewAsciiNEON(text.data(), text.size()));
        }

        TEST_CASE("IsViewAsciiNEON rejects non-ASCII after byte 64") {
            std::string text(80, 'a');
            text[70] = static_cast<char>(0x80);
            CHECK_FALSE(string_search::neon::IsViewAsciiNEON(text.data(), text.size()));
        }
    ...
    TEST_SUITE("NEON Integration") {
        ...
        TEST_CASE("case-sensitive SIMD with non-ASCII haystack byte") {
            std::string text(kMedPad, 'a');
            text[kMidOffset] = static_cast<char>(0x80);
            text += "ExactToken";
            text += std::string(kMedPad, 'b');
            REQUIRE(string_search::ContainsSubstring(text, "ExactToken"));  // NOLINT(cert-err33-c) - doctest macro; return value is checked by the framework
        }
    }
