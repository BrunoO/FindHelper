...
// ---------------------------------------------------------------------------
// ShouldDeferFilterCacheRebuild — narrow one-frame defer (2026-07-17 interim)
// ---------------------------------------------------------------------------

TEST_SUITE("ShouldDeferFilterCacheRebuild") {

TEST_CASE("No active filters — never defer") {
    GuiState state;
    state.result_pool_->Results() = {
        MakePreloadedResult(R"(C:\a.txt)", kFileSizeNotLoaded, 1000),
    };
    state.result_pool_->Results()[0].lastModificationTime = kFileTimeNotLoaded;
    state.searchCriteria.time_filter = TimeFilter::None;
    state.searchCriteria.size_filter = SizeFilter::None;
    CHECK_FALSE(ShouldDeferFilterCacheRebuild(state));
}

TEST_CASE("Time filter with loaded times — do not defer") {
    GuiState state;
    state.result_pool_->Results() = {
        MakePreloadedResult(R"(C:\a.txt)", 100, 1000),
    };
...
TEST_CASE("Time filter with unloaded times — defer") {
    GuiState state;
    SearchResult r = MakePreloadedResult(R"(C:\a.txt)", 100, 1000);
    r.lastModificationTime = kFileTimeNotLoaded;
    state.result_pool_->Results() = {r};
    state.searchCriteria.time_filter = TimeFilter::Today;
    state.searchCriteria.size_filter = SizeFilter::None;
    CHECK(ShouldDeferFilterCacheRebuild(state));
}

TEST_CASE("Size filter with loaded sizes — do not defer") {
    GuiState state;
    state.result_pool_->Results() = {
        MakePreloadedResult(R"(C:\a.txt)", 100, 1000),
    };
...
TEST_CASE("Size filter with unloaded file sizes — defer") {
    GuiState state;
    SearchResult r = MakePreloadedResult(R"(C:\a.txt)", kFileSizeNotLoaded, 1000);
    state.result_pool_->Results() = {r};
    state.searchCriteria.time_filter = TimeFilter::None;
    state.searchCriteria.size_filter = SizeFilter::Large;
    CHECK(ShouldDeferFilterCacheRebuild(state));
...
TEST_CASE("Size filter with only unloaded directory sizes — do not defer") {
    GuiState state;
    SearchResult dir = MakePreloadedResult(R"(C:\dir)", kFileSizeNotLoaded, 1000);
