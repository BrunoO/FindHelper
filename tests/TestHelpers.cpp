...
void SaveCurrentSettings(int& old_chunk_size, int& old_hybrid_percent,
...
TestSettingsFixture::TestSettingsFixture() {
  SaveCurrentSettings(old_chunk_size_, old_hybrid_percent_, settings_were_active_);
...
TestSettingsFixture::TestSettingsFixture([[maybe_unused]] std::string_view strategy) {
  SaveCurrentSettings(old_chunk_size_, old_hybrid_percent_, settings_were_active_);

  // Enable in-memory mode (hybrid is always used; strategy param ignored)
  AppSettings current;
  LoadSettings(current);
...
void CreateTestFileIndex(FileIndex& index, size_t file_count, const std::string& base_path) {
  ...
  index.Insert(ntfs_file_reference::NtfsFileReference(root_id), ntfs_file_reference::NtfsFileReference(0), base_path, true, root_time);
  ...
  for (size_t i = 1; i <= file_count; ++i) {
    ...
    index.Insert(ntfs_file_reference::NtfsFileReference(file_id), ntfs_file_reference::NtfsFileReference(root_id), name, is_dir, mod_time);
  }
...
TestIndexOperationsFixture::TestIndexOperationsFixture()
    : storage_(mutex_), path_operations_(storage_, path_storage_),
      operations_(storage_, path_operations_, remove_not_in_index_count_,
                  ...
                  [this](const index_domain_events::ParentHealed& healed) {
                    received_parent_healed_.push_back(healed);
                    healed_awaiting_count_.fetch_add(1);
                  }),
      lock_(mutex_) {}
...
TestDirectoryResolverFixture::TestDirectoryResolverFixture()
    : storage_(mutex_), path_operations_(storage_, path_storage_),
      operations_(storage_, path_operations_, remove_not_in_index_count_,
                  remove_inconsistency_count_,
                  nullptr),  // no heal assertions in resolver tests
...
void TestLazyAttributeLoaderFixture::InsertFileEntryInternal(uint64_t id, const std::string& name,
  ...
  storage_.InsertLocked(ntfs_file_reference::NtfsFileReference(id),
                        ntfs_file_reference::NtfsFileReference(0), name, is_dir,
                        kFileTimeNotLoaded);
...
MinimalLoaderSetup& CreateLoaderSetupWithDirectory(MinimalLoaderSetup& setup, uint64_t dir_id,
  ...
  setup.storage.InsertLocked(ntfs_file_reference::NtfsFileReference(dir_id),
                             ntfs_file_reference::NtfsFileReference(0), dir_name, true,
                             kFileTimeNotLoaded);
...
MinimalLoaderSetup& CreateLoaderSetupWithEmptyPath(MinimalLoaderSetup& setup, uint64_t file_id,
  ...
  setup.storage.InsertLocked(ntfs_file_reference::NtfsFileReference(file_id),
                             ntfs_file_reference::NtfsFileReference(0), file_name, false,
                             kFileTimeNotLoaded);
...
size_t CollectAndVerifyFutures(std::vector<std::future<SearchResultBatch>>& futures,
...
size_t CollectFuturesAndCount(std::vector<std::future<SearchResultBatch>>& futures) {
...
size_t TestCancellation(size_t file_count, bool cancel_immediately,
  ...
  auto futures = index_fixture.GetIndex().SearchAsyncWithData(
    "file_", 4, nullptr, "", nullptr, false, false, nullptr, cancel_flag.get(), nullptr);
...
size_t VerifyFuturesSizeAndCollect(std::vector<std::future<SearchResultBatch>>& futures,
...
std::pair<size_t, size_t> CollectTwoFutureVectors(
  std::vector<std::future<SearchResultBatch>>& futures1,
  std::vector<std::future<SearchResultBatch>>& futures2) {
  auto results1 = CollectFutures(futures1);
  auto results2 = CollectFutures(futures2);
  return {results1.size(), results2.size()};
}

size_t TestGetFuturesAndCollect(size_t file_count,
  ...
  auto futures = index_fixture.GetIndex().SearchAsyncWithData(query, thread_count, nullptr, "",
                                                              nullptr, false, false, nullptr, nullptr,
                                                              nullptr);
...
std::vector<ConcurrentSearchResult> RunConcurrentSearches(FileIndex& index,
  ...
  // Start all searches
  std::vector<std::vector<std::future<SearchResultBatch>>> all_futures;
...
std::vector<SearchResultData> CollectFutures(std::vector<std::future<SearchResultBatch>>& futures) {
  std::vector<SearchResultBatch> batches;
  batches.reserve(futures.size());
  size_t total_hits = 0;
  for (auto& future : futures) {
    batches.push_back(future.get());
    total_hits += batches.back().hits.size();
  }
  std::vector<SearchResultData> all_results;
  all_results.reserve(total_hits);
  for (const SearchResultBatch& batch : batches) {
    AppendBatchToResultData(all_results, batch);
  }
  return all_results;
}
