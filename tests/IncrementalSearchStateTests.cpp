...
TEST_SUITE("IncrementalSearchState") {  // NOLINT(readability-identifier-naming,cppcoreguidelines-avoid-non-const-global-variables) - doctest macro expands to a global test suite object with required name
  ...
  TEST_CASE_FIXTURE(SearchStateFixture, "RefreshFilter updates filtered results with modified attributes") {
    results.push_back(store.MakeResult("folder/foo.txt"));
    results[0].fileSize = 100;
    results[0].fileSizeDisplay = "100 B";

    state.Begin(results, selected_row, 0.0F, 1U);
    state.UpdateQuery("foo", results, selected_row, scroll_to_selected);
    CHECK_EQ(state.MatchCount(), 1);
    CHECK_EQ(state.FilteredResults()[0].fileSize, 100);
    CHECK_EQ(state.FilteredResults()[0].fileSizeDisplay, "100 B");

    // Base results are updated (e.g. async size loaded)
    results[0].fileSize = 500;
    results[0].fileSizeDisplay = "500 B";

    state.RefreshFilter(results);
    CHECK_EQ(state.MatchCount(), 1);
    CHECK_EQ(state.FilteredResults()[0].fileSize, 500);
    CHECK_EQ(state.FilteredResults()[0].fileSizeDisplay, "500 B");
  }
