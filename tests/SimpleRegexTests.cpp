...
TEST_SUITE("SimpleRegex") {
  ...
  // Characterization of multi-star / boundary glob behavior. These pin the
  // established recursive implementation's outputs, so the linear-matcher
  // migration must reproduce them byte-identically (see GlobMatchImpl).
  TEST_CASE("GlobMatchMultiStar") {
    const std::vector<MatchTestCase> test_cases = {
        {"**", "anything", true},
        {"**", "", true},
        {"a**b", "axb", true},
        {"a**b", "axxb", true},
        {"a**b", "ab", true},
        {"a**b", "axc", false},
        {"***test***", "pretestpost", true},
        {"***test***", "pretespost", false},
        {"*a*b*c*", "xxayybzzcww", true},
        {"*a*b*c*", "xxayy", false},
        {"*x*y*z*", std::string(100, 'q'), false},
        {"*usr*lib*", "/usr/local/lib/foo", true},
        {"*usr*lib*", "/opt/bin/foo", false},
        {"test**", "testing", true},
        {"**test", "pretest", true},
        {"*?", "", false},
        {"*?", "a", true},
  ...
  TEST_CASE("GlobQuestionMarkIsSingleByte") {
    // '?' consumes exactly one byte, not one Unicode code point.
    CHECK(simple_regex::GlobMatch("?", "a"));
    CHECK_FALSE(simple_regex::GlobMatch("?", ""));
    CHECK_FALSE(simple_regex::GlobMatch("?", "ab"));
    CHECK_FALSE(simple_regex::GlobMatch("?", "\xC3\xA9"));
    CHECK(simple_regex::GlobMatch("??", "\xC3\xA9"));
    CHECK(simple_regex::GlobMatch("a?c", "abc"));
    CHECK_FALSE(simple_regex::GlobMatch("a?c", "ac"));
    CHECK_FALSE(simple_regex::GlobMatch("a?c", "abbc"));
    CHECK(simple_regex::GlobMatchI("A?C", "abc"));
  }
}
