...
TEST_CASE("DirectoryResolver::GetOrCreateDirectoryId creates single directory") {
  ...
  CHECK(entry->parentID.raw == 0); // Root
...
TEST_CASE("DirectoryResolver::GetOrCreateDirectoryId creates nested directories recursively") {
  ...
  CHECK(nested_entry->parentID.raw == john_id);
  ...
  CHECK(john_entry->parentID.raw == users_id);
  ...
  CHECK(users_entry->parentID.raw == 0); // Root
}
...
TEST_CASE("DirectoryResolver C:\\ aliases to C: (no empty-named child)") {
  // Display helpers return "C:\\" for drive-root files; index keys use "C:".
  // GetOrCreateDirectoryId must trim so both forms resolve to one entry.
  test_helpers::TestDirectoryResolverFixture fixture;

  const uint64_t id_colon = fixture.GetResolver().GetOrCreateDirectoryId("C:");
  CHECK(id_colon > 0);
  CHECK(fixture.GetStorage().GetDirectoryId("C:") == id_colon);

  const uint64_t id_root = fixture.GetResolver().GetOrCreateDirectoryId(R"(C:\)");
  CHECK(id_root == id_colon);
  CHECK(fixture.GetStorage().Size() == 1);
  CHECK(fixture.GetStorage().GetDirectoryId(R"(C:\)") == 0);  // cache key is trimmed form

  CHECK(fixture.GetResolver().GetOrCreateDirectoryId("/") == 0);
  CHECK(fixture.GetStorage().Size() == 1);
}
