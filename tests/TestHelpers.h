...
public:
  ...
  /**
   * Create fixture with a specific load balancing strategy name.
   * Only "hybrid" is supported; the argument is accepted for API compatibility.
   *
   * @param strategy Accepted but ignored (hybrid is always used)
   */
...
public:
  ...
  /**
   * Get reference to healed_awaiting_count atomic.
   *
   * @return Reference to the atomic counter
   */
  std::atomic<size_t>& GetHealedAwaitingCount() { return healed_awaiting_count_; }

  /**
   * Published ParentHealed events, in publication order.
   *
   * The fixture subscribes its healed counter through this same sink, so the
   * counter and the received log always agree. Tests asserting event payloads
   * (child/parent ids) read here.
   *
   * @return Received heal events since fixture construction
   */
  const std::vector<index_domain_events::ParentHealed>& ReceivedParentHealed() const {
    return received_parent_healed_;
  }
...
private:
  ...
  std::atomic<size_t> healed_awaiting_count_{0};
  std::vector<index_domain_events::ParentHealed> received_parent_healed_;
...
/**
 * Run a test function with hybrid strategy settings.
 *
 * @tparam TestFunc Function type: void(FileIndex&, const std::string&)
 * @param index FileIndex to use
 * @param test_func Test function to run (receives index and strategy name "hybrid")
 * @param reset_thread_pool Whether to reset thread pool before running (default: true)
 */
...
/**
 * Run test with hybrid strategy settings and a freshly populated index.
 *
 * @param file_count Number of files in test index
 * @param test_func Function to execute (receives index and strategy name "hybrid")
 */
template<typename TestFunc>
...
/**
 * Collect SearchResultBatch futures into owned SearchResultData rows.
 *
 * Single-threaded conversion via AppendBatchToResultData; preferred over the
 * template above when the engine returns batches (Stage 1 arena migration).
 *
 * @param futures Vector of batch futures to collect from
 * @return Combined vector of all result rows
 */
