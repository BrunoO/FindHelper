...
struct FILETIME {
    ...
    // Allow initialization: FILETIME{0, 0} (constexpr for compile-time sentinels)
    constexpr FILETIME() : dwLowDateTime(0), dwHighDateTime(0) {}
    constexpr FILETIME(uint32_t low, uint32_t high) : dwLowDateTime(low), dwHighDateTime(high) {}

    // Comparison operators for test assertions
    constexpr bool operator==(const FILETIME& other) const {  // NOSONAR(cpp:S2807) - Member operator is acceptable for test stub; could be refactored to hidden friend later if needed
