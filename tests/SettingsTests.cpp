...
struct ScopedTempHomeForDiskSettings {
  ...
  [[nodiscard]] std::string PrimaryFile() const {
    return path_utils::JoinPath(PrimaryDir(), "findhelper_settings.json");
  }
};
...
TEST_SUITE("Settings") {
  ...
  TEST_SUITE("UI Mode and Backward Compatibility") {
    ...
    TEST_CASE_FIXTURE(SettingsFixture, "SaveSettings with uiMode") {
      // Must use in-memory mode: SaveSettings without isolation would overwrite the
      // real USERPROFILE/.FindHelper/findhelper_settings.json with mostly-default AppSettings.
