...
TEST_CASE("IndexOperations::Insert initializes file size including zero-byte files") {
  test_helpers::TestIndexOperationsFixture fixture;

  fixture.GetOperations().Insert(NtfsFileReference(1), NtfsFileReference(0), "root", true);

  // 1. Zero-byte file: should be loaded with size 0, not kFileSizeNotLoaded
  fixture.GetOperations().Insert(NtfsFileReference(2), NtfsFileReference(1), "empty.txt", false,
                                 {UINT32_MAX, UINT32_MAX}, IndexOperations::InsertOptions{}, 0ULL);
  const FileEntry* const empty_entry = fixture.GetStorage().GetEntry(2);
  REQUIRE(empty_entry != nullptr);
  CHECK_FALSE(empty_entry->fileSize.IsNotLoaded());
  CHECK(empty_entry->fileSize.IsLoaded());
  CHECK(empty_entry->fileSize.GetValue() == 0ULL);

  // 2. Non-zero file: should be loaded with the specified size
  fixture.GetOperations().Insert(NtfsFileReference(3), NtfsFileReference(1), "data.bin", false,
                                 {UINT32_MAX, UINT32_MAX}, IndexOperations::InsertOptions{}, 4096ULL);
  const FileEntry* const data_entry = fixture.GetStorage().GetEntry(3);
  REQUIRE(data_entry != nullptr);
  CHECK_FALSE(data_entry->fileSize.IsNotLoaded());
  CHECK(data_entry->fileSize.IsLoaded());
  CHECK(data_entry->fileSize.GetValue() == 4096ULL);

  // 3. Default not-loaded sentinel: should remain not loaded
  fixture.GetOperations().Insert(NtfsFileReference(4), NtfsFileReference(1), "lazy.txt", false);
  const FileEntry* const lazy_entry = fixture.GetStorage().GetEntry(4);
  REQUIRE(lazy_entry != nullptr);
  CHECK(lazy_entry->fileSize.IsNotLoaded());
  CHECK_FALSE(lazy_entry->fileSize.IsLoaded());
}

namespace {

// Bulk-staging options: deferred path indexing, no awaiting repair (the
// post-population recompute owns both). Mirrors IndexBuildRun::CommitBatch.
IndexOperations::InsertOptions DeferredBulkOptions() {
  IndexOperations::InsertOptions options{};
  options.resolve_awaiting = false;
  options.defer_path_indexing = true;
  return options;
}

}  // namespace

TEST_CASE("IndexOperations deferred staging: drain delete skips inconsistency diagnostic") {
  test_helpers::TestIndexOperationsFixture fixture;

  constexpr uint64_t kParent = 0x00010000000000A0ULL;
  constexpr uint64_t kChild = 0x00010000000000B0ULL;
  fixture.GetOperations().Insert(NtfsFileReference(kParent), NtfsFileReference(0), "DrainDir", true,
                                 {UINT32_MAX, UINT32_MAX}, DeferredBulkOptions());
  fixture.GetOperations().Insert(NtfsFileReference(kChild), NtfsFileReference(kParent), "drain.txt",
                                 false, {UINT32_MAX, UINT32_MAX}, DeferredBulkOptions());
  REQUIRE(fixture.GetStorage().GetEntry(kChild) != nullptr);
  CHECK_FALSE(fixture.GetPathOperations().HasPath(kChild));

  // Replay-drain delete of a staged-but-pathless entry: storage removal works,
  // and the miss must not skew the inconsistency metric (expected by design).
  fixture.GetOperations().Remove(NtfsFileReference(kChild));
  CHECK(fixture.GetStorage().GetEntry(kChild) == nullptr);
  CHECK(fixture.GetRemoveInconsistencyCount().load() == 0U);
}

TEST_CASE("IndexOperations deferred staging: drain rename stays bare") {
  test_helpers::TestIndexOperationsFixture fixture;
  ...
  // A drain-window rename must not impersonate a rooted path (volume-root
  // fallback): the entry stays bare until the recompute resolves it.
  REQUIRE(fixture.GetOperations().Rename(NtfsFileReference(kChild), "renamed.txt"));
  CHECK(fixture.GetPathOperations().GetPathView(kChild) == "renamed.txt");
}

TEST_CASE("IndexOperations deferred staging: drain move keeps arena leaf name") {
  test_helpers::TestIndexOperationsFixture fixture;

  constexpr uint64_t kOldParent = 0x00010000000000A0ULL;
  constexpr uint64_t kNewParent = 0x00010000000000C0ULL;
  constexpr uint64_t kChild = 0x00010000000000B0ULL;
  for (const uint64_t dir : {kOldParent, kNewParent}) {
    fixture.GetOperations().Insert(NtfsFileReference(dir), NtfsFileReference(0), "DrainDir", true,
                                   {UINT32_MAX, UINT32_MAX}, DeferredBulkOptions());
  }
  fixture.GetOperations().Insert(NtfsFileReference(kChild), NtfsFileReference(kOldParent),
                                 "moved.txt", false, {UINT32_MAX, UINT32_MAX},
                                 DeferredBulkOptions());

  // Both entry and target parent are pathless: the moved entry must keep its
  // arena leaf name (bare), not an empty or separator-only path.
  REQUIRE(fixture.GetOperations().Move(NtfsFileReference(kChild), NtfsFileReference(kNewParent)));
  CHECK(fixture.GetPathOperations().GetPathView(kChild) == "moved.txt");
  CHECK(fixture.GetStorage().GetEntry(kChild)->parentID.raw == kNewParent);
}
