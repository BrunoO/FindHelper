...
TEST_SUITE("LazyAttributeLoader - Failure Scenarios") {
  ...
  TEST_CASE("GetAttributes - loads both attributes together and caches them") {
    const test_helpers::TempFileFixture temp_file;
    test_helpers::TestLazyAttributeLoaderFixture fixture;

    const size_t path_idx = fixture.GetPathStorage().InsertPath(
        fixture.GetFileId(), temp_file.path, false, std::nullopt);
    fixture.GetStorage().SetPathStorageIndex(fixture.GetFileId(), path_idx);

    const FileAttributes attrs = fixture.GetLoader().GetAttributes(fixture.GetFileId());

    CHECK(attrs.success == true);
    CHECK(attrs.fileSize == temp_file.expectedSize);
    CHECK(IsValidTime(attrs.lastModificationTime));

    // Subsequent call returns cached values
    const FileAttributes cached = fixture.GetLoader().GetAttributes(fixture.GetFileId());
    CHECK(cached.success == true);
    CHECK(cached.fileSize == temp_file.expectedSize);
    CHECK(cached.lastModificationTime.dwLowDateTime == attrs.lastModificationTime.dwLowDateTime);
  }

  TEST_CASE("GetAttributes - directory with failed size keeps 0-size rule (no sentinel leak)") {
    test_helpers::lazy_loader_test_helpers::MinimalLoaderSetup setup;
    test_helpers::lazy_loader_test_helpers::CreateLoaderSetupWithDirectory(setup);

    // Pre-condition: directory size previously marked failed (e.g. a failed aggregate fetch),
    // modification time still not loaded.
    {
      FileEntry* const entry = setup.storage.GetEntryMutable(1);
      REQUIRE(entry != nullptr);
      REQUIRE(entry->isDirectory);
      entry->fileSize.MarkFailed();
    }

    const FileAttributes attrs = setup.loader.GetAttributes(1);

    // Time loads via I/O; size must keep the directory 0-size rule — a Failed size
    // sentinel must never leak into the result with success=true.
    CHECK(attrs.success == true);
    CHECK(attrs.fileSize == 0);
    CHECK(IsValidTime(attrs.lastModificationTime));

    // Subsequent call returns cached values (both-loaded fast path, still 0 for directories)
    const FileAttributes cached = setup.loader.GetAttributes(1);
    CHECK(cached.success == true);
    CHECK(cached.fileSize == 0);
    CHECK(cached.lastModificationTime.dwLowDateTime == attrs.lastModificationTime.dwLowDateTime);
  }
}
