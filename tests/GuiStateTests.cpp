...
TEST_SUITE("SelectionState::AddSelectionRange") {
  TEST_CASE("Stale ImGui range anchor outside [lo,hi] falls back instead of asserting") {
    // Regression: double-click open aborted in Debug via AssertSelectionInvariant
    // because ImGui's RangeSrcItem carried a stale anchor (results replaced
    // between frames). Must not abort; anchor falls back to the primary row.
    SelectionState selection;
    selection.AddSelectionRange(1, 1, /*anchor_row=*/5, /*primary_row=*/1);
    CHECK(selection.IsRowSelected(1));
    CHECK_FALSE(selection.IsRowSelected(5));
  }

  TEST_CASE("Valid anchor inside [lo,hi] is kept") {
    SelectionState selection;
    selection.AddSelectionRange(1, 3, /*anchor_row=*/2, /*primary_row=*/3);
    CHECK(selection.IsRowSelected(1));
    CHECK(selection.IsRowSelected(2));
    CHECK(selection.IsRowSelected(3));
  }

  TEST_CASE("Anchor of -1 (no range anchor) stays -1") {
    SelectionState selection;
    selection.AddSelectionRange(1, 1, /*anchor_row=*/-1, /*primary_row=*/1);
    CHECK(selection.IsRowSelected(1));
  }
}
