...
void PopulateDeepHierarchy(FileIndex& index, int depth, const std::string& base_name = "level") {
  ...
  std::string current_path = "C:\\Deep";

  // Create base directory
  uint64_t current_id = 1000000;
...
// Helper to get extension offset in path (reduces nesting in ValidateResults; S134).
// Now using PathUtils, this helper is simplified or can be removed if direct usage is cleaner
// Keeping it for now to match existing structure but using new helpers
std::string_view GetExtension(std::string_view path) {
  return path_utils::GetExtension(path);
}
...
void ValidateResults(const std::vector<SearchResultData>& results,
  ...
  for (const auto& result : results) {
    ...
    std::string_view filename_view = path_utils::GetFilename(path_view);
    ...
    if (expected_extensions && !expected_extensions->empty()) {
      std::string_view extension = path_utils::GetExtension(path_view);
      std::string result_ext = !extension.empty() ? std::string(ToLower(extension)) : std::string();
...
TEST_SUITE("FileIndex Search Strategies") {
  ...
  TEST_SUITE("Hybrid Strategy") {
    ...
    TEST_CASE("Hybrid strategy uses dynamic chunks") {
      ...
      // available
      auto settings = test_helpers::search_strategy_test_helpers::CreateHybridSettings(50);
    ...
    TEST_CASE("Hybrid strategy caps initial chunks correctly") {
      auto settings = test_helpers::search_strategy_test_helpers::CreateHybridSettings(75);
      size_t total_results = test_helpers::search_strategy_test_helpers::TestGetFuturesAndCollect(
    ...
    TEST_CASE("Hybrid strategy respects initial work percentage") {
      ...
      for (int percent : {50, 60, 70, 80, 90}) {
        auto settings = test_helpers::search_strategy_test_helpers::CreateHybridSettings(percent);
        test_helpers::TestSettingsFixture settings_fixture(settings);
    ...
    TEST_CASE("Hybrid strategy handles overlapping concurrent searches") {
      test_helpers::TestSettingsFixture settings("hybrid");
      ...
      // is running
      std::vector<std::string> queries = {"file_", "file_", "file_"};
      auto results = test_helpers::search_strategy_test_helpers::RunConcurrentSearches(
        index_fixture.GetIndex(), queries, 4, true);

      // All searches should complete and return results
      REQUIRE(results.size() == 3);
      REQUIRE(results[0].total_results > 0);
      REQUIRE(results[1].total_results > 0);
      REQUIRE(results[2].total_results > 0);
    }
  ...
  TEST_SUITE("Dynamic Strategy") {
    ...
    TEST_CASE("Dynamic strategy processes all work dynamically") {
      test_helpers::TestSettingsFixture settings("dynamic");
    ...
    TEST_CASE("Dynamic strategy handles extension filter") {
      ...
      std::vector<std::string> extensions = {".txt", ".cpp", ".h"};
    ...
    TEST_CASE("SearchContext data preserved through async execution") {
      ...
      std::vector<std::string> fake_extensions = {".xyz_unique_12345"};
    ...
    TEST_CASE("Search in deep hierarchy") {
      ...
      REQUIRE(results.size() == 1);
      CHECK(results[0].fullPath.find("level_20") != std::string::npos);
    }
    ...
    TEST_CASE("Dynamic strategy handles overlapping concurrent searches") {
      ...
      REQUIRE((results[0].total_results > 0 || results[1].total_results > 0 ||
               results[2].total_results > 0));
    }

    TEST_CASE("Dynamic strategy handles max iterations limit") {
      auto settings = test_helpers::search_strategy_test_helpers::CreateDynamicSettings(
  ...
  TEST_SUITE("Pattern Matcher Setup") {
    ...
    TEST_CASE("PathPattern patterns (auto-detected with ^) work across all strategies") {
      ...
      test_helpers::search_strategy_test_helpers::RunTestForAllStrategiesWithSetup(
        100, [](FileIndex& idx, const std::string&) {
          ...
          for (const auto& result : results) {
            std::string_view path_view(result.fullPath);
            std::string_view filename = path_utils::GetFilename(path_view);
    ...
    TEST_CASE("Extension-only mode skips pattern matchers") {
      ...
      for (const auto& result : results) {
        std::string_view path_view(result.fullPath);
        if (size_t last_dot = path_view.find_last_of('.');
            last_dot != std::string_view::npos && last_dot + 1 < path_view.length()) {
          std::string_view ext = path_view.substr(last_dot + 1);
          bool has_match = (ext == "txt" || ext == "cpp");
          CHECK(has_match);
        }
      }
    }
