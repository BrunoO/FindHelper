...
void PopulateDeepHierarchy(FileIndex& index, int depth, const std::string& base_name = "level") {
  ...
  index.Insert(ntfs_file_reference::NtfsFileReference(current_id), ntfs_file_reference::NtfsFileReference(parent_id), current_path, true, {0, 0});
  parent_id = current_id;

  for (int i = 1; i <= depth; ++i) {
    const std::string name = base_name + "_" + std::to_string(i);
    current_id++;
    index.Insert(ntfs_file_reference::NtfsFileReference(current_id), ntfs_file_reference::NtfsFileReference(parent_id), name, true, {0, 0});
    parent_id = current_id;

    // Add a file in each level
    index.Insert(ntfs_file_reference::NtfsFileReference(current_id + 1000), ntfs_file_reference::NtfsFileReference(parent_id), "file_at_level_" + std::to_string(i) + ".txt", false,
                 kFileTimeNotLoaded);
  }
...
CollectSearchResults(  // NOSONAR(cpp:S107) - Test-only convenience wrapper with explicit parameters
  ...
  auto futures = index.SearchAsyncWithData(query, thread_count, nullptr, path_query, extensions,
                                           folders_only, case_sensitive, thread_timings, nullptr,
                                           nullptr);
...
std::vector<SearchResultData>
CollectSearchResultsWithType(
  FileIndex& index, const std::string& query, int thread_count = -1,
  const std::vector<std::string>* extensions = nullptr,
  ItemTypeFilter item_type = ItemTypeFilter::All,
  bool case_sensitive = false, const std::string& path_query = "") {
  std::vector<ThreadTiming> timings;
  auto futures = index.SearchAsyncWithData(query, thread_count, nullptr, path_query, extensions,
                                           item_type, case_sensitive, &timings, nullptr,
                                           nullptr);
  return test_helpers::CollectFutures(futures);
}
...
std::vector<std::future<SearchResultBatch>> SearchAsyncDefault(
...
TEST_SUITE("FileIndex Search Strategies") {
  ...
  TEST_SUITE("Hybrid Strategy - Dynamic Phase") {
    ...
    TEST_CASE("Dynamic strategy handles FilesOnly item_type_filter") {
      const test_helpers::TestSettingsFixture settings("hybrid");
      test_helpers::TestFileIndexFixture index_fixture(1000);

      auto results = CollectSearchResultsWithType(index_fixture.GetIndex(), "file_", 4, nullptr, ItemTypeFilter::FilesOnly);

      REQUIRE(!results.empty());
      // Should only return files (non-directories)
      for (const auto& result : results) {
        REQUIRE(!result.isDirectory);
      }
    }
      ...
      for (int attempt = 0; attempt < kMaxAttempts; ++attempt) {
        ...
        auto writer_task = [&index, &writer_error] {
          ...
          for (int i = 0; i < kWriterIterations && !writer_error.load(); ++i) {
            ...
            index.Insert(ntfs_file_reference::NtfsFileReference(id), ntfs_file_reference::NtfsFileReference(1), name, false, kFileTimeNotLoaded);
            index.Remove(ntfs_file_reference::NtfsFileReference(id));
          }
        };
