...
// Shared single-file tree (root + a.txt) with an aggregator already
// requesting root. Covers the dedup / Reset / in-flight / fresh-count cases,
// which differ only in file size and tail assertions
// (Sonar: duplicate setup blocks).
struct SingleFileRootFixture {
  FileIndex index;
  FolderSizeAggregator aggregator{index};
  std::string root_path =
      path_utils::JoinPath(path_utils::GetDefaultVolumeRootPath(), "root");
  explicit SingleFileRootFixture(uint64_t file_size) {
    index.Insert(ntfs_file_reference::NtfsFileReference(1), ntfs_file_reference::NtfsFileReference(0), "root", true);
    index.Insert(ntfs_file_reference::NtfsFileReference(2), ntfs_file_reference::NtfsFileReference(1), "a.txt", false, {0, 0}, file_size);
    index.RecomputeAllPaths();
...
TEST_CASE("FolderSizeAggregator - deduplicates requests") {
  SingleFileRootFixture fixture{100};

  fixture.aggregator.Request(1, fixture.root_path);  // Should be a no-op.

  const auto result = WaitForResult(fixture.aggregator, 1);
  REQUIRE(result.has_value());
...
TEST_CASE("FolderSizeAggregator - Reset clears results") {
  ...
  const auto before_reset = WaitForResult(fixture.aggregator, 1);
  CHECK(before_reset.has_value());

  fixture.aggregator.Reset();

  CHECK_FALSE(fixture.aggregator.GetResult(1).has_value());
}
...
TEST_CASE("FolderSizeAggregator - Reset discards in-flight result") {
  ...
  SingleFileRootFixture fixture{42};

  fixture.aggregator.Reset();  // Bumps generation before job completes.
...
TEST_CASE("FolderSizeAggregator - Reset required after index inserts for fresh file_count") {
  ...
  SingleFileRootFixture fixture{100};
  FileIndex& index = fixture.index;
  FolderSizeAggregator& aggregator = fixture.aggregator;
  const std::string& root_path = fixture.root_path;
