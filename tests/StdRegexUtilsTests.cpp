...
TEST_SUITE("StdRegexUtils") {
  ...
  TEST_SUITE("Simple pattern matching (SimpleRegex path)") {
    ...
    TEST_CASE("Caret and dollar anchors in partial matches") {
      SUBCASE("Caret matches start") {
        const std::vector<test_helpers::RegexMatchTestCase> test_cases = {
          {"^hello", "hello world", true},
...
// ---------------------------------------------------------------------------
// ExtractRequiredLiteral: branch-level unit tests
// One TEST_CASE per distinct code path in the function.
// NOTE (Sonar triage): the repetitive TEST_CASE wrappers below are intentional.
// Each case documents one branch and keeps failure attribution precise; folding
// them into a single table would hurt readability for no production-code gain.
// Marked Won't-Fix in SonarCloud (duplication in test code only).
// ---------------------------------------------------------------------------
TEST_SUITE("StdRegexUtils - detail::ExtractRequiredLiteral") {

  void RunExtractRequiredLiteralCases(
      std::initializer_list<std::pair<std::string_view, std::string_view>> test_cases) {
    for (const auto& [pattern, expected] : test_cases) {
      CHECK(std_regex_utils::detail::ExtractRequiredLiteral(pattern) == expected);
    }
  }

  TEST_CASE("anchors are transparent to the literal run") {
    RunExtractRequiredLiteralCases({
        {"^report", "report"},
        {"pdf$", "pdf"},
        {"^pdf$", "pdf"},
        {"^abc$", "abc"},
  ...
  TEST_CASE("escaped metachar is treated as a required literal") {
    RunExtractRequiredLiteralCases({
        {"\\.pdf", ".pdf"},
        {"\\.tar\\.gz", ".tar.gz"},
    });
  ...
  TEST_CASE("character-class escape stops the run; suffix may still be extracted") {
    RunExtractRequiredLiteralCases({
        {"report_\\d{4}", "report_"},
        {"\\d{4}", ""},
        {"\\w+test", "test"},
        {"prefix\\s+rest", "prefix"},
    });
    // \w+ stops the prefix; "test" is then extracted as the suffix run.
  }

  TEST_CASE("character class [...] stops the run; suffix may still be extracted") {
    RunExtractRequiredLiteralCases({
        {"file[0-9].txt", "file"},
        {"[abc]test", "test"},
        {"pre[A-Z]+suf", "pre"},
    });
    // [abc] stops the prefix; "test" is then extracted as the suffix run.
  }

  TEST_CASE("dot stops the run; following quantifier is consumed") {
    RunExtractRequiredLiteralCases({
        {"pre.suf", "pre"},
        {"pre.*suf", "pre"},
        {"pre.+suf", "pre"},
    ...
    // Dot followed by brace quantifier: the {…} branch inside the dot block
  }

  TEST_CASE("top-level alternation returns empty immediately") {
    RunExtractRequiredLiteralCases({
        {"pdf|log", ""},
        {"report|invoice", ""},
        {"abc|def|ghi", ""},
  ...
  TEST_CASE("group content is skipped; alternation inside group returns empty") {
    RunExtractRequiredLiteralCases({
        {"(\\.pdf|\\.log)", ""},
        {"(report|invoice)", ""},
    });
  ...
  TEST_CASE("literal after closing group is extracted") {
    // Content inside () is skipped, but the suffix outside is a required literal.
    RunExtractRequiredLiteralCases({
        {"(report|invoice)\\.xlsx", ".xlsx"},
        {"(a|b)suffix", "suffix"},
    });
  }

  TEST_CASE("plain char with * or ? makes it optional — char excluded from run") {
    RunExtractRequiredLiteralCases({
        {"a*bc", "bc"},
        {"a?bc", "bc"},
        {"xxa*bc", "xx"},
        {"prefix_a?suffix", "prefix_"},
    });
    // "xx" and "bc" are the same length (2); the first run wins (strict > comparison).
  }

  TEST_CASE("plain char with + is required — char included in run") {
    RunExtractRequiredLiteralCases({
        {"a+bc", "abc"},
        {"xa+b", "xab"},
    });
  ...
  TEST_CASE("plain char with {…} quantifier stops the run") {
    RunExtractRequiredLiteralCases({
        {"a{2}bc", "bc"},
        {"pre_a{3,5}suf", "pre_"},
    });
  ...
  TEST_CASE("escaped char with * or ? makes it optional — excluded from run") {
    RunExtractRequiredLiteralCases({
        {"\\.?pdf", "pdf"},
        {"\\(?abc", "abc"},
    });
  ...
  TEST_CASE("escaped char with + is required — included in run") {
    RunExtractRequiredLiteralCases({
        {"\\.+pdf", ".pdf"},
    });
  }

  TEST_CASE("escaped char with {…} quantifier stops the run") {
    RunExtractRequiredLiteralCases({
        {"\\a{3}bc", "bc"},
    });
  }

  TEST_CASE("longest run wins when multiple disjoint runs exist") {
    RunExtractRequiredLiteralCases({
        {"a\\d{4}long_literal", "long_literal"},
        {"report_\\d{4}\\.xlsx", "report_"},
    });
  }

  TEST_CASE("empty and all-literal patterns") {
    RunExtractRequiredLiteralCases({
        {"", ""},
        {"abc", "abc"},
  ...
  TEST_SUITE("FindRegexSpans") {
    TEST_CASE("highlights all non-overlapping matches") {
      std::vector<string_search::MatchSpan> spans;
      CHECK(std_regex_utils::FindRegexSpans(R"(file\d+)", "pref_file42_file7_suf", true, spans));
      REQUIRE(spans.size() == 2);
      CHECK(spans[0].start_ == 5);
      CHECK(spans[0].end_ == 11);  // "file42"
      CHECK(spans[1].start_ == 12);
      CHECK(spans[1].end_ == 17);  // "file7"
    }

    TEST_CASE("anchored pattern highlights whole string (single match)") {
      std::vector<string_search::MatchSpan> spans;
      CHECK(std_regex_utils::FindRegexSpans("^report_.*\\.txt$", "report_final.txt", true, spans));
      REQUIRE(spans.size() == 1);
      CHECK(spans[0].start_ == 0);
      CHECK(spans[0].end_ == 16);
    }

    TEST_CASE("case-insensitive finds all matches") {
      std::vector<string_search::MatchSpan> spans;
      CHECK(std_regex_utils::FindRegexSpans("hello", "Say HELLO and hello there", false, spans));
      REQUIRE(spans.size() == 2);
      CHECK(spans[0].start_ == 4);
      CHECK(spans[0].end_ == 9);
      CHECK(spans[1].start_ == 14);
      CHECK(spans[1].end_ == 19);
    }

    TEST_CASE("no match and invalid patterns yield empty spans") {
      std::vector<string_search::MatchSpan> spans;
      CHECK_FALSE(std_regex_utils::FindRegexSpans("xyz", "hello", true, spans));
      CHECK(spans.empty());
      CHECK_FALSE(std_regex_utils::FindRegexSpans("", "hello", true, spans));
      CHECK_FALSE(std_regex_utils::FindRegexSpans("(", "hello", true, spans));
    }
  }
}
