...
void Insert(FileIndexStorage& storage, uint64_t id, uint64_t parent_id,
    ...
    storage.InsertLocked(ntfs_file_reference::NtfsFileReference(id),
...
TEST_SUITE("PathBuilder::BuildFullPathWithLogging") {
  ...
  TEST_CASE("FRN sequence mismatch resolves parent chain via record number") {
    // Simulates NTFS ParentFileReferenceNumber with a stale sequence while the
    // parent directory is indexed under the current sequence (git remotes/origin case).
    std::shared_mutex mutex;
    FileIndexStorage storage(mutex);

    Insert(storage, 1, 1, "refs", true);
    const uint64_t remotes_id = 0x0002000000000050ULL;
    Insert(storage, remotes_id, 1, "remotes", true);

    const uint64_t stale_remotes_ref = 0x0001000000000050ULL;
    const uint64_t origin_id = 0x0003000000000060ULL;
    Insert(storage, origin_id, stale_remotes_ref, "origin", true);
    Insert(storage, 0x0004000000000070ULL, origin_id, "HEAD");

    const std::string path = PathBuilder::BuildFullPathWithLogging(
        0x0004000000000070ULL, origin_id, "HEAD", storage, storage.GetNameCache());

    CHECK(path == BuildExpectedPath({"refs", "remotes", "origin", "HEAD"}));
  }

  TEST_CASE("stale parent FRN prefers canonical record over exact stale id") {
    std::shared_mutex mutex;
    FileIndexStorage storage(mutex);

    Insert(storage, 1, 1, "refs", true);
    const uint64_t stale_remotes_ref = 0x0001000000000060ULL;
    const uint64_t remotes_id = 0x0002000000000060ULL;
    Insert(storage, stale_remotes_ref, 1, "wrong", true);
    Insert(storage, remotes_id, 1, "remotes", true);

    const uint64_t origin_id = 0x0003000000000070ULL;
    Insert(storage, origin_id, stale_remotes_ref, "origin", true);

    const std::string path = PathBuilder::BuildFullPathWithLogging(
        origin_id, stale_remotes_ref, "origin", storage, storage.GetNameCache());

    CHECK(path == BuildExpectedPath({"refs", "remotes", "origin"}));
  }
