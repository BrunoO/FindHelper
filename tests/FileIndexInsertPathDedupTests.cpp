...
TEST_CASE("RecomputeAllPaths populates path_to_id map enabling deduplication and unlinking") {
  FileIndex index;
  constexpr uint64_t kVolumeRootFrn = 0x0001000000000005ULL;
  ...
  // InsertPath with identical path must deduplicate and NOT increment index size
  index.InsertPath(file_path, false);
  CHECK(index.Size() == 2);

  const std::string dir_path = index.GetPathAccessor().GetPathCopy(kDirId);
  CHECK_FALSE(dir_path.empty());
  index.InsertPath(dir_path, true);
  CHECK(index.Size() == 2);

  // Removing the file unlinks it from path_to_id
  index.Remove(ntfs_file_reference::NtfsFileReference(kFileId));
  CHECK(index.Size() == 1);
  CHECK(index.GetEntry(kFileId) == nullptr);

  // Subsequent InsertPath succeeds because the previous one was unlinked.
  // NOTE (portability): the exact size is platform-dependent and must not be
  // pinned. InsertPath resolves the parent via DirectoryResolver, whose
  // directory cache batch inserts never populate, so it synthesizes the
  // missing ancestors alongside the reinserted file — and ancestor depth
  // varies with platform path shape (observed: 3 total on macOS/Linux,
  // 4 on Windows). Pre-existing behavior, orthogonal to the single-pass fold
  // (verified identical on the pre-fold code). The portable contract pinned
  // here is: reinsert lands as a NEW entry (old id stays gone), then dedups.
  index.InsertPath(file_path, false);
  const size_t size_after_reinsert = index.Size();
  CHECK(size_after_reinsert > 1);
  CHECK(index.GetEntry(kFileId) == nullptr);

  // Subsequent identical InsertPath deduplicates the newly inserted path
  index.InsertPath(file_path, false);
  CHECK(index.Size() == size_after_reinsert);
}
