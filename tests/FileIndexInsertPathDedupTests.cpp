...
TEST_CASE("InsertBatch defers healing to RecomputeAllPaths") {
  ...
  constexpr uint64_t kParent = 0x00010000000001A0ULL;
  constexpr uint64_t kChild = 0x00010000000001B0ULL;

  FileIndex::PopulationBatchEntry child{};
  child.id = ntfs_file_reference::NtfsFileReference(kChild);
  child.parent_id = ntfs_file_reference::NtfsFileReference(kParent);
  child.name = file_name::FileName("bulk.txt");
  child.is_directory = false;
  FileIndex::PopulationBatchEntry parent{};
  parent.id = ntfs_file_reference::NtfsFileReference(kParent);
  parent.parent_id = ntfs_file_reference::NtfsFileReference(0);
  parent.name = file_name::FileName("BulkDir");
  parent.is_directory = true;
  index.InsertBatch({child, parent});

  CHECK(index.GetPathAccessor().GetPathCopy(kChild) == "bulk.txt");
...
TEST_CASE("WalkIndexedPath matches materialized paths after mixed inserts and recompute") {
  ...
  auto track_insert = [&](uint64_t id, uint64_t parent, std::string_view name, bool is_dir) {
    index.Insert(ntfs_file_reference::NtfsFileReference(id), ntfs_file_reference::NtfsFileReference(parent), name, is_dir);
  ...
  track_insert(1, kVolumeRoot, "root", true);
  // Out-of-order: files before parents, then heal via parent inserts.
  track_insert(kFileC, kFileB, "c.txt", false);
  track_insert(kFileB, kDirA, "b.txt", false);
  track_insert(kDirD, 1, "DirD", true);
  track_insert(kDirA, 1, "DirA", true);

  // Rename + move before the topological pass (map tracks the rename).
  CHECK(index.Rename(ntfs_file_reference::NtfsFileReference(kFileB), "renamed.txt"));
  names[kFileB] = "renamed.txt";
  CHECK(index.Move(ntfs_file_reference::NtfsFileReference(kFileC), ntfs_file_reference::NtfsFileReference(kDirD)));

  index.RecomputeAllPaths();
  REQUIRE(index.GetAwaitingStats().count == 0);

  for (const uint64_t id : ids) {
    const auto walked = WalkIndexedPath(index, id, names);
    REQUIRE(walked.has_value());
    CHECK(*walked == index.GetPathAccessor().GetPathCopy(id));
  }
  CHECK(index.CheckBareNameInvariant());
}
...
TEST_CASE("WalkIndexedPath agrees with awaiting tracking without recompute") {
  ...
  track_insert(1, kVolumeRoot, "root", true);
  track_insert(kFileB, kDirA, "b.txt", false);  // Parent missing: awaiting.
  track_insert(kDirD, 1, "DirD", true);          // Parent present: resolved.
  REQUIRE(index.GetAwaitingStats().count == 1);
  ...
  for (const uint64_t id : ids) {
    const auto walked = WalkIndexedPath(index, id, names);
    if (index.IsAwaitingChild(ntfs_file_reference::NtfsFileReference(id))) {
