...
TEST_CASE("RecomputeAllPaths populates path_to_id map enabling deduplication and unlinking") {
  ...
  // Size is 3, not 2: InsertPath resolves the parent via DirectoryResolver,
  // whose directory cache batch inserts never populate, so it creates a
  // synthetic duplicate parent alongside the reinserted file. Pre-existing
  // behavior, orthogonal to the single-pass fold (verified identical on the
  // pre-fold code) — pinned here, not fixed.
  index.InsertPath(file_path, false);
  const size_t size_after_reinsert = index.Size();
  CHECK(size_after_reinsert == 3);
