 ...
 * - String options (index-from-file, crawl-folder, dump-index-to, win-volume):
 *   both formats
 * - Unknown arguments are collected, warned about, and otherwise ignored
...
TEST_SUITE("ParseCommandLineArgs") {
  ...
  TEST_CASE("No arguments returns all defaults") {
    ...
    CHECK(result.startup_args_display.empty());
  }

  TEST_CASE("startup_args_display captures provided arguments") {
    SUBCASE("single argument") {
      Args args({"prog", "--help"});
      const CommandLineArgs result = ParseCommandLineArgs(args.argc(), args.argv());
      CHECK(result.startup_args_display == "--help");
    }
    SUBCASE("multiple arguments preserve order and spacing") {
      Args args({"prog", "--thread-pool-size=4", "--crawl-folder", "/tmp/docs"});
      const CommandLineArgs result = ParseCommandLineArgs(args.argc(), args.argv());
      CHECK(result.startup_args_display ==
            "--thread-pool-size=4 --crawl-folder /tmp/docs");
    }
  }
  ...
  TEST_CASE("Unknown arguments are collected, warned about, and ignored") {
    Args args({"prog", "--unknown-flag", "--another=value", "bare-arg"});
    const CommandLineArgs result = ParseCommandLineArgs(args.argc(), args.argv());
    CHECK(!result.show_help);
    CHECK(!result.exit_requested);
    CHECK(result.thread_pool_size_override == -1);
    CHECK(result.unknown_args == std::vector<std::string>{"--unknown-flag", "--another=value", "bare-arg"});
    CHECK(result.unknown_args_display == "--unknown-flag --another=value bare-arg");
  }

  TEST_CASE("No unknown arguments leaves unknown args empty") {
    Args args({"prog", "--thread-pool-size=4"});
    const CommandLineArgs result = ParseCommandLineArgs(args.argc(), args.argv());
    CHECK(result.unknown_args.empty());
    CHECK(result.unknown_args_display.empty());
    CHECK(result.thread_pool_size_override == 4);
  }
  ...
  TEST_CASE("Help flag alongside other arguments") {
    Args args({"prog", "--help", "--thread-pool-size=4"});
    const CommandLineArgs result = ParseCommandLineArgs(args.argc(), args.argv());
    CHECK(result.show_help);
    CHECK(result.exit_requested);
