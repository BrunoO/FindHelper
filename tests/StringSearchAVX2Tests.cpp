...
TEST_SUITE("StringSearch AVX2") {
    ...
    TEST_SUITE("AVX2 Integration") {
        TEST_CASE("IsViewAsciiAVX2 accepts long ASCII buffer") {
            std::string text(100, 'a');
            text += "ASCII_ONLY";
            text += std::string(100, 'b');
            CHECK(string_search::avx2::IsViewAsciiAVX2(text.data(), text.size()));
        }

        TEST_CASE("IsViewAsciiAVX2 rejects non-ASCII after byte 64") {
            std::string text(80, 'a');
            text[70] = static_cast<char>(0x80);
            CHECK_FALSE(string_search::avx2::IsViewAsciiAVX2(text.data(), text.size()));
        }
        ...
        TEST_CASE("case-sensitive SIMD with non-ASCII haystack byte") {
            std::string text(100, 'a');
            text[50] = static_cast<char>(0x80);
            text += "ExactToken";
            text += std::string(50, 'b');
            REQUIRE(string_search::ContainsSubstring(text, "ExactToken"));
        }
    }
