...
TEST_CASE("UsnJournalQueue - Push and Pop round-trip") {
    UsnJournalQueue q(4);
    const std::vector payload = {'a', 'b', 'c'};
    CHECK(q.Push(std::vector(payload)));
...
TEST_CASE("UsnJournalQueue - Stop discards pending items") {
    UsnJournalQueue q(4);
    q.Push({'a'});
    q.Push({'b'});
    q.Stop();

    // Shutdown must not apply a multi-thousand-buffer backlog on join.
    CHECK(q.Size() == 0);
    std::vector<char> out;
