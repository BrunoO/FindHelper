...
// ---------------------------------------------------------------------------
// 1. MatchPattern: unit-level correctness for rs: patterns
//    Tests the underlying regex semantics without any pre-filter involved.
// ---------------------------------------------------------------------------
TEST_SUITE("RegexSearchPrefilter - MatchPattern correctness") {

    TEST_CASE("extension anchor patterns") {
        // Suffix anchor: must match exactly at end of string.
        const auto matcher = [](std::string_view text) {
            return search_pattern_utils::MatchPattern("rs:\\.pdf$", text, false);
        };
    ...
    TEST_CASE("start-of-name anchor patterns") {
        const auto matcher = [](std::string_view text) {
            return search_pattern_utils::MatchPattern("rs:^report", text, false);
        };
        ExpectPrefixAnchorBehavior(matcher);
    }

    TEST_CASE("mid-name literal with digit class") {
        CHECK(search_pattern_utils::MatchPattern("rs:report_\\d{4}",  "report_2024.xlsx", false));
        CHECK_FALSE(search_pattern_utils::MatchPattern("rs:report_\\d{4}", "report_abcd.xlsx", false));
        CHECK_FALSE(search_pattern_utils::MatchPattern("rs:report_\\d{4}", "report_123.xlsx",  false)); // 3 digits
    }

    TEST_CASE("compound extension anchor") {
        CHECK(search_pattern_utils::MatchPattern("rs:\\.tar\\.gz$",   "archive.tar.gz",   false));
        CHECK_FALSE(search_pattern_utils::MatchPattern("rs:\\.tar\\.gz$", "archive.tar",  false));
        CHECK_FALSE(search_pattern_utils::MatchPattern("rs:\\.tar\\.gz$", "archive.gz",   false));
    }

    TEST_CASE("alternation - both branches must match") {
        // No single literal can pre-filter this pattern.
        const auto matcher = [](std::string_view text) {
            return search_pattern_utils::MatchPattern("rs:(\\.pdf|\\.log)$", text, false);
        };
        ExpectAlternationBehavior(matcher);
    }

    TEST_CASE("pure character class - no literal to extract") {
        // \d{4} contains no literal string; pre-filter must pass all files through.
        const auto four_digits_matcher = [](std::string_view text) {
            return search_pattern_utils::MatchPattern("rs:\\d{4}", text, false);
        };
        const auto starts_with_digit_matcher = [](std::string_view text) {
            return search_pattern_utils::MatchPattern("rs:^\\d", text, false);
        };
    ...
    TEST_CASE("case sensitivity: pre-filter must mirror regex icase behaviour") {
        // Case-insensitive: uppercase extension must still match lowercase pattern.
        CHECK(search_pattern_utils::MatchPattern("rs:\\.pdf$",     "REPORT.PDF",         false));
        CHECK(search_pattern_utils::MatchPattern("rs:^report",     "REPORT_2024.xlsx",   false));
        // Case-sensitive: uppercase text must NOT match lowercase pattern.
        CHECK_FALSE(search_pattern_utils::MatchPattern("rs:\\.pdf$", "REPORT.PDF",       true));
        CHECK_FALSE(search_pattern_utils::MatchPattern("rs:^report", "REPORT_2024.xlsx", true));
        // Case-sensitive: exact case matches.
        CHECK(search_pattern_utils::MatchPattern("rs:\\.PDF$",     "REPORT.PDF",         true));
        CHECK(search_pattern_utils::MatchPattern("rs:^REPORT",     "REPORT_2024.xlsx",   true));
    }

    TEST_CASE("escaped special characters in pattern") {
        // Literal parentheses in the filename text must not confuse the pattern.
        CHECK(search_pattern_utils::MatchPattern("rs:file\\(1\\)", "file(1).txt", false));
        CHECK_FALSE(search_pattern_utils::MatchPattern("rs:file\\(1\\)", "file1.txt", false));
    }

    TEST_CASE("no-match pattern returns empty results") {
        CHECK_FALSE(search_pattern_utils::MatchPattern("rs:^XXXX_NO_SUCH_FILE", "report.pdf",  false));
        CHECK_FALSE(search_pattern_utils::MatchPattern("rs:^XXXX_NO_SUCH_FILE", "config.json", false));
    }
}

// ---------------------------------------------------------------------------
// 2. CreateFilenameMatcher: callable-level tests
...
// ---------------------------------------------------------------------------
// 3. SearchAsyncWithData integration: full parallel search pipeline
