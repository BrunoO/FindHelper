...
TEST_SUITE("ParallelSearchEngine - SearchAsync") {
  ...
  TEST_CASE("ProcessChunkRangeIds - polls cancellation in chunks without interval multiples") {
    ...
    // to the chunk start, mirroring ProcessChunkRange's items_checked counter.
    MockSearchableIndex index;
    ...
    lightweight_callable::LightweightCallable<bool, std::string_view> no_matcher;

    // Chunk [1, 128): contains no multiple of kCancellationCheckInterval.
    std::vector<uint64_t> ids;
    ParallelSearchEngine::ProcessChunkRangeIds(view, 1, 128, ids, ctx,
                                               index.GetStorageSize(), no_matcher, no_matcher);
    CHECK(ids.empty());  // Cancelled at the very first item of the chunk
    ...
    // early return (ValidateChunkRangeIds rejects silently).
    cancel_flag = false;
    ParallelSearchEngine::ProcessChunkRangeIds(view, 1, 128, ids, ctx,
                                               index.GetStorageSize(), no_matcher, no_matcher);
    CHECK(ids.size() == 127);
