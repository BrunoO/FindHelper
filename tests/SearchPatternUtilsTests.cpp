...
TEST_SUITE("SearchPatternUtils - MatchPattern") {
  ...
  TEST_CASE("path pattern via pp: prefix") {
    CHECK(search_pattern_utils::MatchPattern("pp:**/*.cpp", "src/utils/Logger.cpp", false));
    CHECK_FALSE(search_pattern_utils::MatchPattern("pp:**/*.cpp", "src/utils/Logger.h", false));
  }

  TEST_CASE("path pattern auto-detected via double-star") {
    // "**" triggers PathPattern auto-detection (no pp: prefix needed).
    CHECK(search_pattern_utils::MatchPattern("**/*.txt", "docs/file.txt", false));
    CHECK_FALSE(search_pattern_utils::MatchPattern("**/*.txt", "docs/file.cpp", false));
  }
}
