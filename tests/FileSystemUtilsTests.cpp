...
TEST_SUITE("FileSystemUtils") {
    ...
    TEST_SUITE("GetFileAttributes") {
        ...
        TEST_CASE("const char* overload handles non-existent file and nullptr") {
            const char* nonExistentPath = "/nonexistent/path/to/file/that/should/not/exist.txt";
            FileAttributes attrs = GetFileAttributes(nonExistentPath);
            ...
            FileAttributes null_attrs = GetFileAttributes(static_cast<const char*>(nullptr));
            CHECK(null_attrs.success == false);
            CHECK(null_attrs.fileSize == kFileSizeFailed);
        }
    }
