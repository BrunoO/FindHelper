...
**Status (2026-09-04):** This addendum still describes the **§9 minimal** semantics (single visible buffer, no early clear). Debounced instant search now matches auto-refresh (keep visible until replace). The **ephemeral back-buffer handoff** (pre-sort, skip-swap, directory reconcile) is documented in `2026-02-24_RESULTS_AUTO_REFRESH_DOUBLE_BUFFERING_SPEC.md` **§11** and is outside this TLA model’s scope (the model cares that the *visible* buffer is not cleared early; promotion vs skip is an implementation detail of `PollResults`).

### 1. Context and Goal

This note complements `2026-02-24_RESULTS_AUTO_REFRESH_DOUBLE_BUFFERING_SPEC.md`.

That spec describes designs for fixing the “blank table on auto-refresh” issue:

- A **full front/back double-buffering** design (sections 1–8), where `GuiState` owns explicit front and back buffers and promotion semantics — **not implemented**.
- A **minimal alternative** (section 9), which keeps today’s single **visible** buffer but changes **when** results are cleared for auto-refresh / debounce — **shipped**.
- An **ephemeral PollResults back buffer** (section 11) for sort-before-promote and skip-swap — **shipped**; does not change the TLA “no early clear” invariant.

For §9:

- Manual searches still call `ClearSearchResults` at search start.
- Auto-refresh and **debounced instant search** do **not** clear the visible pool at start; they keep old results until `PollResults` replaces or skip-swaps.

This document focuses on the **minimal §9 alternative**. It provides:
...
- **Manual** searches clear `searchResults` immediately at start.
- **Auto-refresh** (and, in the shipped product, **debounced** search) does **not** clear `searchResults` at start; it leaves them unchanged until the moment a new full result set is ready, which then replaces `searchResults` in one step (or skip-swaps — §11).
