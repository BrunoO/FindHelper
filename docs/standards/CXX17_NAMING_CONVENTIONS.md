...
#### **Struct and POD Data Members (Public Fields)**
**Convention:** `snake_case` (without trailing underscore)

Data members of plain structs, PODs, and context parameter bundles are named like ordinary variables:

```cpp
struct CommandLineArgs {
  bool show_help = false;
  size_t thread_count = 0;
  std::string crawl_folder;
};
...
- Public data members in aggregates and context structs represent direct data fields rather than encapsulated internal state
- Distinguishes public struct fields (`args.show_help`) from encapsulated class members (`monitor.is_active_`)
- Note: `GuiState` members use `camelCase` by design exception (UI state aggregate)

#### **Value-Object Comparison Operators (Hidden Friends)**
**Convention:** `operator==` / `operator!=` on value types are hidden friends, not members.

New domain value types (`NtfsFileReference`, `FilteredDirectory`, `JournalCursor`, …) follow this checklist:
- `explicit` constructor, `[[nodiscard]]` on all observers
- comparison as hidden friends (symmetric, ADL-found; Sonar `cpp:S2807` flags member operators)
