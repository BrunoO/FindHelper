...
// Else-if chain (one init-statement; variable in scope for all branches)
if (int w = std::atoi(value.c_str()); w >= 640 && w <= 4096) {
  args.window_width_override = w;
} else if (w > 4096) {
  args.window_width_override = 4096;
}
