...
| **Shared interface in platform folder** | Prefer a path that does not suggest a single OS. | ✅ `FileOperations.h` is the **common** interface and now lives under `platform/FileOperations.h`; all platforms include it via the shared `platform/` path. |
