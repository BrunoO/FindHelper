...
- **`src/core/main_common.h`** — Shared logic for all platforms. It:
  ...
  4. **Windows only:** If no `--crawl-folder` and process is not elevated, offers “Restart as administrator” via `PromptForElevationAndRestart(nullptr, argc, argv)`. If the user accepts, we call `RestartAsAdmin()` (ShellExecuteEx “runas”), which triggers the system UAC dialog; then this process exits with 0. Original `argv` (minus `argv[0]`, MSVCRT-quoted) is forwarded as `lpParameters` so the elevated process keeps the same options; unrecognized flags warn on stderr and are listed in Help → About.
...
- **`src/platform/windows/ShellContextUtils.cpp`** — `IsProcessElevated()` (used in Settings UI and in main_common for the elevation prompt); `PromptForElevationAndRestart(HWND, argc, argv)` (used at startup when not elevated and no `--crawl-folder`; forwards argv to the elevated process, triggers RestartAsAdmin → UAC).
...
- On Windows, `use_usn_monitor` is true when `bootstrap.monitor != nullptr` and there is no explicit `--crawl-folder` (a persisted Settings crawl folder does not override USN).
