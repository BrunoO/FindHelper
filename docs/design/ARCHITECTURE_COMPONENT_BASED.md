...
- Runs the hybrid parallel schedule (`HybridStrategy` in `LoadBalancingStrategy.cpp`)
- Manages pattern matcher creation
- Aggregates search results
- Determines optimal thread count

**Key Features:**
- Hybrid chunking only (no runtime strategy selection)
...
- Delegates to `HybridStrategy` for work distribution
...
ParallelSearchEngine
  ...
  ├─► HybridStrategy / LoadBalancingStrategy (uses)
