...
6. **SearchWorker / Logger / IndexBuildState**: Each has its own mutex; no ordering is required with FileIndex or UsnMonitor as long as no lock is held across I/O.
...
| UsnJournalQueue     | `std::mutex`        | queue_, stop_                          | No               |
...
When adding new code that holds a lock, add a brief comment if non-obvious (e.g. “No I/O under this lock – see docs/design/LOCK_ORDERING_AND_CRITICAL_SECTIONS.md”).
