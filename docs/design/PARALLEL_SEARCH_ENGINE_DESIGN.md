...
**Date:** December 31, 2025 (load-balancing section updated 2026-04-11)  
**Status:** Production  
**Version:** 1.1
...
3. **Load balancing**
   - Uses **`HybridStrategy`** (`LoadBalancingStrategy.h` / `LoadBalancingStrategy.cpp`) as the only parallel work schedule
   - No runtime strategy selection (settings / CLI / UI do not switch algorithms)
   - Determines optimal thread count via `DetermineThreadCount()`
...
3. **Hybrid chunking**
   - Initial large chunks plus a dynamic tail phase (tunable via `SearchContext`: `dynamic_chunk_size`, `hybrid_initial_percent`, etc.)
...
### Example 3: Tune hybrid chunking parameters

```cpp
// Parallel schedule is always hybrid; tune chunking via SearchContext:
...
- `HybridStrategy` / `LoadBalancingStrategy` headers — hybrid work distribution
...
- **Main architecture:** `docs/design/ARCHITECTURE_COMPONENT_BASED.md`
...
| 2026-04-11 | 1.1 | — | Document hybrid-only schedule; remove multi-strategy / streaming future item; fix doc paths |
