...
- [ ] **Documentation organization**: If creating or updating documentation, ensure it's in the correct location:
  ...
  - **Historical documents**: Move completed work summaries, implementation reports, and code reviews to a maintainer-only historical location
  - **Obsolete documents**: Move superseded plans, old analysis, and experimental approaches to a maintainer-only archive location
  - **Update index**: Update `docs/DOCUMENTATION_INDEX.md` when moving documents in `docs/`
...
add_executable(file_index_search_strategy_tests 
    ParallelSearchEngine.cpp  # ❌ Same file as main target
    # ... other sources
)
