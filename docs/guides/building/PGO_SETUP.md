...
```powershell
# Reconfigure so CMake selects Phase 2, then rebuild
cmake -S . -B build -DENABLE_PGO=ON -DPGO_PHASE=optimize
cmake --build build --config Release
```
...
**How it works**:
- **First build** (`ENABLE_PGO=ON`, no `.pgd` or `-DPGO_PHASE=instrument`): instrumented executable (generates `.pgc` files when run)
- **After training**: merge `.pgc` files into `FindHelper.pgd` next to the executable
- **Second build**: reconfigure so CMake can pick Phase 2. `PGO_PHASE=auto` looks for `FindHelper.pgd` in `<build>/Release/` (Visual Studio) and `<build>/` (Ninja). Or pass `-DPGO_PHASE=optimize` explicitly.
