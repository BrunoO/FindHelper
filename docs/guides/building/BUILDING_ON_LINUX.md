...
**Optional – Boost** (for `-DFAST_LIBS_BOOST=ON`): Install `libboost-dev`. Boost 1.81+ is required; Ubuntu 24.04 provides 1.83. On **Ubuntu 22.04** (default is 1.74), use either the PPA or build from source below.
  ...
  **Option B – Build Boost from source** (any distro, or when PPA is not desired):
  ...
  1. Download a release (1.81+) from [boost.org](https://www.boost.org/users/download/) (e.g. `boost_1_83_0.tar.bz2`).
...
- **Fontconfig**: Not required at build time (nothing links it). Font discovery
  shells out to the `fc-list` tool at runtime (provided by the `fontconfig`
  system package, preinstalled on most distributions).
- **X11**: Required by GLFW on Linux (usually auto-linked, but explicitly linked for compatibility)
- **System idle detection**: stubbed on Linux (no X11 Screen Saver dependency; `libxss-dev` is not needed)
...
1. Verify `fc-list` works: `fc-list : family | head -5` (provided by the `fontconfig` system package, not a build dependency)
2. Check font directories are accessible: `/usr/share/fonts/`, `~/.fonts/`
...
Linux font rendering shells out to `fc-list` for font discovery (no Fontconfig link dependency). Common fonts are automatically mapped:
