...
This script will:
1. Create the `build_tests` directory (or reuse it)
2. Configure CMake with tests enabled (`BUILD_TESTS=ON`)
...
**Solution**: Test targets on macOS/Linux do not need an extra CMake option. `configure_test_target` defines `USN_WINDOWS_TESTS` when `NOT WIN32`. If you still see Windows headers:

1. You are on macOS or Linux (not a Windows toolchain)
2. You configured with `-DBUILD_TESTS=ON` (or used `./scripts/build_tests_macos.sh`)
3. You are building a `*_tests` target, not Windows-only application sources
