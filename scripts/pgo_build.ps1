...
function Do-Phase1 {
    ...
    if (Test-Path $PgdPath) {
        Write-Warn "FindHelper.pgd already exists in $ReleaseDir. Phase1 forces -DPGO_PHASE=instrument so this build stays instrumented."
        Write-Warn "Merge will overwrite $PgdPath after the next training run."
    }
    $cmakeArgs = @("-S", ".", "-B", $BuildDir, "-DENABLE_PGO=ON", "-DPGO_PHASE=instrument", "-DCMAKE_BUILD_TYPE=Release", "-DENABLE_CTRACK_PROFILING=OFF")
    & cmake @cmakeArgs
...
function Do-Phase2 {
    ...
    $cmakeArgs = @("-S", ".", "-B", $BuildDir, "-DENABLE_PGO=ON", "-DPGO_PHASE=optimize", "-DCMAKE_BUILD_TYPE=Release", "-DENABLE_CTRACK_PROFILING=OFF")
    & cmake @cmakeArgs
    if ($LASTEXITCODE -ne 0) { throw "CMake configure failed." }
    $buildArgs = @("--build", $BuildDir, "--config", "Release")
    if ($CleanFirst) { $buildArgs += "--clean-first" }
