...
# CMakeLists.txt reads the same file via file(STRINGS ...).
# Optional targets (e.g. gemini_api_utils_tests) are skipped gracefully in the run loop below.
# Note: while-read used instead of mapfile for bash 3.2 compatibility (macOS system bash).
TEST_TARGETS=()
while IFS= read -r line; do
    TEST_TARGETS+=("$line")
done < "${SCRIPT_DIR}/test_targets.txt"
