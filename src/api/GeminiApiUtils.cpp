...
using nlohmann::json;  // NOLINT(readability-identifier-naming) - match nlohmann library convention
...

{
  ...
  "search_config": {
    ...
    "files_only": false,               // Optional: default false
...
- **pp: (PathPattern)** - PREFERRED for speed, use for simple patterns:
  ...
  - Wildcards: `**` (any chars incl. separators), `*` (in-segment only), `?` (one char)
  - `**/` matches zero or more whole segments (segment boundary kept)
  - Folder starting with: `pp:**/report*/**/*.pdf` (`report_q1`, not `my_report`)
  - Folder containing: `pp:**/*report*/**/*.pdf` (`report_q1` and `my_report`)
  - ✅ Slashes: `/` matches `/` and `\` on all OSes
  - ✅ Fastest matching performance
  - ❌ Does NOT support: alternation `(a|b)`, lookahead `(?!...)`

- **rs: (regex)** - ONLY use when pathPattern is insufficient:
  - **Alternation (REQUIRED for any alternatives)**: PathPattern does NOT support `(a|b)`, e.g. `rs:.*\\.(cpp|hpp|cxx|cc)$`
  - **Lookahead (REQUIRED for exclusions)**: PathPattern does NOT support lookahead `(?!...)`, so use regex for exclusions: `rs:^(?!.*[/\\\\]thirdparty[/\\\\]).*\\.cpp$`
  - ⚠️ Paths in rs: MUST use `[/\\\\]` for separators (JSON) for Windows+Unix.
...
- ✅ CORRECT partial: `pp:**/report*/**/*.pdf` (folder starting with `report`), `pp:**/*report*/**/*.pdf` (folder containing `report`)
- ✅ CORRECT folder scope + filename: `pp:**/*client_xxx*/**` + `"filename": "*devis*"`
- ❌ WRONG: `pp:folder_name/**` or `pp:**/folder_name` (missing prefix/suffix)
- ❌ WRONG: `pp:/client_xxx/` (single leading `/` matches nothing; bare `client_xxx` is exact-only). Always start with `**/`.

### Options
- **folders_only**: true = directories only
- **files_only**: true = files only (exclude directories)
- If both are true, folders_only wins.
...
6. **Language**: queries may be English or French ("dossier contenant X" = folder containing X); same rules apply
...
User: "tous les fichiers contenant devis dans le nom, dans un dossier contenant client_xxx"
Response: {"version":"1.0","search_config":{"path":"pp:**/*client_xxx*/**","filename":"*devis*"}}
...
std::string ExtractTextFromCandidates(const json& response_json) {
  ...
  void ParseAllFields(const json& search_config, SearchConfig& result_config, bool& has_any_fields) {
    ...
    ParseBooleanField(search_config, "files_only", result_config.files_only, has_any_fields);
