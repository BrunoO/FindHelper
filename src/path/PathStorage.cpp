...
size_t PathStorage::InsertPath(uint64_t id, std::string_view path,
  ...
  path = TruncateAtEmbeddedNull(path);
  ...
  if (existing_index.has_value()) {
    ...
    // Length invariant: the SoA search hot path derives length from offset
    // differences (next_offset - offset - 1), never strlen. An in-place shorten
    // would leave the offsets (hence GetPathLength) reflecting the old, longer
    // length with stale tail bytes visible to the matcher. Only equal-length
    // updates can stay in place; shorter or longer entries tombstone + append.
...
void PathStorage::Reserve(size_t expected_entries, size_t expected_path_bytes) {
  path_storage_.reserve(expected_path_bytes);
  path_offsets_.reserve(expected_entries);
  path_ids_.reserve(expected_entries);
  filename_start_.reserve(expected_entries);
  extension_start_.reserve(expected_entries);
  is_deleted_.reserve(expected_entries);
  is_directory_.reserve(expected_entries);
}
...
PathStorage::PathComponentsView PathStorage::GetPathComponentsByIndex(size_t index) const {
  ...
  result.directory_path =
      path_utils::GetDirectoryPathView(result.full_path, filename_offset);
