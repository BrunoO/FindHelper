...
public:
  ...
  /**
   * @brief Whether the entry currently holds a path index
   *
   * False for entries staged with deferred path indexing (bulk population)
   * that RecomputeAllPaths has not indexed yet. Distinguishes
   * expected-by-design missing paths from genuine index inconsistency.
   *
   * @pre Caller must hold shared_lock or unique_lock on mutex
   */
  [[nodiscard]] bool HasPath(uint64_t id) const;
