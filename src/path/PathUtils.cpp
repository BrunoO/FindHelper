...
std::string GetParentDirectory(std::string_view path) {
  // Volume roots are fixed points under GetDirectoryPathView ("C:\\"→"C:\\", "/"→"/").
  // Treat them as having no parent so walks that stop on empty (IndexFromFile, tests) terminate.
  if (IsVolumeRootPath(path)) {
    return {};
  }
  const std::string_view parent = GetDirectoryPathView(path);
  if (parent.empty()) {
    return {};
  }
