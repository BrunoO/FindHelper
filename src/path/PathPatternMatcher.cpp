...
bool NeedsSegmentSemantics(std::string_view pattern) {
  ...
  if (pattern.size() >= 3) {
    const std::string_view tail = pattern.substr(pattern.size() - 3);
    if (tail == "/**" || tail == "\\**") {
      return true;
    }
  }
...
CompiledPathPattern CompileSimplePattern(
  ...
  const auto dfa_start = std::chrono::high_resolution_clock::now();
  ...
  const auto dfa_end = std::chrono::high_resolution_clock::now();
  const auto dfa_duration = std::chrono::duration_cast<std::chrono::microseconds>(dfa_end - dfa_start);  // NOSONAR(cpp:S1481,cpp:S1854) - Used in LOG_INFO_BUILD below
...
CompiledPathPattern CompileAdvancedPattern(
  ...
  const auto parse_duration =  // NOSONAR(cpp:S1481,cpp:S1854) - Used in LOG_INFO_BUILD below
 ...
 * backslash. Unlike CompilePattern, no quantifier parsing applies, so '+',
 * '{' and '}' stay plain literals (e.g. a "C++" folder still matches).
 *
 * @param pattern Pattern string (without anchors)
 * @param case_insensitive Whether matching should be case-insensitive
 * @param compile_start Start time for timing measurements
 * @return Compiled pattern (valid = true on success)
 */
// NOLINTBEGIN(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) - i < pattern.size() by loop condition; i+1 guarded; count < 64 by loop condition
CompiledPathPattern CompileSimplePatternAsBacktracking(
  ...
  if (i < pattern.size()) {
    // Truncated: more input than atom storage holds. Report invalid rather
    // than matching only a prefix of the pattern (mirrors the token limit
    // in CompileSimplePattern via BuildSimpleTokens).
...
RequiredSubstringInfo ExtractRequiredSubstring(std::string_view pattern, bool case_insensitive) {
  ...
  for (const char c : pattern) {  // NOSONAR(cpp:S6184) - Complex stateful processing, not a simple "all_of" check
    ...
    // absorbed).
    if (const bool is_safe_literal = IsSafeLiteralChar(c); !is_safe_literal) {
  ...
  if (current.substring.length() > best.substring.length()) {
    ...
    best.is_suffix = true;
  } else if (current.substring.length() == best.substring.length() && !current.substring.empty() &&
             best.substring == current.substring) {
    // Tie with the same literal text: that text genuinely ends the pattern,
    // so the suffix flag is valid for it. A different text must not inherit
    // the flag (a tied middle run stays a contains check, not suffix).
    best.is_suffix = true;
  }
...
CompiledPathPattern CompilePathPattern(std::string_view pattern, MatchOptions options) {
  ...
  if (!UsesAdvancedFeatures(pattern_without_anchors)) {
    if (NeedsSegmentSemantics(pattern_without_anchors)) {
      ...
      compiled = CompileSimplePattern(pattern_without_anchors, case_insensitive, compile_start);
    }
  } else {
...
bool FindPathPatternSpans(std::string_view pattern, std::string_view text, bool case_sensitive,
  ...
  for (const std::string_view lit : runs) {
    ...
    if (!string_search::FindSubstringSpans(remaining, lit, case_sensitive, one) || one.empty()) {
      continue;
    }
  ...
  if (out_spans.size() >= 2U) {
    size_t write = 0;
    for (size_t read = 1; read < out_spans.size(); ++read) {
      string_search::MatchSpan& cur = out_spans[write];  // NOLINT(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access)
      const string_search::MatchSpan& next = out_spans[read];  // NOLINT(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access)
      if (cur.end_ == next.start_) {
        cur.end_ = next.end_;
      } else {
        ++write;
        out_spans[write] = next;  // NOLINT(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access)
      }
    }
    out_spans.resize(write + 1U);
  }
