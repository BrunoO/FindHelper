...
// Returns true if every byte in [data, data+len) is ASCII (high bit clear).
#if defined(__GNUC__) || defined(__clang__)
__attribute__((target("avx2")))
#endif  // defined(__GNUC__) || defined(__clang__)
bool IsViewAsciiAVX2Impl(const char* data, size_t len) {
    size_t i = 0;
    for (; i + 32 <= len; i += 32) {
        const auto* chunk_ptr =
            reinterpret_cast<const __m256i*>(data + i);  // NOSONAR(cpp:S3630) - Required to load unaligned 256-bit chunk for ASCII validation intrinsic
        const __m256i chunk = _mm256_loadu_si256(chunk_ptr);
...
__attribute__((target("avx2")))
#endif  // defined(__GNUC__) || defined(__clang__)
bool IsViewAsciiAVX2(const char* data, size_t len) {
    return IsViewAsciiAVX2Impl(data, len);
}
