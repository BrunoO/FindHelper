...
public:
   ...
   *
   * Uses try_lock so it never deadlocks if the crashing thread already holds
   * the logger mutex. Always flushes when the write succeeds. Safe to call from
   * Windows UnhandledExceptionFilter and std::terminate handlers.
   * Does nothing if the lock cannot be acquired or the log file is closed.
   */
...
#ifdef LOGGING_ENABLED
    // Early exit before the syscall + formatting: with the Release default
    // min_log_level_ (LOG_IMPORTANT) this LOG_INFO message would be discarded
    // by Log() anyway, making the GetPrivateMemoryBytes() work pure waste.
...
private:
  ...
  Logger() {
#ifdef LOGGING_ENABLED
#ifdef _WIN32
    // GUI subsystem: attach to a parent console when launched from cmd/PowerShell.
    // AllocConsole is Debug-only — in Release it would show a spurious window from Explorer.
    ...
    // Per-instance session id (timestamp + pid) — must be set before the
    // first TryOpenLogFile so the filename is unique across running instances.
  ...
  [[nodiscard]] std::string GetExecutableName() const {
    ...
    if (uint32_t size = sizeof(path); _NSGetExecutablePath(path, &size) == 0) {  // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay,hicpp-no-array-decay) - macOS API requires C-style buffer
      // Extract just the filename from the full path
      return ExtractFilename(path);  // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay,hicpp-no-array-decay) - path is a C-style buffer
    }
#else  // Linux
  ...
  // Unique per-instance log filename: <exe>_<session-id>.log, where session-id
  // bundles the local session start timestamp and the process ID so two
  // concurrently running instances never share (nor race to rotate) one file.
  ...
  // Session identifier for per-instance log files: "20260916_143522_pid1234".
  // Computed once at Logger construction (session start), before the first
  // TryOpenLogFile, and reused for every rotation of this session.
  ...
  // Process id for the log filename (Windows API on _WIN32, getpid() elsewhere).
  ...
  std::string session_id_; // NOLINT(readability-identifier-naming)  // Per-instance session id (timestamp + pid), set once in the constructor
#ifdef NDEBUG
...
// Emits LOG_IMPORTANT so that elapsed times are recorded during production benchmark runs.
class ScopedBenchmarkTimer {  // NOSONAR(cpp:S3624) - Default copy/move is correct (simple RAII timer)
public:
  ...
  ~ScopedBenchmarkTimer() {
    try {
      ...
      if (ms >= static_cast<double>(logger_constants::kMillisecondsPerSecond)) {
        const double seconds = ms / static_cast<double>(logger_constants::kMillisecondsPerSecond);
    ...
    } catch (...) {  // NOLINT(bugprone-empty-catch) NOSONAR(cpp:S2738, cpp:S2486) - Destructors must not throw exceptions
    }
  }
