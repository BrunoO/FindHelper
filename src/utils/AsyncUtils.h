...
inline void SafeWaitFuture(std::future<T>& f) {
  ...
  try {
    ...
    LOG_ERROR_BUILD("SafeWaitFuture: future_error while draining future: " << e.what());
  } catch (const std::exception& e) {  // NOSONAR(cpp:S1181) - future may rethrow task exceptions derived from std::exception
    LOG_ERROR_BUILD("SafeWaitFuture: exception while draining future: " << e.what());
  } catch (...) {  // NOSONAR(cpp:S2738) - Final catch-all required; must not propagate into caller
    LOG_ERROR_BUILD("SafeWaitFuture: unknown exception while draining future");
  }
}
