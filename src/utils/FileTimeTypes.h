...
inline constexpr FILETIME kFileTimeNotLoaded = {UINT32_MAX, UINT32_MAX};
inline constexpr FILETIME kFileTimeFailed = {1, 0};

// Helper function to check if FILETIME is sentinel (not loaded yet)
[[nodiscard]] constexpr bool IsSentinelTime(const FILETIME &ft) noexcept {
  return ft.dwHighDateTime == UINT32_MAX && ft.dwLowDateTime == UINT32_MAX;
}

// Helper function to check if FILETIME load failed
[[nodiscard]] constexpr bool IsFailedTime(const FILETIME &ft) noexcept {
  return ft.dwHighDateTime == 0 && ft.dwLowDateTime == 1;
}

// Helper function to check if FILETIME is valid (not sentinel, not failed)
[[nodiscard]] constexpr bool IsValidTime(const FILETIME &ft) noexcept {
...
[[nodiscard]] constexpr bool IsEpochTime(const FILETIME &ft) noexcept {
  ...
  if (ft.dwHighDateTime == 0) {
    ...
    constexpr uint64_t kYear1601Max = 0x0000000000595200ULL;
