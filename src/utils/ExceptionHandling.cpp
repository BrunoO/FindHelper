...
[[noreturn]] void TerminateHandler() {
  // Prefer Logger when available (flush). Fall back to cerr — Logger may be
  // mid-destruction or hold its mutex (LogBestEffortFatal uses try_lock).
  Logger::Instance().LogBestEffortFatal(
      "FATAL: std::terminate called (unhandled exception, noexcept violation, "
      "or destructor throw). No Logging session ended — aborting.");
