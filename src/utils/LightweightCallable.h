...
// Lightweight callable wrapper for small callables (lambdas, function pointers).
// Provides a std::function-like interface with a compile-time-enforced Small Buffer
// Optimization (SBO):
// - Guaranteed zero heap allocation: callables larger than kStorageSize fail to
//   compile (static_assert), unlike std::function which silently heap-allocates
//   (libc++'s SBO is only ~24 bytes, so a string-capturing lambda allocates there).
// - Predictable copies: copying a stored callable never allocates.
//
// Dispatch is a single indirect call through a function pointer — the same
// mechanism std::function uses, so per-call overhead is equivalent (measured
// within ~1.5% on the matcher hot path, 2026-09; see
// internal-docs/archive/LIGHTWEIGHTCALLABLE_VS_STD_FUNCTION_ANALYSIS.md).
// The value of this wrapper is the no-allocation guarantee, not call speed.
...
public:
  ...
  LightweightCallable(F&& callable)  // NOLINT(google-explicit-constructor,hicpp-explicit-conversions,cppcoreguidelines-explicit-constructor,misc-explicit-constructor,cppcoreguidelines-pro-type-member-init,hicpp-member-init) NOSONAR(cpp:S1709) - Implicit conversion is intentional for the type-erasure API (mirrors std::function; matcher factories rely on it); storage_ is placement-new'd
  ...
  LightweightCallable& operator=(const LightweightCallable& other) {
    if (this != &other) {
      // Reset first: if copy_ throws, *this is left in a valid empty state
      // instead of keeping function pointers into already-destroyed storage.
      Reset();
  ...
  LightweightCallable& operator=(LightweightCallable&& other) noexcept {
    if (this != &other) {
      // Reset first: keep *this valid-and-empty if move_ throws (noexcept
      // here means std::terminate, but the object state stays consistent).
      Reset();
...
private:
  ...
  // Destroy the stored callable (if any) and return to the empty state.
  // Pointer members are nulled immediately so the object is always in a valid
  // state, even if a subsequent copy/move placement-new throws.
  void Reset() noexcept {
    if (destroy_ != nullptr) {
      destroy_(static_cast<void*>(storage_));
    }
