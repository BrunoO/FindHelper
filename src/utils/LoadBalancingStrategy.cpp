...
// Match-rate assumption shared by the hits-vector and arena reserves: chunk
// items (or bytes) divided by this estimates the match volume per task.
constexpr size_t kMatchRateHeuristic = 20;
...
// Reserve hits before any index lock (matches pre-refactor worker order; avoids heap
// work under lock). The arena grows geometrically during the scan: a handful of
// uncontended allocations whose total bytes are unknowable pre-lock, so it is
// intentionally not pre-sized.
[[nodiscard]] inline SearchResultBatch MakeReservedWorkerResultBuffer(
  size_t chunk_start, size_t chunk_end) {
  SearchResultBatch batch;
  constexpr size_t k_min_initial_capacity = 16;
  const size_t chunk_size = chunk_end - chunk_start;
  batch.hits.reserve((std::max)(chunk_size / kMatchRateHeuristic, k_min_initial_capacity));
  return batch;
}
 ...
 *
 * Deliberate exception: the batch arena gets one exact-byte reserve after the
 * lock (ExecuteHybridStrategyTask; chunk bytes are only knowable from the live
 * SoA view). A single uncontended malloc under shared_lock blocks writers only,
 * which are already stalled for the whole scan — categorically different from
 * the per-hit malloc storm the arena design exists to eliminate.
 */
struct WorkerSearchChunkContext {
  ...
  SearchResultBatch local_results; // NOLINT(readability-identifier-naming)
...
struct ChunkProcessingParams {
  const ThreadSetupAfterLock& setup;
  size_t chunk_start_ = 0; // NOLINT(readability-identifier-naming)
  size_t chunk_end_ = 0; // NOLINT(readability-identifier-naming)
...
inline void ProcessChunkWithExceptionHandling(
  const ChunkProcessingParams& params, SearchResultBatch& local_results,
...
struct DynamicChunksLoopParams {
  ...
  const SearchContext& context;
  SearchResultBatch& local_results;
  int thread_idx_ = 0; // NOLINT(readability-identifier-naming)
  const char* strategy_name_ = nullptr; // NOLINT(readability-identifier-naming)
...
struct HybridStrategyTaskParams {
  ...
  size_t total_items_ = 0; // NOLINT(readability-identifier-naming)
  size_t initial_chunks_end_ = 0; // NOLINT(readability-identifier-naming)
  const std::shared_ptr<std::atomic<size_t>>& next_dynamic_chunk;
  const SearchContext& context;
 ...
 */
// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables) - Static function
inline SearchResultBatch ExecuteHybridStrategyTask(
  const HybridStrategyTaskParams& params) {
  WorkerSearchChunkContext worker(params.index, *params.shared_matchers, params.start_index_,
                                  params.end_index_);
  ...
  if (effective_start < effective_end) {
    // Reserve the arena once from exact chunk bytes: a single uncontended
    // malloc under shared_lock (see the exception recorded on
    // WorkerSearchChunkContext). Same match-rate assumption as the hits
    // reserve in MakeReservedWorkerResultBuffer; dynamic chunks may still
    // grow it geometrically afterwards.
    worker.local_results.arena.reserve(
        ParallelSearchEngine::CalculateChunkBytes(worker.setup.soa_view_, effective_start,
                                                 effective_end,
                                                 worker.setup.storage_size_) /
        kMatchRateHeuristic);
 ...
 */
[[nodiscard]] inline SearchResultBatch ExecuteHybridStrategyTaskWithEnqueueExceptions(
    const HybridStrategyTaskParams& params) {
  try {
 ...
 */
[[nodiscard]] std::vector<std::future<SearchResultBatch>>
HybridStrategy::LaunchSearchTasks(  // NOLINT(readability-function-cognitive-complexity) - try-catch in per-thread lambda adds nesting; further splitting would obscure lambda capture intent
  ...
  // Divide initial work equally among threads. Floor at 1 when there is work:
  // tiny indexes (a single entry with percent < 100) otherwise round every
  // chunk to empty and launch zero tasks — silent empty results. The dynamic
  // phase only runs inside launched tasks, so it cannot recover this.
  size_t initial_chunk_size = (initial_work_items + static_cast<size_t>(thread_count) - 1) / static_cast<size_t>(thread_count);
  if (total_items > 0 && initial_chunk_size == 0) {
  ...
  for (int t = 0; t < thread_count; ++t) {
    ...
    futures.push_back(thread_pool.Enqueue([&index, start_index, end_index, thread_idx = t,  // NOLINT(bugprone-exception-escape) - task exceptions caught below; residual from LOG stream ops in catch blocks (OOM scenario)
                                           ...
                                           shared_context, shared_matchers, thread_timings, thread_count] {  // NOLINT(readability-redundant-lambda-parameter-list) - empty parameter list is redundant
