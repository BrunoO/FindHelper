...
bool IsViewAsciiNEONImpl(const char* data, size_t len) {
    size_t i = 0;
    for (; i + 16 <= len; i += 16) {
        const uint8x16_t chunk = vld1q_u8(  // NOSONAR(cpp:S3630) - Required to load 16-byte buffer into NEON intrinsic for ASCII validation
            reinterpret_cast<const uint8_t*>(data + i));  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast) - NEON vld1q_u8 requires uint8_t*
        if (vmaxvq_u8(chunk) > kMaxAsciiByte) {
            return false;
        }
    }

    for (; i < len; ++i) {
        if (static_cast<unsigned char>(data[i]) > kMaxAsciiByte) {
            return false;
        }
    }
    return true;
...
bool IsViewAsciiNEON(const char* data, size_t len) {
    return IsViewAsciiNEONImpl(data, len);
}

bool ContainsSubstringINEON(const std::string_view& text, const std::string_view& pattern) {
...
bool ContainsSubstringNEON(const std::string_view& text, const std::string_view& pattern) {
    if (pattern.empty()) { return true; }
    if (text.length() < pattern.length()) { return false; }
    if (pattern.length() < 4) {
