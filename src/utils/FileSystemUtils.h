...
inline FileAttributes GetFileAttributes(std::string_view path) {
  ...
  if (path_str.length() >= 3 && path_str[1] == ':' && (path_str[2] == '\\' || path_str[2] == '/')) {
    ...
    auto drive_letter = static_cast<char>(std::toupper(static_cast<unsigned char>(path_str[0])));
  ...
  if (const int stat_result = stat(path_str.c_str(), &file_stat); stat_result == 0) {  // NOLINT(cppcoreguidelines-init-variables) - Variable initialized in C++17 init-statement
...
inline std::string FormatFileSize(uint64_t bytes) {
  ...
  if (bytes < 1024ULL * 1024) {
    ...
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg,hicpp-vararg) - snprintf is C vararg; C++17 has no std::format
    if (const int written = std::snprintf(buffer.data(), buffer.size(), "%.1f KB", kb); true) {  // NOLINT(cppcoreguidelines-init-variables)
  ...
  if (bytes < 1024ULL * 1024 * 1024) {
    ...
    std::string buffer(32, '\0');
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg,hicpp-vararg) - snprintf is C vararg; C++17 has no std::format
    if (const int written = std::snprintf(buffer.data(), buffer.size(), "%.1f MB", mb); true) {  // NOLINT(cppcoreguidelines-init-variables)
      file_system_utils_detail::ResizeBufferFromSnprintf(buffer, written);
    }
    return buffer;
  }
  ...
  std::string buffer(32, '\0');
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg,hicpp-vararg) - snprintf is C vararg; C++17 has no std::format
  if (const int written = std::snprintf(buffer.data(), buffer.size(), "%.2f GB", gb); true) {  // NOLINT(cppcoreguidelines-init-variables)
    file_system_utils_detail::ResizeBufferFromSnprintf(buffer, written);
  }
  return buffer;
}
