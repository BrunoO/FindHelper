...
public:
   ...
   * - BulkMft      (register, no resolve, deferred paths): baseline USN journal
   *                enumeration (ingest source: MFT records via the journal walk);
   *                FRN identity is claimed but per-insert repair is O(N^2) —
   *                RecomputeAllPaths resolves out-of-order parents in one
   *                topological pass. Path indexing is likewise deferred: the
   *                per-insert full-path join + InsertPath is thrown away by
   *                the recompute's Clear(), so bulk staging stores storage
   *                entries only. FileIndex::InsertBatch (via IndexBuildRun).
   *                Requires RecomputeAllPaths before the index is searched
   *                (search is gated on IsIndexBuilding() until then).
  ...
  struct InsertOptions {
    ...
    // Skip the full-path join + InsertPath; the entry is stored but invisible
    // to path search until RecomputeAllPaths indexes it once. Bulk staging
    // only (the recompute Clear()s whatever per-insert paths were built);
    // must not combine with resolve_awaiting=true (asserted in Insert).
