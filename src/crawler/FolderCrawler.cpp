...
bool FolderCrawler::Crawl(std::string_view root_path, std::atomic<size_t>* indexed_file_count,  // NOLINT(readability-function-cognitive-complexity) NOSONAR(cpp:S3776) - Complex error handling and state management required for robust crawling
  ...
  CTRACK_NAME("FolderCrawler::Crawl");
  // Input validation
  if (root_path.empty()) {
  ...
  try {
    ...
    wq_mutexes_.clear();
    wq_mutexes_.resize(num_workers_);
...
void ProcessEntry(  // NOSONAR(cpp:S107) - Function has 10 parameters
  ...
  std::string full_path = path_utils::JoinPath(dir_path, entry.name);
  ...
  // Copy once for pending_subdirs, then move into batch to avoid a second heap copy.
  ...
  batch.emplace_back(std::move(full_path), entry.is_directory);
...
void FolderCrawler::WorkerThread(size_t worker_idx,
                                 ...
                                 const std::atomic<bool>* cancel_flag) {
  CTRACK_DEV_NAME("FolderCrawler::WorkerThread");
...
void FolderCrawler::FlushBatch(const std::vector<std::pair<std::string, bool>>& batch,
  ...
  CTRACK_DEV_NAME("FolderCrawler::FlushBatch");
...
bool EnumerateDirectoryOpenDir(std::string_view dir_path,
  ...
  while ((entry = readdir(dir)) != nullptr) {  // NOLINT(concurrency-mt-unsafe) - each call uses its own DIR*; streams not shared across threads
    ...
    if (entry->d_type != DT_UNKNOWN) {
      ...
      full_path.reserve(dir_path.size() + 1 + std::strlen(entry->d_name));  // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay,hicpp-no-array-decay) - d_name is an array
      full_path.assign(dir_path);
      if (full_path.back() != '/') {
        full_path += "/";
      }
...
bool FolderCrawler::EnumerateDirectory(std::string_view dir_path,
  ...
  AutoFd afd;
  afd.fd_ = open(std::string(dir_path).c_str(), O_RDONLY);
  if (afd.fd_ == -1) {
  ...
  // Reuse per-worker buffer across directories (WorkerThread calls EnumerateDirectory in a loop).
  thread_local std::vector<char> bulk_attr_buf(kBulkAttrBufSize);

  for (;;) {
    const int count = getattrlistbulk(afd.fd_, &alist, bulk_attr_buf.data(), bulk_attr_buf.size(), 0);
    if (count == 0) {
    ...
    const char* ptr = bulk_attr_buf.data();
...
bool FolderCrawler::EnumerateDirectory(std::string_view dir_path,
  ...
  CTRACK_DEV_NAME("FolderCrawler::EnumerateDirectory");
  entries.clear();
  entries.reserve(kDefaultDirEntryReserve);
  ...
  // Reuse per-worker buffer across directories (WorkerThread calls EnumerateDirectory in a loop).
  thread_local std::vector<char> getdents_buf(kGetdents64BufSize);
  bool success = true;
  ...
  while (!read_done) {
    const long nread = syscall(SYS_getdents64, fd,  // NOLINT(google-runtime-int) - syscall() signature uses long
                               getdents_buf.data(), getdents_buf.size());
    if (nread < 0) {
      ...
      ProcessGetdents64Buffer(getdents_buf.data(), nread, dir_path, entries);
    }
  }
