...
void IndexOperations::SyncHealedChildSubtree(std::string_view old_full_path,
  ...
  if (old_full_path.empty() || path_utils::IsDriveLetterRootKey(old_full_path) ||
    ...
    storage_.CacheDirectory(new_full_path, child_id.raw);
  ...
  // Sync directory cache (same trailing semantics as Rename/Move).
  storage_.RemoveDirectoryFromCache(old_full_path);
  storage_.CacheDirectory(new_full_path, child_id.raw);
...
void IndexOperations::Insert(ntfs_file_reference::NtfsFileReference id,
  ...
  storage_.InsertLocked(id, ntfs_file_reference::NtfsFileReference(resolved_parent_id), name,
                        is_directory, modification_time, options.register_mft_record);
...
bool IndexOperations::Rename(ntfs_file_reference::NtfsFileReference id,
  ...
  } else if (IsAwaitingChild(id)) {
    // Bare placeholder (out-of-order child): stay bare so the entry keeps
    // looking unresolved until the parent arrives and heals it. Joining the
    // volume root here would impersonate a real root file (e.g. C:\<hex>)
    // and slip past the bare-name invariant's rooted-means-resolved
    // assumption. Awaiting tracking is untouched, so healing still applies.
