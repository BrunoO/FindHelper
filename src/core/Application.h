...
public:
  ...
  /**
   * @brief Return the id of the last history entry recorded in this session.
   *
   * Empty when no search has been executed since the application started.
   * Used by the Cmd+P / Ctrl+P shortcut to pin/unpin the most recent search.
   */
  [[nodiscard]] std::string_view GetLastHistoryId() const noexcept { return last_history_id_; }
  ...
  /**
   * @brief Persist settings after syncing runtime window size into AppSettings
   * @return true on success, false if settings_ is null or SaveSettings fails
   */
  [[nodiscard]] bool PersistSettings() override;
...
private:
  ...
  // State tracking for search history
  SearchParams last_search_params_;
  std::string last_history_id_;  // Id of the last history entry recorded; empty before first search
  ...
  std::atomic<bool> show_recent_changes_{false};    // Recent changes window visibility (thread-safe)
  ...
  // True when this session's primary index source is the USN monitor (not folder crawl).
  // Persisted settings.crawlFolder must not enable periodic TriggerRecrawl() in that case —
  // StartIndexBuilding clears file_index_ and would replace a full-volume USN index.
  bool use_usn_monitor_ = false;
