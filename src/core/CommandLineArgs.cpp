...
inline bool ParseIntArg(const char* arg, int& i, int argc,
  ...
  if (const int value{[&arg_sv, name_len_with_equals]() {
    ...
    if (value >= config.min_val_ && value <= config.max_val_) {
      *config.target_ = value;
    } else {
      LOG_WARNING_BUILD("Invalid " << config.arg_display_name_ << ": " << value
                        << " (must be " << config.min_val_ << "-" << config.max_val_ << ", ignoring)");
    }
    return true;
  }
  ...
  if (const std::string arg_name_only(arg_name, name_len_with_equals - 1);
    ...
    if (const int value{std::atoi(argv[i + 1])}; value >= config.min_val_ && value <= config.max_val_) {  // NOLINT(cert-err34-c,bugprone-unchecked-string-to-number-conversion) - atoi/argv standard C CLI
      *config.target_ = value;
      ++i;
    } else {
      LOG_WARNING_BUILD("Invalid " << config.arg_display_name_ << ": " << value
...
[[nodiscard]] std::string BuildStartupArgsDisplay(int argc, char** argv) {
