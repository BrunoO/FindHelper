 ...
 */
// NOLINTNEXTLINE(misc-const-correctness) - GLFW C API takes non-const GLFWwindow* (e.g., glfwGetWin32Window)
...
Application::Application(AppBootstrapResultBase& bootstrap, CommandLineArgs cmd_args,
  ...
  const auto search_thread_pool = std::make_shared<SearchThreadPool>(thread_count);
...
Application::~Application() {
  ...
  state_.cloud_files.WaitAndDrain();
}
...
bool Application::StartIndexBuilding(
  ...
  // Stop USN population/monitoring so IsIndexBuilding() and GetIndexedFileCount() reflect
  // the crawl session, not a volume scan still running from bootstrap.
  if (monitor_ != nullptr) {
    monitor_->Stop();
  }
...
void Application::TriggerRecrawl() {
  ...
  const std::string& folder_to_crawl =
      (settings_ != nullptr && !settings_->crawlFolder.empty()) ? settings_->crawlFolder
                                                                : auto_crawl_folder_;
  if (folder_to_crawl.empty()) {
    return;
  }

  // Use existing StartIndexBuilding method which handles stopping old builder
  if (StartIndexBuilding(folder_to_crawl)) {
    LOG_INFO_BUILD("Periodic recrawl started for folder: " << folder_to_crawl);
  } else {
    LOG_ERROR_BUILD("Failed to start periodic recrawl for folder: " << folder_to_crawl);
  }
}
...
bool Application::IsRecrawlEnabled() const {
  if (use_usn_monitor_ || !cmd_args_.index_from_file.empty()) {
    return false;
  }
  return recrawl_enabled_ || (settings_ != nullptr && !settings_->crawlFolder.empty());
}
...
bool Application::HandleFocusedWindow(double idle_timeout_seconds, double idle_wait_timeout) {
  ...
  if (const bool has_background_work{search_worker_.IsBusy() || IsIndexBuilding() ||
                                     (folder_size_aggregator_ &&
                                      folder_size_aggregator_->HasPendingWork()) ||
...
int Application::Run() {  // NOLINT(readability-function-cognitive-complexity) - Main loop: event
  ...
  try {
    ...
    // Log ctrack profiling report on exit (skipped in shipping builds: instrumentation compiled out)
    const std::string ctrack_report = ctrack::result_as_string();
    if (!ctrack_report.empty()) {
      LOG_IMPORTANT_BUILD("=== CTRACK Profiling Results on Exit ===");
      LOG_IMPORTANT_BUILD("\n" << ctrack_report);
    }
...
void Application::ProcessFrame() {
  ...
  ui::UIRenderer::RenderMainWindow(
    {state_, this, *settings_, *file_index_, *thread_pool_, monitor_.get(), search_worker_,
     ...
     native_window, window_, show_settings_, show_metrics_, show_recent_changes_, is_index_building, metrics_available,
  ...
  ui::UIRenderer::RenderFloatingWindows({state_, this, *settings_, *file_index_, monitor_.get(),
                                         search_worker_, window_, show_settings_, show_metrics_, show_recent_changes_,
                                         metrics_available, cmd_args_.startup_args_display,
                                         cmd_args_.unknown_args_display,});
...
void Application::UpdateIndexBuildState(bool index_building_now) {
  ...
  const bool was_index_building_ui = state_.index_build.in_progress;
  const auto now = std::chrono::steady_clock::now();
  ...
  state_.index_build.in_progress = index_building_now;
  state_.index_build.failed = index_build_state_.failed.load();
  state_.index_build.entries_processed =
    index_build_state_.entries_processed.load();
  state_.index_build.files_processed = index_build_state_.files_processed.load();
  state_.index_build.dirs_processed = index_build_state_.dirs_processed.load();
  state_.index_build.error_count = index_build_state_.errors.load();

  // Detect start of a new index build (initial crawl, auto-crawl, USN population, etc.).
  if (index_building_now && !was_index_building_ui) {
    state_.index_build.start_time = now;
    state_.index_build.last_duration_ms = 0U;
    state_.index_build.has_timing = true;
  }

  // Wall-clock duration for any index-building phase (matches WelcomePanel elapsed).
  if (was_index_building_ui && !index_building_now && !state_.index_build.failed) {
    const auto duration =
      std::chrono::duration_cast<std::chrono::milliseconds>(now - state_.index_build.start_time);
    state_.index_build.last_duration_ms = static_cast<uint64_t>(duration.count());
  }
  ...
  if (last_index_build_state_active_ && !now_index_thread_active &&
    ...
    const bool has_crawl_folder =
        (settings_ != nullptr && !settings_->crawlFolder.empty()) || !auto_crawl_folder_.empty();
  ...
  state_.index_build.status_text.clear();
  if (state_.index_build.in_progress) {
    const bool usn_populating =
        monitor_ != nullptr && monitor_->IsPopulatingIndex();
    state_.index_build.status_text = index_build::FormatInProgressStatus(
        index_build_state_.source_description, usn_populating);
  } else if (state_.index_build.failed) {
    std::string error = index_build_state_.GetLastErrorMessage();
    if (error.empty()) {
      error = "Index build failed";
    }
    state_.index_build.status_text = std::move(error);
  } else if (index_build_state_.completed.load() &&
             state_.index_build.entries_processed > 0U) {
    state_.index_build.status_text =
      "Index ready - " + std::to_string(state_.index_build.entries_processed) + " entries";
  }
}

void Application::UpdateSearchState(bool is_index_building) {
  // Capture searchActive at the top, before application_logic::Update().
  // RenderMainWindow (Search / Enter) runs after this function and sets searchActive
  // for the next frame; sampling here sees that previous-frame UI trigger.
  // An end-of-frame last_search_active_ sentinel misses one-frame searches because
  // the UI transition never lands in the same update that records history.
  const bool is_search_active_before{state_.search_pipeline.search_active};

  if (is_search_active_before) {
  ...
  const bool is_search_active_after{state_.search_pipeline.search_active};

  // Search started this frame (F5 / crawl-complete) so params were not snapshotted above.
  // pendingManualHistoryRecord covers F5 that completed and was then replaced by auto-refresh.
  if (!is_search_active_before &&
      (state_.search_pipeline.search_was_manual || state_.search_pipeline.pending_manual_history_record)) {
    last_search_params_ = state_.BuildCurrentSearchParams();
  }

  // Record regardless of result count so zero-result queries can be pinned/recalled.
  if (ShouldRecordManualSearchCompletion(state_.search_pipeline.search_was_manual, is_search_active_after,
                                         state_.search_pipeline.pending_manual_history_record) &&
      settings_ != nullptr) {
    ...
    state_.search_pipeline.ConsumeManualHistoryRecord();
  }

  // Reset manual flag after checking (for next search)
  if (!is_search_active_after) {
    state_.search_pipeline.search_was_manual = false;
  }
  assert(is_search_active_after || !state_.search_pipeline.search_was_manual);
}
...
void Application::HandleIndexDump() {
  ...
  const size_t indexed_count = monitor_->GetIndexedFileCount();
...
bool Application::IsIndexBuilding() const {
  ...
  // USN initial population blocks search only when this session is USN-primary.
  if (monitor_ != nullptr &&
      application_logic::ShouldGateSearchOnUsnPopulation(use_usn_monitor_,
                                                           monitor_->IsPopulatingIndex())) {
    return true;
  }
  return false;
}
...
void Application::TriggerManualSearch(const GuiState& state) {
  ...
  state_.searchCriteria.filename_input.SetValue(state.searchCriteria.filename_input.AsView());
  state_.searchCriteria.extension_input.SetValue(state.searchCriteria.extension_input.AsView());
  state_.searchCriteria.path_input.SetValue(state.searchCriteria.path_input.AsView());
  // Copy effective filter as one unit (SetItemTypeFilter syncs folders_only);
  // copying folders_only separately afterwards would re-introduce legacy desync.
  state_.searchCriteria.SetItemTypeFilter(state.searchCriteria.GetEffectiveItemTypeFilter());
  state_.searchCriteria.case_sensitive = state.searchCriteria.case_sensitive;
  state_.searchCriteria.time_filter = state.searchCriteria.time_filter;
  state_.searchCriteria.size_filter = state.searchCriteria.size_filter;
...
bool Application::IsIndexReadyForRegressionTest() const {
  ...
  return state_.index_build.entries_processed > 0U ||
...
void Application::SetSearchParamsForRegressionTest(std::string_view filename, std::string_view path,
                                                   ...
                                                   bool folders_only) {
  state_.searchCriteria.filename_input.SetValue(filename);
  state_.searchCriteria.path_input.SetValue(path);
  state_.searchCriteria.extension_input.SetValue(extensions_semicolon);
  state_.searchCriteria.SetItemTypeFilter(folders_only ? ItemTypeFilter::FoldersOnly : ItemTypeFilter::All);
}
...
bool Application::IsSearchCompleteForRegressionTest() const {
  return !state_.search_pipeline.search_active;
}
...
size_t Application::GetIndexedFileCount() const {
  if (use_usn_monitor_ && monitor_ != nullptr) {
    return monitor_->GetIndexedFileCount();
  }
  return file_index_->Size();
}

// Export CSV feedback is popup-only. Clear status-bar strings and expire
// export_workflow.notification_time so StatusBar (shared with pin/delete) does not mirror the popup.
static void SetExportPopupFailure(GuiState& state, std::string_view error_message,
                                  std::string_view file_path = {}) {
  state.export_workflow.error_message.assign(error_message);
  state.export_workflow.notification.clear();
  state.export_workflow.notification_time = {};
  state.export_workflow.show_popup = true;
  state.export_workflow.success = false;
  state.export_workflow.file_path.assign(file_path);
  state.export_workflow.result_count = 0;
}

static void SetExportPopupSuccess(GuiState& state, std::string_view file_path,
                                  std::size_t result_count) {
  state.export_workflow.notification.clear();
  state.export_workflow.error_message.clear();
  state.export_workflow.notification_time = {};
  state.export_workflow.show_popup = true;
  state.export_workflow.success = true;
  state.export_workflow.file_path.assign(file_path);
  state.export_workflow.result_count = result_count;
}
...
void Application::ExportToCsv(GuiState& state) {
  ...
  if (results == nullptr || results->empty()) {
    SetExportPopupFailure(state, "No results to export");
    return;
  }

  // Guard: ExportToCsv depends on file_index_. If not yet initialized, skip and set error.
  if (file_index_ == nullptr) {
    SetExportPopupFailure(state, "Export not available (index not ready)");
    return;
  }
  ...
  if (localtime_s(&time_struct, &now_time_t) != 0) {
    SetExportPopupFailure(state, "Failed to get local time for filename");
    return;
  }
#else
  ...
  const std::string output_path = path_utils::JoinPath(export_folder, ss.str());

  if (search::ExportSearchResultsService::ExportToCsv(*results, *file_index_, output_path)) {
    SetExportPopupSuccess(state, output_path, results->size());
  } else {
    SetExportPopupFailure(state, "Failed to export CSV", output_path);
  }
}
