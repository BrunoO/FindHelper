...
Application::Application(AppBootstrapResultBase& bootstrap, CommandLineArgs cmd_args,
  ...
  // Prepopulate settings crawlFolder from the effective crawl path used at startup so the
  // Settings window shows it even when it came from a --crawl-folder argument or the default
  // home-folder fallback. This is in-memory only; nothing is written to disk until the user
  // explicitly saves in the Settings window.
  if (settings_->crawlFolder.empty()) {
    if (!cmd_args_.crawl_folder.empty()) {
      settings_->crawlFolder = cmd_args_.crawl_folder;
    } else if (!auto_crawl_folder_.empty()) {
      settings_->crawlFolder = auto_crawl_folder_;
    }
  }
...
void Application::UpdateIndexBuildState(bool index_building_now) {
  ...
  if (state_.indexBuildInProgress) {
    ...
    state_.indexBuildStatusText = "Indexing " + source;
...
void Application::SaveSettingsOnShutdown() {
  ...
  if (!SaveSettings(*settings_)) {
    LOG_ERROR("Failed to save settings on shutdown — preferences may not persist");
  }
}
