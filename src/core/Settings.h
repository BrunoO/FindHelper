...
inline constexpr int kMinDynamicChunkSize = 100;
inline constexpr int kMaxDynamicChunkSize = 100000;
inline constexpr int kDefaultHybridInitialWorkPercent = 80;
inline constexpr int kMinHybridInitialWorkPercent = 50;
inline constexpr int kMaxHybridInitialWorkPercent = 95;
...
struct SearchHistoryParams {
  ...
  int item_type = 0;  // NOLINT(readability-identifier-naming) - POD history payload; 0=All, 1=FilesOnly, 2=FoldersOnly
  bool folders_only = false;  // NOLINT(readability-identifier-naming) - POD history payload; snake_case without trailing _ (kept for legacy JSON compat)
...
struct AppSettings {
  ...
  // User UI scale (ImGui style.FontScaleMain). Monitor DPI is separate (style.FontScaleDpi).
  ...
  // Persisted to search_history.json alongside findhelper_settings.json.
  // searchHistory is absent from findhelper_settings.json itself; it lives in its own file.
