...
namespace main_common_detail {
  ...
  int ExecuteApplicationMain(const CommandLineArgs& cmd_args,
    ...
    // When --index-from-file is provided, do NOT fall back to settings.crawlFolder:
    // the index is already loaded from file during bootstrap, and crawling settings.crawlFolder
    // would run in the background and pollute/replace the file-based index.
    if (!cmd_args.crawl_folder.empty()) {
      index_builder_config.crawl_folder = cmd_args.crawl_folder;
    } else if (cmd_args.index_from_file.empty() && bootstrap.settings &&
               !bootstrap.settings->crawlFolder.empty()) {
...
#ifdef _WIN32
    // Only an explicit --crawl-folder opts out of USN. A persisted settings.crawlFolder must
    // not disable use_usn_monitor: that set source_description to "Folder: ..." while
    // UsnMonitor still populates, causing a brief "Indexing Folder" flash then USN.
    const bool explicit_crawl_folder = !cmd_args.crawl_folder.empty();
    index_builder_config.use_usn_monitor =
        (bootstrap.monitor != nullptr && !explicit_crawl_folder);
#else
    ...
    if (index_builder != nullptr) {
      ...
      if (index_builder_config.use_usn_monitor) {
        index_build_state.source_description = std::string(index_build::kUsnJournalSourceDescription);
...
int RunApplication(int argc, char** argv) {  // NOLINT(google-objc-function-naming)
  ...
  // Original argv is forwarded so the elevated process keeps the same options.
  if (cmd_args.crawl_folder.empty() && !IsProcessElevated() &&
      PromptForElevationAndRestart(nullptr, argc, argv)) {
  ...
  system_stay_awake::BeginSession();
...
inline int RunApplicationWithCatch(int argc, char** argv) {  // NOLINT(google-objc-function-naming) - C++ template in .h shared by ObjC++ files; follows project PascalCase convention
  try {
    ...
    LOG_IMPORTANT_BUILD("FATAL: uncaught exception in main: " << e.what());
    ...
    LOG_IMPORTANT_BUILD("FATAL: unknown uncaught exception in main");
