...
void SetupWindowResizeCallback(
  ...
  resize_data.renderer_ = renderer;  // RendererType* converts to RendererInterface*
  resize_data.last_window_width_ = last_width;
  resize_data.last_window_height_ = last_height;

  glfwSetWindowSizeCallback(window, [](GLFWwindow* w, int width, int height) {
    auto data = static_cast<WindowResizeData*>(glfwGetWindowUserPointer(w));
    if (data != nullptr && data->renderer_ != nullptr) {
      data->renderer_->HandleResize(width, height);
    }
    if (data != nullptr) {
      if (data->last_window_width_ != nullptr) {
        *data->last_window_width_ = width;
      }
      if (data->last_window_height_ != nullptr) {
        *data->last_window_height_ = height;
      }
    }
  });
