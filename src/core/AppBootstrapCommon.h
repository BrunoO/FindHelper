 ...
 * - Index file loading
 * - Initialization failure logging with platform-supplied partial cleanup
 *
...
inline void LogCpuInformation() {
  ...
  if (physical_cores > 0 && logical_cores > 0) {
    ...
    if (logical_cores > physical_cores) {
      // Floating-point division: integer division would truncate (e.g. 14/12 -> 1x)
      LOG_IMPORTANT_BUILD("HT ratio: " << std::fixed << std::setprecision(2)
 ...
 *
 * Applies thread pool size and window dimension overrides from command-line
 * arguments to the application settings.
 *
 ...
 * - ConfigDpiScaleFonts / ConfigDpiScaleViewports: per-monitor DPI via FontScaleDpi
 *
...
inline void InitializeImGuiContext() {
  ...
  // User UI Scale -> style.FontScaleMain; per-monitor DPI -> style.FontScaleDpi (auto).
 ...
 */
inline std::string BuildErrorMessage(std::string_view prefix, std::string_view what,
 ...
 */
inline std::string_view GetUnknownExceptionMessage() {
  return "Unknown exception during initialization - possible memory corruption or non-standard "
         "exception";
}

/**
 * @brief Log initialization failure and run platform-specific partial cleanup
 *
 * Used from bootstrap try/catch paths: logs the error, then runs cleanup_fn (typically
 * ImGui/GLFW teardown plus any platform extras such as COM on Windows).
 *
 * @tparam BootstrapResult Platform bootstrap result type
 * @tparam CleanupFn Callable type void(BootstrapResult&)
 */
template <typename BootstrapResult, typename CleanupFn>
void LogInitializationFailureAndCleanup(std::string_view error_message, BootstrapResult& result,
