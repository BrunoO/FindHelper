...
public:
  ...
  // inserted out of order. Prunes orphan subtrees, then populates PathStorage
  // and the path_to_id map in a single topological pass.
  void RecomputeAllPaths() {
    size_t indexed_entries = 0;
    {
      const ScopedBenchmarkTimer timer("FileIndex::RecomputeAllPaths");
      const std::unique_lock lock(index_mutex_);
      ...
      path_recomputer_.RecomputeAllPaths([this](uint64_t id, std::string_view path) {
