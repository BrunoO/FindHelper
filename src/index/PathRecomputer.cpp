...
void PathRecomputer::RecomputeAllPaths(const PathInsertedCallback& on_path_inserted) {  // NOLINT(readability-function-cognitive-complexity) NOSONAR(cpp:S3776) - BFS + topological sort require nested loops; splitting would require passing large state structs
  const ScopedBenchmarkTimer timer("PathRecomputer::RecomputeAllPaths");
  const size_t n = storage_.Size();
  path_storage_.Clear();
  path_storage_.Reserve(n, n * 64U);
  ...
  {
    const ScopedBenchmarkTimer collect_timer("PathRecomputer phases 1-4 (collect/adjacency/BFS)");
    records.reserve(n);
    for (const auto& [id, entry] : storage_) {
      records.push_back({id.raw, entry.parentID.raw, name_cache.Find(id.raw), entry.isDirectory, kOrphanDepth});
    }
    ...
    for (size_t i = 0; i < records.size(); ++i) {
      ...
      // Seed direct children of the volume root as depth-0 roots for BFS so that
      // their subtrees are traversed topologically and paths are memoized in O(1).
  ...
  }  // collect_timer: phases 1-4 (collect/adjacency/BFS)

  // ── Step 5: Sort by (depth, id) — parents strictly before children ─────────
  {
    const ScopedBenchmarkTimer sort_timer("PathRecomputer phase 5 (depth sort)");
  ...
  {
    const ScopedBenchmarkTimer insert_timer("PathRecomputer phase 6 (path construction and insertion)");
    for (const auto& rec : records) {
      ...
      if (on_path_inserted) {
        on_path_inserted(rec.id, full_path);
      }
