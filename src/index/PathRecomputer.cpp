...
struct EntryRecord {
  ...
  std::string_view leaf_name;  // View into NameArena — valid until ReleaseNameCache()
  bool is_directory = false;
  int depth = kOrphanDepth;
};
...
void PathRecomputer::RecomputeAllPaths() {  // NOLINT(readability-function-cognitive-complexity) NOSONAR(cpp:S3776) - BFS + topological sort require nested loops; splitting would require passing large state structs
  ...
  //
  // Root detection uses storage_.GetEntry(parent_id) == nullptr — equivalent to
  // checking id membership but uses the existing flat_hash_map (faster than a
  // second unordered_map). id 0 has no entry (IDs start at 1) → parentID=0 → root.
  ...
  for (size_t i = 0; i < records.size(); ++i) {
    ...
    const bool is_root = (rec.parent_id == rec.id)
                      || (storage_.GetEntry(rec.parent_id) == nullptr);
    if (is_root) {
      records[i].depth = 0;
      bfs_queue.push_back(i);
    } else {
      children[rec.parent_id].push_back(i);
    }
  }
  ...
  for (const auto& rec : records) {
    if (const bool use_memo = (rec.depth > 0) && (rec.depth != kOrphanDepth); use_memo) {
      full_path = BuildPathFromMemoizedParent(
          ...
          path_operations_.GetPathView(rec.parent_id), storage_, name_cache);
    } else {
      // depth == 0 (roots, top-level entries with absent parent) or kOrphanDepth
      // (cycles, disconnected subtrees): use existing logic that handles drive
