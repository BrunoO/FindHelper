...
// Forward declarations (the run type lives in index/IndexBuildRun.h,
// Windows-only; this header stays parseable everywhere)
class FileIndex;
 ...
 *   calls this function. The thread is created specifically to handle both initial
  ...
  *   baseline enumeration (using FSCTL_ENUM_USN_DATA) to establish a baseline of all existing
  ...
  *   distinct operations - initial population bulk-reads the journal from the start,
  *   covering all existing files, while monitoring reads incremental changes
  *   from the USN journal.
 * - Records are staged per enumeration buffer (parsing/filtering/MFT reads
 *   with no lock held) and committed with one FileIndex::InsertBatch() call,
 *   i.e. one unique_lock acquisition per buffer instead of one per record,
 *   which blocks concurrent readers (e.g., search operations) briefly. During
 *   initial population, there are no concurrent writers yet (USN monitoring
 *   hasn't started), but this exclusive locking ensures consistency and
 *   prevents searches from seeing incomplete data.
 ...
 *
 * DESIGN TRADE-OFFS:
  * 1. Single-threaded enumeration: The journal enumeration API (FSCTL_ENUM_USN_DATA) is
...
// Returns true on success, false on failure. The caller owns the run: it
// constructs IndexBuildRun (handle, index, progress counter, filter tracker,
// latch, flags), passes it here for Reserve/Run/Result, and keeps it alive
// for RecomputePathsAndPruneOrphans after the replay drain. See
// index/IndexBuildRun.h for the ownership contract.
