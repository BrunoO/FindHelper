...
public:
    // Construct with volume handle and gateway (both caller-owned and must
    // remain valid for the lifetime of the reader). Reads go through the
    // gateway seam (DDD #8) instead of raw DeviceIoControl.
    MftMetadataReader(HANDLE volume_handle, volume_gateway::VolumeGateway& gateway);
...
private:
    ...
    // Kernel ioctl seam (DDD #8); caller-owned, must outlive the reader.
    // NOLINTNEXTLINE(readability-identifier-naming) - project convention: snake_case_ for members
    volume_gateway::VolumeGateway& volume_gateway_;
    ...
    // Returns true if successful, false otherwise
    // NOTE: output_buffer is modified in place (USA fixup); no copy is made.
    [[nodiscard]] bool ParseMftRecord(char* output_buffer, size_t output_buffer_size,
                                       FILETIME* out_modification_time,
                                       uint64_t* out_file_size);
};
