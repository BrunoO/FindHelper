...
MftMetadataReader::MftMetadataReader(HANDLE volume_handle, volume_gateway::VolumeGateway& gateway)
    : volume_handle_(volume_handle), volume_gateway_(gateway) {
...
void MftMetadataReader::LogParseStatistics() const {
    ...
    if (parse_stats_.wrapper_too_small == 0 &&
        ...
        parse_stats_.record_too_large == 0 &&
    ...
    LOG_INFO_BUILD(
        ...
        << ", record_too_large=" << parse_stats_.record_too_large
...
bool MftMetadataReader::TryGetMetadata(uint64_t file_ref_num,
    ...
    CTRACK_DEV_NAME("MftMetadataReader::TryGetMetadata");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
...
bool MftMetadataReader::ReadMftRecord(uint64_t file_ref_num,
    ...
    CTRACK_DEV_NAME("MftMetadataReader::ReadMftRecord");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
    if (buffer == nullptr || bytes_returned == nullptr) {
        return false;
    }

    // NTFS_FILE_RECORD_INPUT_BUFFER is built inside the gateway seam
    // (volume_gateway::DeviceIoControlVolumeGateway::GetFileRecord); the
    // local definition was removed with the raw DeviceIoControl call (DDD #8).
    ...
    // The call goes through the gateway seam (DDD #8); GetLastError() still
    // holds the kernel error here (the live impl makes no API calls after
    // DeviceIoControl).
    DWORD bytes_returned_dword = 0;
    if (!volume_gateway_.GetFileRecord(volume_handle_, file_ref_num,
                                       buffer, static_cast<DWORD>(buffer_size),  // NOSONAR(cpp:S1905) - Cast required: buffer_size is size_t, the kernel takes DWORD
                                       bytes_returned_dword)) {
...
// Validates the ioctl wrapper and applies the USA fixup in place
// (output_buffer is modified; no copy is made). On success sets
// out_record_ptr/size to the record inside output_buffer.
// Reduces cognitive complexity of ParseMftRecord
static inline bool ValidateAndPrepareMftRecord(char* output_buffer,
                                               size_t output_buffer_size,
                                               MftMetadataReader::MftParseStats& parse_stats,
                                               const char*& out_record_ptr,  // NOLINT(readability-non-const-parameter) - output parameter
                                               size_t& out_record_size) {  // NOLINT(readability-non-const-parameter) - output parameter
    ...
    auto* output = reinterpret_cast<NtfsFileRecordOutputBuffer*>(output_buffer);  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast) NOSONAR(cpp:S3630,cpp:S6022) - mutable view: USA fixup below modifies the record in place
    ...
    // The record lives inside the caller's buffer; fixup applies to it directly.
    char* mft_record_ptr = reinterpret_cast<char*>(output->FileRecordBuffer);  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast) NOSONAR(cpp:S3630,cpp:S6022) - Windows MFT structure access
    ...
    if (mft_record_size > kMftRecordSize) {
        ++parse_stats.record_too_large;
        return false; // MFT record too large for our buffer
    }

    // PERFORMANCE: Check if fixup is needed before applying
    const auto* file_record_check = reinterpret_cast<const FileRecord*>(mft_record_ptr);  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast) NOSONAR(cpp:S3630,cpp:S6022) - Windows MFT structure access
    ...
    if (bool needs_fixup = (file_record_check->UsaCount > 0 && file_record_check->UsaOffset > 0); needs_fixup) {
        ApplyMftFixup(mft_record_ptr, mft_record_size, kBytesPerSector);
    }

    // Validate record
    const auto* file_record = reinterpret_cast<const FileRecord*>(mft_record_ptr);  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast) NOSONAR(cpp:S3630,cpp:S6022) - Windows MFT structure access
    if (!file_record->IsValid() || !file_record->IsInUse()) {
        ++parse_stats.invalid_header_or_not_in_use;
        return false;
    }

    // Validate FirstAttributeOffset is within buffer
    if (file_record->FirstAttributeOffset >= mft_record_size ||
        file_record->FirstAttributeOffset < sizeof(FileRecord)) {
        ++parse_stats.invalid_first_attribute_offset;
        return false;  // Invalid attribute offset
    }

    out_record_ptr = mft_record_ptr;
    out_record_size = mft_record_size;
    return true;
}
...
bool MftMetadataReader::ParseMftRecord(char* output_buffer, size_t output_buffer_size,
                                       FILETIME* out_modification_time,
                                       uint64_t* out_file_size) {
    CTRACK_DEV_NAME("MftMetadataReader::ParseMftRecord");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
    // Validate wrapper and apply USA fixup in place (no copy; output_buffer modified).
    const char* record_ptr = nullptr;
    size_t record_size = 0;
    if (!ValidateAndPrepareMftRecord(output_buffer, output_buffer_size, parse_stats_,
    ...
    const auto* file_record = reinterpret_cast<const FileRecord*>(record_ptr);  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast) NOSONAR(cpp:S3630,cpp:S6022) - Windows MFT structure access
    const auto* attr = reinterpret_cast<const AttributeRecord*>(  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast) NOSONAR - Windows MFT structure access
        record_ptr + file_record->FirstAttributeOffset);
    const char* record_end = record_ptr + record_size;
    ...
    while (reinterpret_cast<const char*>(attr) < record_end &&  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast) NOSONAR - Windows MFT structure access
        ...
        // Check for $DATA (0x80) - contains authoritative file size
