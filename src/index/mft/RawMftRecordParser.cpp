...
inline void ParseStandardInformationAttribute(
  ...
  if (attr->type_code == kAttributeStandardInformation && !attr->IsNonResident() &&
    ...
    const auto* std_info = reinterpret_cast<const NtfsStandardInformation*>(
...
inline void ParseFileNameAttribute(
  ...
  const auto* fn_attr = reinterpret_cast<const NtfsFileNameAttribute*>(
      attr_ptr + attr->form.resident.value_offset);
...
bool RawMftRecordParser::ApplyIdempotentUsaFixup(
  ...
  const auto* file_record = reinterpret_cast<const NtfsFileRecordHeader*>(record_buffer);
  ...
  const auto* fixup_array = reinterpret_cast<const uint16_t*>(record_buffer + file_record->usa_offset);
  ...
  auto* record_words = reinterpret_cast<uint16_t*>(record_buffer);
...
std::string RawMftRecordParser::TranscodeUtf16ToUtf8(
  ...
  for (size_t i = 0; i < length; ++i) {
    uint32_t code_point = utf16_chars[i];
    if (code_point >= 0xD800U && code_point <= 0xDBFFU && i + 1 < length) {
      const uint32_t next = utf16_chars[i + 1];
      if (next >= 0xDC00U && next <= 0xDFFFU) {
        code_point = (((code_point - 0xD800U) << 10U) | (next - 0xDC00U)) + 0x10000U;
        ++i;
      }
    }
    ...
    if (code_point < 0x80U) {
      ...
      result.push_back(static_cast<char>(0xC0U | ((code_point >> 6U) & 0x1FU)));
      result.push_back(static_cast<char>(0x80U | (code_point & 0x3FU)));
    } else if (code_point < 0x10000U) {
      result.push_back(static_cast<char>(0xE0U | ((code_point >> 12U) & 0x0FU)));
      result.push_back(static_cast<char>(0x80U | ((code_point >> 6U) & 0x3FU)));
      result.push_back(static_cast<char>(0x80U | (code_point & 0x3FU)));
    } else {
      result.push_back(static_cast<char>(0xF0U | ((code_point >> 18U) & 0x07U)));
...
void RawMftRecordParser::ParseSingleRecord(
  ...
  const auto* header = reinterpret_cast<const NtfsFileRecordHeader*>(record_ptr);
