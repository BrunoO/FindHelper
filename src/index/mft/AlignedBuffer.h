...
#else
    void* ptr = nullptr;
    if (const size_t align = (std::max)(alignment, sizeof(void*));
        posix_memalign(&ptr, align, size) == 0) {
...
#else
      free(data_);  // NOLINT(cppcoreguidelines-no-malloc,cppcoreguidelines-owning-memory) NOSONAR(cpp:S1231) - Low-level aligned buffer: posix_memalign mandates free; VirtualFree on Windows
#endif  // _WIN32
      data_ = nullptr;
      size_ = 0;
    }
