...
void FileIndex::ReserveForPopulation(size_t expected_entries) {
  ...
  // During population most paths are bare-name placeholders (parents often
  // unresolved until RecomputeAllPaths), so name-sized bytes suffice here.
  path_storage_.Reserve(capped, capped * kAvgNameBytes);
...
void FileIndex::InsertBatch(const std::vector<PopulationBatchEntry>& batch) {
  ...
  }
  {
    ...
    for (const auto& entry : batch) {
      ...
      // out-of-order parents in one topological pass.
      ...
      if (!entry.is_directory && entry.file_size != kFileSizeNotLoaded && entry.file_size > 0) {
        storage_.UpdateFileSize(entry.id.raw, entry.file_size);
      }
    }
