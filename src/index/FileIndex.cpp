...
void PruneBrokenParentSubtreesLocked(const FileIndexStorage& storage,
  ...
  for (const auto& [id, entry] : storage) {
    if (IsTrueVolumeRootParent(entry.parentID, id)) {
      continue;
    }
    ...
    const auto [parent_entry, resolved_parent_id] = storage.ResolveEntryReference(entry.parentID);
    if (parent_entry == nullptr) {
      orphan_roots.push_back(id);
      continue;
    }
    children[resolved_parent_id].push_back(id);
  }
