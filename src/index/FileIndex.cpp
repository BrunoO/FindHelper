...
void FileIndex::ReserveForPopulation(size_t expected_entries) {
  ...
  // nothing writes them during population either, but RecomputeAllPaths
  // clear()s them (capacity persists) and refills them via the per-path
  // callback folded into the same topological pass — so this reservation
  // serves that single pass instead of being thrown away.
...
void FileIndex::PruneOrphanSubtreesLocked() {
  const ScopedBenchmarkTimer timer("FileIndex prune orphans");
  PruneBrokenParentSubtreesLocked(storage_, operations_);
}

void FileIndex::RebuildPathToIdMapLocked() {
  const ScopedBenchmarkTimer timer("FileIndex rebuild path-to-id map");
  path_to_id_index_.clear();
  path_to_id_chain_.clear();
