...
// Constants for baseline journal enumeration
namespace {
...
IndexBuildRun::IndexBuildRun(HANDLE volume_handle, volume_gateway::VolumeGateway& gateway,
    ...
    : volume_handle_(volume_handle),
      ...
      enable_mft_metadata_reading_(enable_mft_metadata_reading),
      last_progress_update_time_(std::chrono::steady_clock::now()) {
  if (enable_mft_metadata_reading_) {
    mft_reader_.emplace(volume_handle_, volume_gateway_);
...
bool IndexBuildRun::TryReadMetadata(uint64_t mft_record_number, FILETIME* out_time,
                                    uint64_t* out_size) {
  assert(out_time != nullptr && out_size != nullptr && "TryReadMetadata needs output storage");
  if (!mft_reader_.has_value()) {
    return false;
  }
  // NOTE: no consecutive-record fast path. Enumeration is FRN-ordered, so a
  // repeated record number (hard links) would be adjacent if the journal
  // yields one record per link — but hard links are rare on indexed volumes
  // (system ones are filtered), so a 1-entry cache would save ~nothing while
  // adding state.
  ++mft_total_files_;
  const bool ok = mft_reader_->TryGetMetadata(mft_record_number, out_time, out_size);
  if (ok) {
    ++mft_success_count_;
  } else {
    ++mft_failure_count_;
...
IndexBuildRun::Result IndexBuildRun::GetResult() const {
  return {total_files_, iterations_, filtered_count_, mft_success_count_,
          mft_failure_count_, mft_total_files_, integrity_hit_,};
}
...
void IndexBuildRun::LogOutcome() const {
  ...
  if (mft_total_files_ > 0) {
    const double success_rate =
        (static_cast<double>(mft_success_count_) / static_cast<double>(mft_total_files_)) * 100.0;
    LOG_INFO_BUILD("MFT Statistics: " << mft_success_count_ << " succeeded, "
                                      << mft_failure_count_ << " failed out of " << mft_total_files_
  ...
  if (mft_reader_.has_value()) {
    mft_reader_->LogParseStatistics();
  }
}
...
bool IndexBuildRun::StageRecord(PUSN_RECORD_V2 record,
  ...
  uint64_t file_size = kFileSizeNotLoaded;  // NOLINT(misc-const-correctness) - Passed by non-const pointer to TryReadMetadata as output parameter

  // Optional: read metadata from MFT (files only). Skipped when the run has
  // no reader (metadata reading disabled). On failure mod_time/file_size keep
  // their not-loaded sentinels for the lazy loader; see TryReadMetadata.
  if (!is_directory) {
    CTRACK_DEV_NAME("InitialIndexPopulator::MftMetadataRead");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
    // FSCTL_GET_NTFS_FILE_RECORD expects only the 48-bit file record number.
    TryReadMetadata(ntfs_file_reference::RecordNumber(file_ref_num), &mod_time, &file_size);
  }
...
// Returns true on success, false on failure. The run owns the whole baseline
// lifecycle (reserve/stage/commit/result); recompute/prune stays with the
// caller via IndexBuildRun::RecomputePathsAndPruneOrphans so it runs after
// the replay drain (see UsnMonitor::RunInitialPopulationAndPrivileges).
// RecomputeAllPaths() is intentionally NOT called here: calling it too would
// duplicate the O(N) work.
bool PopulateInitialIndex(index_build_run::IndexBuildRun& run) {
  ...
  // MFT enum cursor: first record, full USN range.
  usn_journal_enum_position::UsnJournalEnumerationPosition position =
      usn_journal_enum_position::UsnJournalEnumerationPosition::Start();

  const int buffer_size = kBufferSize;
  std::vector<char> buffer(buffer_size);

  run.Reserve();
  if (!run.Run(buffer, position)) {
    return false;
  }
