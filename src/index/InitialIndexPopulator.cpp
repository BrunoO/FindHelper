...
bool IndexBuildRun::TryReadMetadata(uint64_t mft_record_number, FILETIME* out_time,
  ...
  // (callers strip the sequence before calling).
...
bool IndexBuildRun::StageRecord(PUSN_RECORD_V2 record,
  ...
  // kFileSizeNotLoaded / MFT failure / zero size all mean "skip the update",
  // matching the previous per-record Insert() behaviour.
...
bool IndexBuildRun::Run(std::vector<char>& buffer,
  ...
  while (!enumeration_complete) {
    ...
    if (!volume_gateway_.EnumMft(volume_handle_, position.Data(), buffer.data(),
