 ...
 * @param root_id File or directory reference (typically a USN FRN)
 */
void RemoveIndexedSubtree(FileIndex& file_index, ntfs_file_reference::NtfsFileReference root_id);

/**
 * @brief Same as RemoveIndexedSubtree; caller MUST hold unique_lock on the index.
 *
 * Used by the USN processor when applying a whole journal buffer under one lock.
 */
