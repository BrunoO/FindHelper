...
void FileIndexStorage::InsertLocked(uint64_t id, uint64_t parent_id,
  ...
  if (register_mft_record) {
    record_number_to_id_.insert_or_assign(ntfs_file_reference::RecordNumber(id), id);
  }

  if (inserted) {
    entry_count_.fetch_add(1);
  } else {
    // MFT enumeration should not revisit the same FRN, but refresh parent linkage if it does.
...
void FileIndexStorage::RemoveLocked(uint64_t id) {
  if (auto it = id_to_entry_.find(id); it != id_to_entry_.end()) {
    ...
    if (const auto record_it = record_number_to_id_.find(ntfs_file_reference::RecordNumber(id));
        record_it != record_number_to_id_.end() && record_it->second == id) {
...
std::pair<const FileEntry*, uint64_t> FileIndexStorage::ResolveEntryReference(
  ...
  if (const auto record_it =
          record_number_to_id_.find(ntfs_file_reference::RecordNumber(file_reference));
