...
struct FileEntry {  // NOSONAR(cpp:S8379) - Lazy cache fields mutated only under FileIndex index_mutex_
  // Full 64-bit FRN (sequence + record); use .raw at uint64_t boundaries.
  ntfs_file_reference::NtfsFileReference parentID;  // NOLINT(readability-identifier-naming) - camelCase kept for backward compatibility
...
public:
  ...
  // Pre-size maps and name arena before a bulk population (e.g. baseline journal enumeration).
  // Eliminates geometric rehash pauses during the build loop. No-op when the
  // requested capacity is below current capacity (reserve never shrinks).
  // Caller must hold unique_lock on mutex (or guarantee exclusive access).
  void Reserve(size_t expected_entries, size_t expected_name_bytes) {
    id_to_entry_.reserve(expected_entries);
    record_number_to_id_.reserve(expected_entries);
    directory_path_to_id_.reserve((expected_entries / 8U) + 64U);
    name_cache_.Reserve(expected_entries, expected_name_bytes);
  }
  ...
  //
  // ids are NtfsFileReference (full 64-bit FRN shape, sequence included). The
  // wrapper is non-validating, so synthetic crawl ids keep working — but the
  // type forces every call site to name which reference it passes instead of
  // threading a bare uint64_t that could be a stripped record number.
  //
  ...
  [[nodiscard]] inline uint64_t GetDirectoryId(std::string_view path) const {
    ...
    if (const auto it = directory_path_to_id_.find(path); it != directory_path_to_id_.end()) {
      return it->second;
    }
#else
    // std::unordered_map (C++17) does not support heterogeneous lookup; construct key once
    const std::string path_str(path);
    if (const auto it = directory_path_to_id_.find(path_str); it != directory_path_to_id_.end()) {
      return it->second;
    }
  ...
  using Iterator = flat_hash_map_t<ntfs_file_reference::NtfsFileReference,
                                   FileEntry>::const_iterator;
...
private:
  ...
  std::shared_mutex& shared_mutex_ref_;
  ...
  // Keyed by MftRecordNumber so a full 64-bit FRN cannot flow into a stripped-key
  // lookup without an explicit strip at the boundary (see FileIndexStorage.cpp).
  flat_hash_map_t<ntfs_file_reference::MftRecordNumber, uint64_t> record_number_to_id_;
  ...
  string_key_map_t<uint64_t> directory_path_to_id_;
