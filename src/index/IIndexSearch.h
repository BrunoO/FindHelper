...
public:
  ...
  /**
   * Overload accepting bool folders_only for backward compatibility.
   */
  [[nodiscard]] virtual std::vector<std::future<SearchResultBatch>>
  SearchAsyncWithData(std::string_view query, int thread_count,  // NOSONAR(cpp:S107) - Parallel search API mirrors configuration; NOLINT(readability-function-size)
                      SearchStats* stats,
                      std::string_view path_query,
                      const std::vector<std::string>* extensions,
