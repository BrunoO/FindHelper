 ...
 * - I/O operations happen without any locks held (see docs/design/LOCK_ORDERING_AND_CRITICAL_SECTIONS.md)
...
class LazyAttributeLoader {
 public:
  ...
  static FileAttributes LoadAttributes(const char* path);
  ...
  static FILETIME GetFileModificationTime(const char* path);
  ...
  [[nodiscard]] FileAttributes GetAttributes(uint64_t id) const;
