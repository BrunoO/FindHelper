...

    {
      ...
      for (const auto& job : batch) {
        ...
        if (job.generation == generation_) {
          ...
          results_.try_emplace(job.folder_id, it != sizes.end() ? it->second : FolderStats{});
...
hash_map_t<uint64_t, FolderSizeAggregator::FolderStats> FolderSizeAggregator::ComputeSizeBatch(
...
#endif  // _WIN32
    prefix_to_id.try_emplace(std::move(prefix_key), job.folder_id);
  }
  ...
  for (const auto& [folder_id, file_data] : file_data_per_dir) {
    ...
    result.try_emplace(folder_id, FolderStats{total, count});
  }
