...
void SearchWorker::ExecuteSearch(const SearchParams& params) {
  ...
  const ItemTypeFilter effective_item_type =
      EffectiveItemTypeFilter(params.itemTypeFilter, params.foldersOnly);
  const char* item_type_str = "All";
  if (effective_item_type == ItemTypeFilter::FilesOnly) {
    item_type_str = "Files only";
  } else if (effective_item_type == ItemTypeFilter::FoldersOnly) {
    item_type_str = "Folders only";
  }
  LOG_INFO_BUILD("Item type filter: " << item_type_str);
