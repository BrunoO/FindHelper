...
std::vector<SearchResult> MergeAndConvertToSearchResults(  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables,readability-identifier-naming) - free function, PascalCase API
  ...
  for (const SearchResultData& datum : data) {
    ...
    if (datum.isDirectory) {
      ...
      // TryGetCachedAttributes acquires shared_lock internally, so the copy of
      // fileSize/lastModificationTime is safe against concurrent USN-journal writes
      // and flat_hash_map rehashes that would otherwise invalidate a raw pointer.
      if (const auto attrs = file_index.TryGetCachedAttributes(datum.id)) {
