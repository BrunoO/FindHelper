...
// Simple glob shapes reduced to cheaper matchers. Only applies when the pattern
// has no '?' (question marks need the general linear matcher). Checked once per
// search at matcher construction, not per item.
template<typename TextType>
inline typename TextTypeTraits<TextType>::ReturnType GlobContainsMatcher(
    std::string literal, bool case_sensitive) {
  if (case_sensitive) {
    const string_search::SimdSubstringHints hints =
        string_search::MakeSimdSubstringHints(literal);
    return [literal = std::move(literal), hints](TextType text) {  // NOLINT(bugprone-exception-escape) - matcher performs string comparison and does not throw in practice
      return string_search::ContainsSubstring(text, literal, hints);
    };
  }
  std::string lower = ToLower(literal);
  const string_search::SimdSubstringHints hints =
      string_search::MakeSimdSubstringHints(lower);
  return [lower = std::move(lower), hints](TextType text) {  // NOLINT(bugprone-exception-escape) - matcher performs string comparison and does not throw in practice
    return string_search::ContainsSubstringI(text, lower, hints);
  };
}

template<typename TextType>
inline typename TextTypeTraits<TextType>::ReturnType GlobPrefixMatcher(
    std::string literal, bool case_sensitive) {
  if (case_sensitive) {
    return [literal = std::move(literal)](TextType text) {  // NOLINT(bugprone-exception-escape) - matcher performs string comparison and does not throw in practice
      return text.size() >= literal.size() && text.substr(0, literal.size()) == literal;
    };
  }
  return [literal = std::move(literal)](TextType text) {  // NOLINT(bugprone-exception-escape) - matcher performs string comparison and does not throw in practice
...
template<typename TextType>
inline typename TextTypeTraits<TextType>::ReturnType GlobSuffixMatcher(
    std::string literal, bool case_sensitive) {
  if (case_sensitive) {
    return [literal = std::move(literal)](TextType text) {  // NOLINT(bugprone-exception-escape) - matcher performs string comparison and does not throw in practice
      return text.size() >= literal.size() &&
             text.substr(text.size() - literal.size()) == literal;
    };
  }
  return [literal = std::move(literal)](TextType text) {  // NOLINT(bugprone-exception-escape) - matcher performs string comparison and does not throw in practice
...
// Classifies '*lit*', 'lit*', '*lit', and all-stars shapes. Returns an empty
// callable when the pattern needs the general linear glob matcher.
template<typename TextType>
inline typename TextTypeTraits<TextType>::ReturnType TryGlobShapeMatcher(
    std::string_view pattern_str, bool case_sensitive) {
  using ReturnType = typename TextTypeTraits<TextType>::ReturnType;
  ...
  if (star_count == 2 && pattern_str.front() == '*' && pattern_str.back() == '*') {
    return GlobContainsMatcher<TextType>(
        std::string(pattern_str.substr(1, pattern_str.size() - 2)), case_sensitive);
  }
  if (star_count == 1 && pattern_str.back() == '*') {
    return GlobPrefixMatcher<TextType>(
        std::string(pattern_str.substr(0, pattern_str.size() - 1)), case_sensitive);
  }
  if (star_count == 1 && pattern_str.front() == '*') {
    return GlobSuffixMatcher<TextType>(std::string(pattern_str.substr(1)), case_sensitive);
  }
  return ReturnType{};
}
...
inline typename TextTypeTraits<TextType>::ReturnType CreateMatcherImplStdRegex(
  ...
  const auto state = std::make_shared<search_pattern_utils_detail::StdRegexMatcherState>();
...
inline typename TextTypeTraits<TextType>::ReturnType CreateMatcherImplPathPattern(
  ...
  const auto compiled_ptr = std::make_shared<path_pattern::CompiledPathPattern>(
...
inline typename TextTypeTraits<TextType>::ReturnType CreateMatcherImplGlob(
  ...
  // Fast path: simple shapes (*lit*, lit*, *lit, all-stars) reduce to cheaper
  // matchers once per search instead of running the glob per item.
  if (auto shaped = search_pattern_utils_detail::TryGlobShapeMatcher<TextType>(
          pattern_str, case_sensitive);
      shaped) {
    return shaped;
  }
  if (case_sensitive) {
