...
inline typename TextTypeTraits<TextType>::ReturnType CreateMatcherImplStdRegex(
  ...
  if (prefilter.empty()) {
    return [compiled_regex, requires_full_match](TextType text) {  // NOLINT(bugprone-exception-escape) - regex_match/search pre-validated by GetCache; no throw expected
      ...
      if (requires_full_match) { return regex_match(text_str, *compiled_regex); }
      return regex_search(text_str, *compiled_regex);
    };
  }
  if (case_sensitive) {
    return [compiled_regex, requires_full_match, prefilter](TextType text) {  // NOLINT(bugprone-exception-escape) - regex_match/search pre-validated by GetCache; no throw expected
      if (!string_search::ContainsSubstring(text, prefilter)) { return false; }
      const std::string text_str = Traits::ConvertToStdString(text);
      if (requires_full_match) { return regex_match(text_str, *compiled_regex); }
      return regex_search(text_str, *compiled_regex);
    };
  }
  // Case-insensitive path — prefilter is pre-lowercased; ContainsSubstringI expects lowercase pattern.
  return [compiled_regex, requires_full_match, prefilter](TextType text) {  // NOLINT(bugprone-exception-escape) - regex_match/search pre-validated by GetCache; no throw expected
    if (!string_search::ContainsSubstringI(text, prefilter)) { return false; }
    const std::string text_str = Traits::ConvertToStdString(text);
    if (requires_full_match) { return regex_match(text_str, *compiled_regex); }
    return regex_search(text_str, *compiled_regex);
  };
