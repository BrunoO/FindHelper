 ...
 * non-directory descendants. Uses a single background worker thread for the
 * index scan and result commit; large Pass-2 stat sweeps additionally run on
 * transient threads spawned per batch (see ComputeSizeBatch for why the shared
 * SearchThreadPool is deliberately not used).
 *
 ...
 *             In results_. Reached when the batch flush in ResultsTable::Render()
 *             calls GetResult() and writes the value into SearchResult.
 *
 *  Allowed transitions:
 *
 *    UNQUEUED  --[Request()/RequestBatch()]--> PENDING
 *    PENDING   --[worker done]-->               COMPUTED
 *    COMPUTED  --[GetResult() + write]-->       WRITTEN
 *
 *  The COMPUTED → WRITTEN transition happens inside the per-frame batch flush in
 ...
 *    - RenderResultsTableRow: directory with real size has formatted fileSizeDisplay != "..."
...
private:
  struct Job {
    uint64_t folder_id_;     // NOLINT(readability-identifier-naming) - project convention: snake_case_ for members
    std::string folder_path_;  // NOLINT(readability-identifier-naming) - project convention: snake_case_ for members
    uint64_t generation_;    // NOLINT(readability-identifier-naming) - Matches generation_ at enqueue time; used to detect Reset().
  };

  void WorkerThread();
  // Compute stats for all jobs in a single index scan: walk each file's parentID
  // chain (O(N_index × depth) instead of O(K × N_index) path-prefix scans).
  // Returns folder_id → FolderStats.
