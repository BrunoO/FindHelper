...
ParallelSearchEngine::SearchAsync(const ISearchableIndex& index,
  ...
  // shared_ptr keeps context alive after FileIndex::SearchAsyncWithData returns (see 2026-01-21 UAF fix).
  ...
  for (int t = 0; t < thread_count; ++t) {
    ...
    futures.push_back(pool.Enqueue(
        [&index, start_index, end_index, shared_context, shared_matchers] {  // NOLINT(bugprone-exception-escape,readability-redundant-lambda-parameter-list) - exceptions propagate to std::future
          CTRACK_NAME("ParallelSearchEngine::WorkerScanChunkIds");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
  ...
  // after collecting results from futures (see FileIndex::SearchAsyncWithData)
  if (stats != nullptr) {
...
void ParallelSearchEngine::ProcessChunkRangeIds(  // NOSONAR(cpp:S3776, cpp:S107) - Hot path with explicit parameter list; further splitting would introduce call overhead or additional indirection
    ...
    const lightweight_callable::LightweightCallable<bool, std::string_view>& filename_matcher,
    const lightweight_callable::LightweightCallable<bool, std::string_view>& path_matcher) {
  CTRACK_DEV_NAME("ParallelSearchEngine::ProcessChunkRangeIds");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
  ...
  for (size_t i = chunk_start; i < chunk_end; ++i) {
    // Check for cancellation every kCancellationCheckInterval items RELATIVE to
    // the chunk start (mirrors ProcessChunkRange's items_checked counter). Using
    // the absolute index (i & mask) meant a chunk whose range straddled no
    // multiple of kCancellationCheckInterval never polled the flag.
    ...
    if (has_cancel_flag && (((i - chunk_start) & kCancellationCheckMask) == 0U) &&  // NOSONAR(cpp:S1066) - Merge if: separate conditions for readability
