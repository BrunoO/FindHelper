...
// Quarantine predicate for the merge loop below: awaiting-parent entries
// (bare-name placeholders for not-yet-indexed parents) are internal backlog,
// not display truth. has_awaiting is the GetAwaitingStats().count > 0 fast path
// so the common case avoids per-hit index locking.
[[nodiscard]] bool ShouldQuarantineHit(const FileIndex& file_index, bool has_awaiting,
                                 ntfs_file_reference::NtfsFileReference id) {
  return has_awaiting && file_index.IsAwaitingChild(id);
}

// Batch attribute backfill for merged rows (single shared-lock lookup via
// FillCachedAttributeSnapshots). Extracted to keep the merge loop below the
// cognitive-complexity budget.
void ApplyCachedAttributeSnapshots(const FileIndex& file_index,
                                    const std::vector<uint64_t>& file_ids,
                                    const std::vector<size_t>& file_result_indices,
...
std::vector<SearchResult> MergeAndConvertToSearchResults(  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables,readability-identifier-naming) - free function, PascalCase API
  ...
  for (const SearchResultBatch& batch : batches) {
    total_hits += batch.hits.size();
    for (const SearchResultHit& hit : batch.hits) {
      // Debug-only producer-consumer contract check: offsets come from
      // AppendHit, so they must land inside the arena. Overflow-safe form.
      assert(hit.path_len <= batch.arena.size() &&
             hit.path_offset <= batch.arena.size() - hit.path_len &&
             "batch hit must reference bytes inside its arena");
      pool_bytes_needed += hit.path_len + 1;
    }
  }
  std::vector<SearchResult> results;
  results.reserve(total_hits);
  pool.reserve(pool.size() + pool_bytes_needed);

  std::vector<uint64_t> file_ids;
  std::vector<size_t> file_result_indices;
  file_ids.reserve(total_hits);
  file_result_indices.reserve(total_hits);

  size_t pool_offset = pool.size();
  // Quarantine: awaiting-parent entries (bare-name placeholders for
  // not-yet-indexed parents) are internal backlog, not display truth — skip
  // them at commit so residual placeholders never reach the results table.
  // Fast path avoids per-hit locking when nothing is awaiting (common case).
  const bool has_awaiting = file_index.GetAwaitingStats().count > 0;
  for (const SearchResultBatch& batch : batches) {
    // Batch alive for the whole merge; arena offsets valid throughout.
    const char* const arena_data = batch.arena.data();
    for (const SearchResultHit& hit : batch.hits) {
      if (ShouldQuarantineHit(file_index, has_awaiting,
                               ntfs_file_reference::NtfsFileReference(hit.id))) {
        continue;
      }
      // Single copy of the path bytes in the pipeline (worker arenas hold the
      // only other copy); safe to read here.
      pool.insert(pool.end(), arena_data + hit.path_offset,
                  arena_data + hit.path_offset + hit.path_len);
      pool.push_back('\0');
      const char* path_start = pool.data() + pool_offset;
      pool_offset += hit.path_len + 1;

      SearchResult result;
      result.fileId = hit.id;
      result.isDirectory = hit.is_directory;
      result.fullPath = std::string_view(path_start, hit.path_len);
      result.filename_offset = hit.filename_start;
      result.extension_offset =
          (hit.extension_start == SIZE_MAX) ? std::string_view::npos : hit.extension_start;
  ...
  ApplyCachedAttributeSnapshots(file_index, file_ids, file_result_indices, results);
  return results;
}
...
void EnsureDisplayStringsPopulated(const SearchResult& result, const FileIndex& file_index) {
  ...
  // Load size and/or modification time if not yet loaded (files only for size; directories use FolderSizeAggregator).
  ...
  const bool need_size = !result.isDirectory &&
      (result.fileSize == kFileSizeNotLoaded || result.fileSize == kFileSizeFailed);

  // Init-statement: need_time is used by the whole else-if chain only.
  if (const bool need_time = IsSentinelTime(result.lastModificationTime);
      need_size && need_time) {
    const auto [size, time] = file_index.GetAttributesById(result.fileId);
  ...
  // If fileSizeDisplay is "..." but fileSize has a real value, FlushAggregatorFolderStats
  // failed to overwrite the placeholder when the aggregator result arrived.
...
[[nodiscard]] inline bool IsPendingDirectory(const SearchResult& result) {
  return result.isDirectory &&
         (result.fileSize == kFileSizeNotLoaded ||
          result.folderFileCount == kFolderFileCountNotLoaded);
}

size_t ApplyAggregatorStatsToResults(std::vector<SearchResult>& results,
                                    const FolderSizeAggregator& aggregator) {
  size_t updated_count = 0;
  for (auto& result : results) {
    if (!IsPendingDirectory(result)) {
      continue;
    }
...
size_t FlushAggregatorFolderStats(GuiState& state, FolderSizeAggregator* aggregator) {
  if (aggregator == nullptr || state.result_pool_->Results().empty()) {
    return 0;
  }

  // Idle fast-path: same result set as the last full scan (commits bump the version),
  // the aggregator has no pending work (no new completions can have arrived), and the
  // last scan found no pending directories — a rescan would find nothing again.
  // Any commit, any aggregator progress, or any previously-pending dir re-arms the scan.
  if (!state.search_pipeline.folder_stats_flush_had_pending &&
      state.search_pipeline.last_folder_stats_flush_version == state.GetResultsVersion() &&
      !aggregator->HasPendingWork()) {
    return 0;
  }
  ...
  // Idle check: no pending directories in the result pool.
  if (pending_before == 0) {
    state.search_pipeline.last_folder_stats_flush_version = state.GetResultsVersion();
    state.search_pipeline.folder_stats_flush_had_pending = false;
    return 0;
  }

  // Pass 1: enqueue all unqueued pending directories in a single lock acquisition.
  if (!batch.empty()) {
    aggregator->RequestBatch(batch);
  }

  // Pass 2: collect and apply any results that have finished computing.
  const size_t updated_count =
      ApplyAggregatorStatsToResults(state.result_pool_->Results(), *aggregator);

  // Sync active filter caches so filtered views immediately reflect new stats.
  if (updated_count > 0 && state.searchCriteria.time_filter != TimeFilter::None &&
      state.filter_caches.time.valid && !state.filter_caches.time.results.empty()) {
    ApplyAggregatorStatsToResults(state.filter_caches.time.results, *aggregator);
  }

  // Sort trigger: only for complete result sets while sorting by Size or Descendant Files.
  // Display handoff runs during in-flight searches (e.g. auto-refresh double-buffering) so
  // visible directory rows can leave "..." without waiting for the next search to finish.
  if (const bool is_sorting_by_folder_stats =
          (state.async_sort_.last_sort_column == ResultColumn::Size ||
           state.async_sort_.last_sort_column == ResultColumn::FolderFiles);
      state.search_pipeline.results_complete && is_sorting_by_folder_stats) {
    const auto pending_after = static_cast<size_t>(std::count_if(  // NOLINT(llvm-use-ranges) - C++17; std::ranges requires C++20
        state.result_pool_->Results().begin(),
        state.result_pool_->Results().end(),
        IsPendingDirectory));
    if (pending_before > pending_after && pending_after == 0) {
      assert(pending_after == 0 &&  // NOSONAR(cpp:S2583) - Intentional debug invariant; documents sort pre-condition alongside count_if
             std::none_of(state.result_pool_->Results().begin(),
                          state.result_pool_->Results().end(), IsPendingDirectory) &&  // NOLINT(llvm-use-ranges) - C++17; std::ranges requires C++20
             "ResultsTable: sort triggered before all directory stats are confirmed");
      state.search_pipeline.results_updated = true;
      // Phase 3: folder stats changed result content; stale filter caches.
      state.BumpResultsVersion();
    }
  }

  // Record this full scan for the idle fast-path (read at function entry).
  state.search_pipeline.last_folder_stats_flush_version = state.GetResultsVersion();
  state.search_pipeline.folder_stats_flush_had_pending = pending_before > 0;
  return updated_count;
}
...
static bool HandleCloudFileTimeLoading(
  ...
  ThreadPool* thread_pool,  // NOLINT(misc-const-correctness) NOSONAR(cpp:S995) - Parameter is used to invoke non-const operations
  ...
  if (state->cloud_files.TryEnqueue(result.fileId)) {
...
static std::vector<SearchResult> ApplyTimeFilter(const std::vector<SearchResult>& results,
  ...
  if (!cloud_files_to_load.empty() && state != nullptr && thread_pool != nullptr) {
    for (const uint64_t file_id : cloud_files_to_load) {
      state->cloud_files.loading_futures.emplace_back(thread_pool->enqueue([file_id, &file_index] {
...
// Phase 3 helpers: same semantics as before, plus version capture so the
// slice is considered valid until the next results change (BumpResultsVersion).
// Take the cache slice by reference (sonar-cpp cpp:S107: bundle the fields
// instead of a 6/8-parameter out-list).
static void UpdateFilterCacheForWelcomePanel(TimeFilterCacheSlice& slice,
                                             TimeFilter current_filter,
                                             ResultsVersion current_version) {
  slice.results.clear();
  slice.count = 0;
  slice.cached_filter = current_filter;
  slice.valid = true;
  slice.captured_version = current_version;
}

static void UpdateFilterCacheForWelcomePanel(SizeFilterCacheSlice& slice,
                                             SizeFilter current_filter,
                                             ResultsVersion current_version) {
  slice.results.clear();
  slice.count = 0;
  slice.cached_filter = current_filter;
...
static void UpdateFilterCacheForNoFilter(TimeFilterCacheSlice& slice,
                                         TimeFilter current_filter,
                                         size_t source_count,
                                         ResultsVersion current_version) {
  slice.results.clear();
  slice.count = source_count;
  slice.cached_filter = current_filter;
  slice.valid = true;
  slice.captured_version = current_version;
}

static void UpdateFilterCacheForNoFilter(SizeFilterCacheSlice& slice,
                                         SizeFilter current_filter,
                                         size_t source_count,
                                         ResultsVersion current_version) {
  slice.results.clear();
...
static bool CleanUpCloudFutures(GuiState& state) {
  ...
  if (cloud_files_completed) {
    state.filter_caches.time.valid = false;
...
// Helper: check whether the time filter cache needs rebuild.
// Phase 3: cache staleness is version-keyed — valid requires captured_version
// to match GuiState::ResultsVersion() (bumped on every results change).
static bool ShouldRebuildTimeFilterCache(const GuiState& state, bool cloud_files_completed) {
  const bool filter_changed =
      (state.filter_caches.time.cached_filter != state.searchCriteria.time_filter);
  const bool need_rebuild = !state.filter_caches.time.IsValidFor(state.GetResultsVersion()) ||
                            filter_changed || state.search_pipeline.results_updated || cloud_files_completed;
  return need_rebuild;
}

bool ShouldDeferFilterCacheRebuild(const GuiState& state) {
  const bool time_filter_active = state.searchCriteria.time_filter != TimeFilter::None;
  const bool size_filter_active = state.searchCriteria.size_filter != SizeFilter::None;
...
void UpdateTimeFilterCacheIfNeeded(GuiState& state, const FileIndex& file_index,
  ...
  if (!state.search_pipeline.search_active && state.result_pool_->Results().empty()) {
    UpdateFilterCacheForWelcomePanel(state.filter_caches.time, state.searchCriteria.time_filter,
                                     state.GetResultsVersion());
    return;
  }

  // If no time filter, filtered results == full results
  if (state.searchCriteria.time_filter == TimeFilter::None) {
    UpdateFilterCacheForNoFilter(state.filter_caches.time, state.searchCriteria.time_filter,
                                 state.result_pool_->Results().size(), state.GetResultsVersion());
    return;
  }
  ...
  if (state.search_pipeline.defer_filter_cache_rebuild) {
    if (!ShouldDeferFilterCacheRebuild(state)) {
      state.search_pipeline.defer_filter_cache_rebuild = false;
    } else {
      // Show unfiltered results for this frame, rebuild cache on next frame
      state.filter_caches.time.results.clear();
      state.filter_caches.time.count = state.result_pool_->Results().size();
  ...
  if (const bool total_size_in_progress =
          !state.filter_caches.total_size.valid && state.filter_caches.total_size.computation_index > 0;
  ...
  state.filter_caches.time.results =
    ApplyTimeFilter(state.result_pool_->Results(), state.searchCriteria.time_filter, file_index, &state, thread_pool);
  state.filter_caches.time.count = state.filter_caches.time.results.size();
  state.filter_caches.time.cached_filter = state.searchCriteria.time_filter;
  state.filter_caches.time.MarkStoredVersion(state.GetResultsVersion());
}
...
void UpdateSizeFilterCacheIfNeeded(GuiState& state, const FileIndex& file_index) {
  // If search is not active AND no results, keep cache trivial
  if (!state.search_pipeline.search_active && state.result_pool_->Results().empty()) {
    UpdateFilterCacheForWelcomePanel(state.filter_caches.size, state.searchCriteria.size_filter,
                                     state.GetResultsVersion());
    return;
  }
  ...
  if (state.searchCriteria.time_filter != TimeFilter::None && !state.filter_caches.time.results.empty()) {
    // Use time-filtered results as the source
    source_results = &state.filter_caches.time.results;
    source_count = state.filter_caches.time.count;
  } else {
  ...
  if (state.searchCriteria.size_filter == SizeFilter::None) {
    UpdateFilterCacheForNoFilter(state.filter_caches.size, state.searchCriteria.size_filter,
                                 source_count, state.GetResultsVersion());
  ...
  if (state.search_pipeline.defer_filter_cache_rebuild) {
    if (!ShouldDeferFilterCacheRebuild(state)) {
      ...
      state.filter_caches.size.count = source_count;
      state.search_pipeline.defer_filter_cache_rebuild = false;
      return;
    }
  }
  ...
  const bool filter_changed =
      (state.filter_caches.size.cached_filter != state.searchCriteria.size_filter);
  const bool need_rebuild = !state.filter_caches.size.IsValidFor(state.GetResultsVersion()) ||
                            filter_changed || state.search_pipeline.results_updated ||
  ...
  if (LoadSizeAttributesWithBudget(*source_results, file_index, kFilterAttributeLoadsPerFrame)) {
    state.filter_caches.size.results.clear();
    state.filter_caches.size.count = source_count;
    state.filter_caches.size.valid = false;
    return;
  }

  state.filter_caches.size.results = ApplySizeFilter(*source_results, state.searchCriteria.size_filter, file_index);
  state.filter_caches.size.count = state.filter_caches.size.results.size();
  state.filter_caches.size.cached_filter = state.searchCriteria.size_filter;
  state.filter_caches.size.MarkStoredVersion(state.GetResultsVersion());

  // Piggyback: accumulate total size during size filter pass (sizes already loaded)
  state.filter_caches.total_size.bytes = ComputeTotalFileBytes(state.filter_caches.size.results);
...
void UpdateDisplayedTotalSizeIfNeeded(GuiState& state, const FileIndex& file_index) {
  ...
  if (state.filter_caches.total_size.valid) {
  ...
  if (!state.search_pipeline.defer_filter_cache_rebuild) {
    if (state.searchCriteria.size_filter != SizeFilter::None && state.filter_caches.size.valid) {
      display_results_ptr = &state.filter_caches.size.results;
    } else if (state.searchCriteria.time_filter != TimeFilter::None && state.filter_caches.time.valid) {
      display_results_ptr = &state.filter_caches.time.results;
    }
  }
  ...
  size_t& idx = state.filter_caches.total_size.computation_index;
  uint64_t& acc = state.filter_caches.total_size.computation_bytes;

  // Reset when idx is out of bounds (e.g. after invalidation or source vector replaced).
  // Source changes are handled by InvalidateDisplayedTotalSize when filter_caches.time.results is rebuilt.
  ...
  if (idx >= results.size()) {
    state.filter_caches.total_size.bytes = acc;
    state.filter_caches.total_size.valid = true;
    state.ResetDisplayedTotalSizeProgress();
  }
}
...
static void RunAttributeLoadBatchTask(std::vector<SortAttributeLoadItem> batch,
  ...
  std::vector loaded_sizes(batch.size(), kFileSizeNotLoaded);
  std::vector loaded_times(batch.size(), kFileTimeNotLoaded);
  ...
  for (size_t b = 0; b < batch.size(); ++b) {
    ...
    if (item.need_size_ && item.need_time_) {
      const auto [size, time] = ctx.file_index_->GetAttributesById(item.file_id_);
      loaded_sizes[b] = size;
      loaded_times[b] = time;
...
void SortSearchResults(std::vector<SearchResult>& results, int column_index,
                       ImGuiSortDirection direction, const FileIndex& file_index,
                       ThreadPool& thread_pool, bool show_hierarchy) {
  if (results.empty()) {
    return;
  }
  ...
  std::sort(results.begin(), results.end(),
            CreateSearchResultComparator(column_index, direction, show_hierarchy));
}
...
bool CheckSortAttributeLoadingAndSort(GuiState& state, std::vector<SearchResult>& results,
  ...
  std::sort(results.begin(), results.end(), CreateSearchResultComparator(column_index, direction, show_hierarchy));
