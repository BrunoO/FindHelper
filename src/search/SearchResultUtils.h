...
/**
 * @brief Merge SearchResultData into a path pool and convert to SearchResult.
 *
 * Appends each path to pool (with trailing null for .c_str() use); each
 * SearchResult.fullPath is a string_view into the pool. Cached file size/time
 * are copied via a single FillCachedAttributeSnapshots batch lookup.
 ...
 * @return true if UpdateSearchResults should set deferFilterCacheRebuild
 */
 ...
 * in state.displayedTotalSizeBytes. Skipped while state.resultsComplete is false (search
 * still running). Uses cached value when displayedTotalSizeValid is true.
 *
