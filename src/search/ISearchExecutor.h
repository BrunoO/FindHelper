 ...
 * @brief Interface for search execution operations used by HybridStrategy
 *
 * This interface decouples HybridStrategy from the concrete ParallelSearchEngine
 * implementation, enabling:
 * - Better testability (can mock the interface)
 * - Reduced coupling (strategy doesn't depend on ParallelSearchEngine)
 *
 * DESIGN DECISION:
 * Only GetThreadPool() is virtualized because:
 * - It's the only instance method needed by the strategy
