...
void CompilePatternIfNeeded(std::string_view query,
                            search_pattern_utils::PatternType pattern_type,
                            bool case_sensitive,
  ...
  if (pattern_type != search_pattern_utils::PatternType::PathPattern) {
    return;
  }
...
SearchContext SearchContextBuilder::Build(
  ...
  context.item_type_filter = item_type_filter;
  context.folders_only = (item_type_filter == ItemTypeFilter::FoldersOnly);
  ...
  const auto filename_pattern_type = search_pattern_utils::DetectPatternType(query);
  context.use_glob =
      (filename_pattern_type == search_pattern_utils::PatternType::Glob);

  const auto path_pattern_type = search_pattern_utils::DetectPatternType(path_query);
  if (!path_query.empty()) {
  ...
  if (!context.extension_only_mode) {
    search_context_builder_detail::CompilePatternIfNeeded(
        query, filename_pattern_type, case_sensitive, context.filename_pattern, "filename");
    search_context_builder_detail::CompilePatternIfNeeded(
        path_query, path_pattern_type, case_sensitive, context.path_pattern, "path");
  }
