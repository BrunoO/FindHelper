...
struct MetricsTimer {
  std::chrono::high_resolution_clock::time_point start_{std::chrono::high_resolution_clock::now()};  // NOLINT(readability-identifier-naming) - project convention: snake_case_ for members
 ...
 *
 * Depends on IIndexSearch for parallel search execution; FileIndex implements
 * that interface in production.
 */
class SearchWorker {
public:
  explicit SearchWorker(IIndexSearch& index_search);
  ~SearchWorker();
  ...
  /**
   * Returns the latest search result batches (arena-owned paths plus offset
   * hits) for path-pool conversion on the UI thread.
   ...
   */
  std::vector<SearchResultBatch> GetResultsData();
...
private:
  ...
  // Returns total hit count across all batches (for logging/metrics).
  size_t RunFilteredSearchPath(const SearchParams& params,
                               const std::vector<std::string>& extensions,
                               std::string_view filename_search,
                               std::string_view path_search,
                               std::vector<SearchResultBatch>& results);

  IIndexSearch& index_search_;  // NOLINT(readability-identifier-naming) - project convention: snake_case_ for members
  ...
  bool thread_running_ = true;  // NOLINT(readability-identifier-naming) - project convention: snake_case_ for members
  ...
  std::vector<SearchResultBatch> results_data_;  // NOLINT(readability-identifier-naming) - project convention: snake_case_ for members
