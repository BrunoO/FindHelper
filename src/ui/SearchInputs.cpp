...
inline void RenderAISearchStatusMessages(GuiState& state, bool api_key_set, bool is_api_call_in_progress) {
  ...
  if (!state.gemini_error_message_.empty()) {
    ...
    if (now < state.gemini_error_display_time_) {
      ...
      if (is_success) {
        ImGui::TextColored(Theme::Colors::Success, "%s", state.gemini_error_message_.c_str());
      } else if (is_error) {
        ImGui::TextColored(Theme::Colors::Error, "%s", state.gemini_error_message_.c_str());
      } else {
        ImGui::TextWrapped("%s", state.gemini_error_message_.c_str());
      }
    } else {
