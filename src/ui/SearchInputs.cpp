...
inline void RenderAISearchTooltips(bool api_key_set) {
  ...
  if (api_key_set) {
    ...
    ImGui::Spacing();
    ImGui::TextDisabled("(Optional: Set GEMINI_API_KEY for one-click AI search)");
  }
  ImGui::EndTooltip();
}
...
inline void RenderAISearchStatusMessages(GuiState& state, bool api_key_set, bool is_api_call_in_progress) {
  if (!api_key_set) {
    ImGui::TextColored(Theme::Colors::TextDim,
                        ICON_FA_LIGHTBULB " Set GEMINI_API_KEY for one-click AI search (or use Generate Prompt below).");
    ImGui::Spacing();
  }
  ...
  if (state.gemini.api_call_in_progress && state.gemini.api_future.valid()) {
    const auto status = state.gemini.api_future.wait_for(std::chrono::seconds(0));
  ...
  if (!state.gemini.error_message.empty()) {
    ...
    if (now < state.gemini.error_display_time) {
      ImGui::Spacing();
      const bool is_success = state.gemini.error_message.find("✅") == 0;
      const bool is_error = state.gemini.error_message.find("Error:") == 0 ||
                            state.gemini.error_message.find("Failed") == 0 ||
                            state.gemini.error_message.find("Warning:") == 0;

      if (is_success) {
        ImGui::TextColored(Theme::Colors::Success, "%s", state.gemini.error_message.c_str());
      } else if (is_error) {
        ImGui::TextColored(Theme::Colors::Error, "%s", state.gemini.error_message.c_str());
      } else {
        ImGui::TextWrapped("%s", state.gemini.error_message.c_str());
      }
    } else {
...
inline bool RenderPathSearchInput(GuiState& state, float available_width, float button_area_width,
  ...
  if (SearchInputs::RenderInputFieldWithEnter("Path Search", "##path", state.searchCriteria.path_input.Data(),
...
inline bool RenderExtensionsInput(GuiState& state, float button_area_width, float uniform_button_width) {
  ...
  if (SearchInputs::RenderInputFieldWithEnter(
          "Extensions", "##extensions", state.searchCriteria.extension_input.Data(),
          SearchInputField::MaxLength(), state,
...
inline bool RenderFilenameInput(GuiState& state, float available_width, float button_area_width,
  ...
  if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
    ImGui::SetTooltip("Export currently displayed results to CSV file (Downloads, or Desktop if unavailable)");
  }
  if (!can_export) {
    ImGui::EndDisabled();
  }
...
inline void RenderSearchOptions(GuiState& state, bool is_index_building, float uniform_button_width) {
  ...
  ImGui::SameLine();
  static constexpr float kItemTypeComboWidth = 125.0F;
  static constexpr std::array<const char*, 3> kItemTypeLabels = {
      "All items",
      "Files only",
      "Folders only",
  };
  ...
  ImGui::SetNextItemWidth(kItemTypeComboWidth);
  if (ImGui::Combo("##ItemTypeFilter", &current_item_type, kItemTypeLabels.data(), static_cast<int>(kItemTypeLabels.size()))) {
...
void SearchInputs::RenderAISearch(GuiState &state, const AISearchArgs& args) {
  ...
  // Render buttons
  RenderAISearchButtons(state, args.window, args.actions, args.api_key_set, has_description,
                        args.is_index_building, is_api_call_in_progress);

  // Render status messages
  RenderAISearchStatusMessages(state, args.api_key_set, is_api_call_in_progress);
}
