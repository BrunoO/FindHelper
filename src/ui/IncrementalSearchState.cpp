...
bool IncrementalSearchMatches(std::string_view query, const SearchResult& result,
  ...
  // Same volume-root rules as the Path column (C:\file.txt → "C:\", /file → "/")
  const std::string_view filename = path_utils::GetFilename(full_path);
  const std::string_view directory = path_utils::GetDirectoryPathView(full_path);

  if (string_search::ContainsSubstringI(filename, query, hints)) {
    return true;
  }

  if (string_search::ContainsSubstringI(directory, query, hints)) {
    return true;
  }
...
bool IncrementalSearchMatches(std::string_view query, const SearchResult& result) {
  return IncrementalSearchMatches(query, result, string_search::MakeSimdSubstringHints(query));
}
...
void IncrementalSearchState::UpdateQuery(std::string_view new_query,
  ...
  query_hints_ = string_search::MakeSimdSubstringHints(query_);
  filter_active_ = true;
...
void IncrementalSearchState::RefreshFilter(const std::vector<SearchResult>& base_results) {
  if (filter_active_) {
    RebuildFilter(base_results);
  }
}
...
void IncrementalSearchState::RebuildFilter(const std::vector<SearchResult>& base_results) {
  ...
  for (const auto& result : base_results) {
    if (IncrementalSearchMatches(query_view, result, query_hints_)) {
