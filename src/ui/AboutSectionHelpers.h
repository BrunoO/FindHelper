...
/**
 * Platform/monitoring label, e.g. "Windows (Monitoring Active)" or "Windows (No Monitoring)".
 * Pass is_monitoring_active=true on Windows when the USN monitor is running and active.
 * macOS and Linux always show "(No Monitoring)" regardless of the flag.
 */
[[nodiscard]] const char* GetAboutPlatformMonitoringLabel(bool is_monitoring_active = false);
