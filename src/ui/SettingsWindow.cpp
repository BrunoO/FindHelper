...
// Performance tuning: chunk/hybrid bounds live in settings_defaults (single source of truth)
constexpr int kDynamicChunkSizeStep = 500;  // NOLINT(readability-identifier-naming)

// Settings window dimensions
constexpr int kSettingsWindowWidth = 600;   // NOLINT(readability-identifier-naming) - wide enough for InputText + Browse + Copy Path on one line
...
void RenderAppearanceSettings(AppSettings& settings) {
  ...
  // User UI scale (FontScaleMain). Monitor DPI is applied via ConfigDpiScaleFonts / FontScaleDpi.
  if (float scale = settings.fontScale; ImGui::SliderFloat(
    ...
    font_utils::ApplyUserFontScale(scale);
  }

  // Auto: restore default font size and reset user UI scale (DPI is automatic).
  ImGui::SameLine();
  ImGui::PushID("AppearanceAutoFontScale");
  if (ImGui::SmallButton("Auto")) {
    settings.fontScale = 1.0F;
    settings.fontSize = settings_defaults::kDefaultFontSize;
    font_utils::ApplyUserFontScale(settings.fontScale);
  }
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip(
      "Restore default font size (%.0f) and reset UI scale to 1.0.\n"
      "Monitor DPI is applied automatically (current viewport DPI: %.2f).\n"
      "Font size takes effect after Save and restart.",
      static_cast<double>(settings_defaults::kDefaultFontSize),
      static_cast<double>(ImGui::GetWindowDpiScale()));
  }
  ImGui::PopID();
  ...
  constexpr std::array<const char*, 3> kModeLabels = {"Full UI", "Simplified UI",
                                                      "Minimalistic UI"};
  auto current_mode = static_cast<int>(settings.uiMode);
  if (const auto max_mode_index = static_cast<int>(kModeLabels.size()) - 1;
      current_mode < 0 || current_mode > max_mode_index) {
    current_mode = 0;
  }
  ImGui::Combo("UI Mode", &current_mode, kModeLabels.data(),
               static_cast<int>(kModeLabels.size()));
  // Sync every frame so working_settings always matches the combo (same pattern as sliders;
  // avoids relying solely on Combo's "changed" return for persistence on Save).
...
void RenderDynamicChunkSize(AppSettings& settings) {
  if (int chunk_size = settings.dynamicChunkSize;  // NOSONAR(cpp:S1854) - ImGui pattern: modified
      ...
      ImGui::InputInt(
        "Dynamic Chunk Size", &chunk_size, settings_defaults::kMinDynamicChunkSize,
        settings_ui_constants::kDynamicChunkSizeStep)) {  // NOSONAR(cpp:S6004) - Variable modified
                                                          // by reference in ImGui, then used after
                                                          // if
    // Clamp to valid range (bounds from settings_defaults)
    chunk_size =
      (std::max)(chunk_size,
                 settings_defaults::kMinDynamicChunkSize);  // NOLINT(readability-use-std-min-max)
                                                            // - Using (std::max) to prevent
                                                            // Windows macro expansion
    chunk_size =
      (std::min)(chunk_size,
                 settings_defaults::kMaxDynamicChunkSize);  // NOLINT(readability-use-std-min-max)
                                                            // - Using (std::min) to prevent
                                                            // Windows macro expansion
    settings.dynamicChunkSize = chunk_size;
  }
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip("Dynamic chunk size for the Hybrid strategy's second phase\n"
...
void RenderHybridInitialWorkPercent(AppSettings& settings) {
  if (int initial_percent =
      ...
      ImGui::SliderInt(
        ...
        settings_defaults::kMinHybridInitialWorkPercent,
        settings_defaults::kMaxHybridInitialWorkPercent,  // NOSONAR(cpp:S6004) - Variable
...
void RenderPerformanceSettings(AppSettings& settings) {
  if (ImGui::CollapsingHeader("Advanced performance tuning")) {
    ...
    ImGui::Separator();
    if (ImGui::Checkbox("Show the Metrics button in the UI", &settings.showMetricsButton)) {
      // No immediate side effects beyond updating working settings; persistence happens on Save.
    }
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip("When enabled, the Metrics button appears in the main toolbar.\n"
                        "Click it to open the Metrics window with performance and monitoring details.");
    }
...
void RenderRecrawlSummary(const AppSettings& settings) {
  ...
  ImGui::Spacing();
  ImGui::TextDisabled("Recrawl runs every %d minutes (skipped while a search is in progress).",
...
void RenderIndexConfiguration(AppSettings& settings,
...
void RenderTipsSection() {
  ...
  if (ImGui::CollapsingHeader("Tips")) {
    ImGui::TextWrapped("Settings are saved to a small JSON file next to the executable.\n"
                       ...
                       "Chunk size and hybrid initial work percent apply on the next search.\n"
...
struct SaveFeedbackState {
  bool* has_save_result;  // NOLINT(readability-identifier-naming) - output bundle for HandleSaveAction
  bool* last_save_success;  // NOLINT(readability-identifier-naming) - output bundle for HandleSaveAction
  std::string* last_save_message;  // NOLINT(readability-identifier-naming) - output bundle for HandleSaveAction
};
...
void HandleSaveAction(bool* p_open, AppSettings& settings, const AppSettings& working_settings,
  ...
  // Check if thread pool size changed relative to the live value before committing.
  // Compare against live `settings` (which reflects the state when the window opened),
  // not against disk — a fresh LoadSettings() call here reads stale data and may trigger
  // spurious pool resets if the disk copy is out of date.
  const int initial_thread_pool_size = settings.searchThreadPoolSize;
  const bool thread_pool_size_changed =
    (settings.searchThreadPoolSize != working_settings.searchThreadPoolSize);

  // The Settings window only edits a subset of AppSettings. Apply working copy while
  // preserving runtime fields (window size, search history) the UI does not manage.
  ApplyWorkingSettingsPreservingRuntime(settings, working_settings);

  const bool save_ok = (actions != nullptr) && actions->PersistSettings();
  ...
  if (save_ok) {
    ...
    *save_feedback.last_save_message = "Settings saved successfully.";
    ...
    if (thread_pool_size_changed) {
      ...
      LOG_INFO_BUILD("Thread pool size changed from " << initial_thread_pool_size
...
void SettingsWindow::Render(bool* p_open, AppSettings& settings, FileIndex& file_index,
  ...
  // Recenter on each open to avoid reopening far away when the main window moved significantly.
  // ImGuiCond_Appearing only applies on open/restore, so users can still drag the window afterward.
  CenterNextWindowInMainWindow(ImGuiCond_Appearing);
...

  {
    ...
    if (window_guard.ShowContent()) {
      RenderAppearanceSettings(working_settings);
      RenderPerformanceSettings(working_settings);
      RenderIndexConfiguration(working_settings, actions, folder_browser);
      RenderLogFileSection(glfw_window);

      RenderTipsSection();
      RenderSaveStatus(has_save_result, last_save_success, last_save_message);

      detail::RenderToolWindowFooterSeparator();
      detail::AlignToolWindowFooterButtons(2, LayoutConstants::kSecondaryButtonWidth);
  ...
  // Reset state when the window was closed via the ImGui title-bar X button.
  // The Close button and a successful Save already reset previous_open directly.
  // RenderFloatingWindows only calls Render() when *p_open is true, so the
  // early-return guard above never fires after an X-close; without this block,
  // previous_open stays true and the next open skips re-syncing working_settings
  // from live settings, causing stale (potentially Minimalistic) values to be saved.
  if (!*p_open) {
    previous_open = false;
    has_save_result = false;
    last_save_message.clear();
