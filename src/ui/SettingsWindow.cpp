...
void HandleSaveAction(bool* p_open, AppSettings& settings, const AppSettings& working_settings,
  ...
  // `settings` when the window opens; if the user keeps Settings open while searching, live history
  // diverges. A full assignment would clobber it — preserve searchHistory from the current live
  // settings.
  auto search_history_preserve = settings.searchHistory;

  // Apply working settings to live settings before persisting
  settings = working_settings;
  settings.searchHistory = std::move(search_history_preserve);
