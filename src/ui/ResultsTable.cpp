...
inline const char* CalculateDisplayPath(
  ...
  // Handle paths with no directory component (relative filename only)
...
[[nodiscard]] const char* GetRowFilenameDisplayCstr(const SearchResult& result,
                                                    std::array<char, 512>& filename_buffer,
...
void RenderResultsTableRow(int row, const RenderRowParams& params) {  // NOSONAR(cpp:S3776) - Cohesive row rendering; splitting would fragment locality
  ...
  ImGui::SetNextItemSelectionUserData(row);
  if (const char* display_cstr =
          GetRowFilenameDisplayCstr(params.result, filename_buffer, filename_display_buffer);
      ImGui::Selectable(display_cstr, is_selected, ImGuiSelectableFlags_AllowDoubleClick)) {
  ...
  if (state.searchActive || !state.result_pool_->Results().empty()) {
    ...
    if (constexpr int kResultColumnCount = 7;
      ...
      if (ImGui::IsItemHovered(ImGuiHoveredFlags_DelayNormal)) {
        ImGui::SetTooltip(
          "* Recursive non-directory file count; computed from cached index state and may lag recent file changes (refreshes at most every ~5 minutes while the index is changing).");
      }
