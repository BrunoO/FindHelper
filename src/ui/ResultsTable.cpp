...
// @p path is the full path (directory + filename) shown in the Full Path column.
inline const char* CalculateDisplayPath(
  ...
  // Thread-local buffer for null-terminated ImGui text (same pattern as filename/extension)
  static thread_local std::array<char, 512>
    path_buffer;  // Thread-local buffer for performance-critical UI rendering (hot path)

  if (path.size() >= path_buffer.size()) {
    // Very long path - truncate from the beginning (filename at the end stays visible)
    result.truncatedPathDisplay = ResultsTable::TruncatePathAtBeginning(path, max_width);
    result.truncatedPathColumnWidth = max_width;
    return result.truncatedPathDisplay.c_str();
  }

  std::memcpy(path_buffer.data(), path.data(), path.size());
  path_buffer.at(path.size()) = '\0';
  const char* path_cstr = path_buffer.data();

  if (const ImVec2 path_size = ImGui::CalcTextSize(path_cstr);
      path_size.x <= max_width) {  // Use init-statement pattern
    result.truncatedPathDisplay.assign(path);
    result.truncatedPathColumnWidth = max_width;
    return path_cstr;
  }
...
struct RenderRowParams {
  ...
  bool& drag_candidate_active;
  int& drag_candidate_row;
  ImVec2& drag_start_pos;
  bool& drag_started;
  int focus_row = -1;  // Row that should receive SetKeyboardFocusHere(0); -1 = none
  NameHighlightQuery name_highlight{};
  ColumnHighlightQuery path_highlight{};
  // Borrowed from Render() (backed by BuildExtensionHighlightQuery's cache); held by pointer
  // because ExtensionHighlightQuery owns a std::vector<std::string> and this struct is copied
  // per visible row — by-value copies allocated per row per frame.
  const ExtensionHighlightQuery* extension_highlight = nullptr;
};
...
struct RenderVisibleRowsParams {
  ...
  NameHighlightQuery name_highlight{};
  ColumnHighlightQuery path_highlight{};
  // Borrowed from Render() (backed by BuildExtensionHighlightQuery's cache); see RenderRowParams.
  const ExtensionHighlightQuery* extension_highlight = nullptr;
};
...
inline void RenderVisibleTableRows(const RenderVisibleRowsParams& params) {
  // The borrowed highlight query must be set by the caller (Render binds it to
  // BuildExtensionHighlightQuery's cache); a null here would fault per row below.
  assert(params.extension_highlight != nullptr);
  ...
  while (clipper.Step()) {
    for (int row = clipper.DisplayStart; row < clipper.DisplayEnd; ++row) {
      const auto& result = params.display_results[static_cast<size_t>(row)];  // NOLINT(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) NOSONAR(cpp:S1905) - clipper guarantees bounds; required cast
      ...
      const RenderRowParams row_params{result,
                                       ...
                                       params.drag_started,
                                       focus_row,
                                       params.name_highlight,
                                       params.path_highlight,
                                       params.extension_highlight,
  };
 ...
 *
 * Counts path segments in the directory path. Volume-root-only directories
 * ("C:\\", "/") and empty paths return 0 so drive-root files are not indented.
 * E.g., "docs/review/2026-02-14-v2" -> 3, "docs" -> 1, "C:\\" -> 0.
 */
[[nodiscard]] inline int GetPathDepth(const SearchResult& result) {
  ...
  if (dir_path.empty() || path_utils::IsVolumeRootPath(dir_path)) {
    return 0;
  }
  int count = 1;
...
[[nodiscard]] const char* GetRowFilenameCstr(const SearchResult& result,
  ...
  if (has_extension && write_len < max_len) {
    ...
    if (remaining > 0U) {
      ...
      if (ext_copy_len > 0U) {
        auto dest_it = filename_buffer.begin()  // NOLINT(llvm-qualified-auto,readability-qualified-auto) - MSVC C3535 if auto* used with iterator
...
// Filename column display buffer: 512-byte name + Font Awesome icon + space.
constexpr size_t kFilenameDisplayBufferSize = 560U;

template <size_t N>
size_t AppendTextToDisplayBuffer(std::array<char, N>& display_buffer, size_t write_len,
                                 size_t max_len, std::string_view text) {
  for (const char ch : text) {
    if (write_len >= max_len) {
      break;
    }
    display_buffer.at(write_len++) = ch;
  }
  return write_len;
}

// Display label for the Name column Selectable: icon + space + filename.
// Operations (open, delete, copy filename, drag-drop) must use GetRowFilenameCstr or
// result.fullPath — never this display string.
// When @p out_filename_byte_offset is non-null, it receives the byte offset of the
// filename within the returned display string (after icon + space).
[[nodiscard]] const char* GetRowFilenameDisplayCstr(
    const SearchResult& result, std::array<char, 512>& filename_buffer,
    std::array<char, kFilenameDisplayBufferSize>& display_buffer,
    size_t* out_filename_byte_offset = nullptr) {
  const char* const filename_cstr = GetRowFilenameCstr(result, filename_buffer);
  const char* const icon =
      result.isDirectory ? ICON_FA_FOLDER_OPEN : ICON_FA_NOTE_STICKY;

  size_t write_len = 0;
  const size_t max_len = display_buffer.size() - 1U;
  write_len = AppendTextToDisplayBuffer(display_buffer, write_len, max_len, icon);
  write_len = AppendTextToDisplayBuffer(display_buffer, write_len, max_len, " ");
  if (out_filename_byte_offset != nullptr) {
    *out_filename_byte_offset = write_len;
  }
  write_len = AppendTextToDisplayBuffer(display_buffer, write_len, max_len, filename_cstr);
  display_buffer.at(write_len) = '\0';
  return display_buffer.data();
}

// Double-click open guard: SearchResult.fullPath is a view into the result
// path pool, which auto-refresh/live ingest can replace between the two
// clicks of a double-click. Copying a dangling view faults inside the open
// call before any log line (silent close). Validate the range first; only
// member reads here, never a dereference of the view.
[[nodiscard]] inline bool IsLiveResultView(const GuiState& state,
                                           const SearchResult& result) {
  const std::string_view view = result.fullPath;
  if (view.empty()) {
    return true;  // Existing validation rejects empties; nothing to fault on.
  }
  const std::vector<char>& pool = state.result_pool_->Pool();
  if (view.size() > pool.size()) {
    return false;
  }
  const char* const begin = pool.data();
  return view.data() >= begin && view.data() + view.size() <= begin + pool.size();
}

// Logs double-click open diagnostics, then opens only when the view is live.
// Stale views are skipped with a warning instead of faulting.
inline void OpenResultPathWithStaleGuard(GuiState& state, const SearchResult& result,
                                         int row, bool open_parent) {
  const uint64_t batch = state.result_pool_->BatchNumber();
  LOG_INFO_BUILD("Double-click open: row=" << row << " batch=" << batch
                 << " view_size=" << result.fullPath.size()
                 << " pool_size=" << state.result_pool_->Pool().size());
  if (!IsLiveResultView(state, result)) {
    LOG_WARNING_BUILD("Double-click open skipped: stale fullPath view (row=" << row
                      << " batch=" << batch << ")");
    return;
  }
  if (open_parent) {
...
void RenderResultsTableRow(int row, const RenderRowParams& params) {  // NOSONAR(cpp:S3776) - Cohesive row rendering; splitting would fragment locality
  ...
  // Post-condition: if directory has a real size, the placeholder must not remain.
  ...
  static thread_local std::array<char, kFilenameDisplayBufferSize> filename_display_buffer;
  static thread_local std::vector<string_search::MatchSpan> match_span_scratch;
  ...
  const char* const display_cstr = GetRowFilenameDisplayCstr(
      params.result, filename_buffer, filename_display_buffer, &filename_byte_offset);
  constexpr ImGuiSelectableFlags k_name_selectable_flags = ImGuiSelectableFlags_AllowDoubleClick;
  if (params.name_highlight.IsActive() || params.path_highlight.IsActive()) {
    const std::string_view filename_view{display_cstr + filename_byte_offset};
    RenderHighlightedFilenameSelectable(HighlightedFilenameSelectableParams{
        display_cstr, filename_byte_offset, filename_view, &params.name_highlight,
        &params.path_highlight, is_selected, k_name_selectable_flags, &match_span_scratch,
        });
  } else if (ImGui::Selectable(display_cstr, is_selected, k_name_selectable_flags)) {
  ...
  if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0)) {
    OpenResultPathWithStaleGuard(params.state, params.result, row, /*open_parent=*/false);
  }
#ifdef _WIN32
  ...
  if (ImGui::IsItemHovered() && ImGui::IsMouseDown(ImGuiMouseButton_Right) &&
      !params.state.ui_visibility.context_menu_open) {
    // Debounce: prevent opening context menu if one was recently opened
    const auto now = std::chrono::steady_clock::now();
    const auto time_since_last =
      std::chrono::duration_cast<std::chrono::milliseconds>(now - params.state.ui_visibility.last_context_menu_time)
        .count();
    if (time_since_last >= kContextMenuDebounceMs) {
      ...
      params.state.ui_visibility.context_menu_open = true;
      params.state.ui_visibility.last_context_menu_time = now;
  ...
  if (!ImGui::IsMouseDown(ImGuiMouseButton_Right)) {
    params.state.ui_visibility.context_menu_open = false;
  }
#endif  // _WIN32
  ...
  // Column 3: Full Path (directory + filename; leading ellipsis when truncated)
  ...
  ResultsTable::RenderPathColumnWithEllipsis(PathColumnRenderParams{
      &params.result, is_selected, row, &params.state, params.native_window, params.glfw_window,
      current_path_column_width, &params.path_highlight, &match_span_scratch,
  });
  ...
  // Column 4: Extension (highlight when Extensions allowlist matches this row)
  ...
  ImGui::SetNextItemSelectionUserData(row);
  // Null-tolerant: the assert above documents the contract, but a null query
  // must degrade to the plain selectable in Release (assert compiled out),
  // never fault per row.
  const bool highlight_active = params.extension_highlight != nullptr &&
                                params.extension_highlight->IsActive();
  if (highlight_active) {
    RenderExtensionHighlightedSelectable(ext_cstr, *params.extension_highlight, is_selected, 0);
  } else if (ImGui::Selectable(ext_cstr, is_selected, 0)) {
    // Selection handled by ImGui multi-select.
  }
  ImGui::PopID();
...
std::vector<std::string_view> CollectEligibleDragPaths(
  ...
  for (const int row : state.selection.GetSelectedRows()) {
    ...
    const auto& result = display_results[static_cast<std::size_t>(row)];  // NOLINT(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) NOSONAR(cpp:S1905) - guarded by range check above; required cast
    ...
    if (!ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
      ...
      if (over_threshold && row_valid) {
        const auto& drag_result =
          display_results[static_cast<std::size_t>(drag_candidate_row)];  // NOLINT(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access) NOSONAR(cpp:S1905) - guarded by row_valid above; required cast
...
// feedback via state.export_workflow.notification / state.export_workflow.error_message.
void ExecuteMultiDelete(GuiState& state) {
  ...
  if (failed_paths.empty()) {
    state.export_workflow.notification =
      (deleted_count == 1) ? "Deleted 1 item"
                           : "Deleted " + std::to_string(deleted_count) + " items";
    state.export_workflow.error_message = "";
  } else {
    state.export_workflow.notification =
      (deleted_count > 0) ? "Deleted " + std::to_string(deleted_count) + "/" +
                              std::to_string(deleted_count + failed_count) + " items"
                          : "";
    state.export_workflow.error_message = "Failed to delete: " + failed_paths;
  }
  state.export_workflow.notification_time = std::chrono::steady_clock::now();
}
...
void HandleDeleteKeyAndPopup(GuiState& state) {  // NOSONAR(cpp:S3776) - Same rationale as above
  ...
  if (ImGui::BeginPopupModal("Confirm Delete", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
    if (const auto delete_count = state.selection.filesToDelete.size(); delete_count == 1) {
      ...
      ImGui::Text("Are you sure you want to delete %zu items?", delete_count);
...
void HandleBulkDeletePopup(GuiState& state, const FileIndex& file_index) {
  ...
  if (ImGui::BeginPopupModal("Confirm Bulk Delete", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    ImGui::Text("Are you sure you want to delete %zu marked files?", marked_count);
...
[[nodiscard]] const std::vector<SearchResult>& ResolveIncrementalDisplayResults(
    const std::vector<SearchResult>& base_results,
    IncrementalSearchState& incremental_search,
    size_t folder_stats_updated) {
  if (!incremental_search.IsFilterActive()) {
    return base_results;  // NOLINT(bugprone-return-const-ref-from-parameter) - base_results is caller-owned
  }
  if (folder_stats_updated > 0) {
    incremental_search.RefreshFilter(base_results);
  }
  return incremental_search.FilteredResults();
}
...
[[nodiscard]] ImGuiMultiSelectIO* BeginMultiSelectForTable(
  ...
  ImGuiMultiSelectFlags ms_flags =
      ImGuiMultiSelectFlags_ScopeRect |    // NOLINT(hicpp-signed-bitwise,bugprone-signed-bitwise) - ImGui flags bitmask
      ImGuiMultiSelectFlags_BoxSelect1d |  // NOLINT(hicpp-signed-bitwise,bugprone-signed-bitwise)
      ImGuiMultiSelectFlags_NoAutoSelect;  // NOLINT(hicpp-signed-bitwise,bugprone-signed-bitwise) - Down/Up/N/P handled manually; prevents SetKeyboardFocusHere from generating a conflicting auto-select SetRange
  if (!incremental_search.IsPromptVisible()) {
    ms_flags |= ImGuiMultiSelectFlags_ClearOnEscape;  // NOLINT(hicpp-signed-bitwise,bugprone-signed-bitwise)
  }
...
bool ResultsTable::RenderPathColumnWithEllipsis(const PathColumnRenderParams& params) {
  CTRACK_DEV_NAME("ResultsTable::RenderPathColumnWithEllipsis");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
  assert(params.result_ != nullptr);
  assert(params.state_ != nullptr);
  assert(params.path_highlight_ != nullptr);
  assert(params.span_scratch_ != nullptr);
  const SearchResult& result = *params.result_;
  const bool is_selected = params.is_selected_;
  const int row = params.row_;
  GLFWwindow* const glfw_window = params.glfw_window_;
  const float max_width = params.max_width_;
  const ColumnHighlightQuery& path_highlight = *params.path_highlight_;
  std::vector<string_search::MatchSpan>& span_scratch = *params.span_scratch_;
  ...
  // Full path including filename. Path-field highlight spans are found against this
  // visible string (including the trailing name when not clipped by leading ellipsis).
  ...
  const char* display_path_cstr = CalculateDisplayPath(
      const_cast<SearchResult&>(result), full_path, max_width);  // NOSONAR(cpp:S859) NOLINT(cppcoreguidelines-pro-type-const-cast) - caching optimization

  ImGui::SetNextItemSelectionUserData(row);
  // Selection handled by ImGui multi-select. Keep double-click → OpenParentFolder below.
  // Highlight against the visible display string so ellipsis truncation needs no index remapping.
  const bool selection_changed =
      path_highlight.IsActive()
          ? RenderHighlightedTextSelectable(std::string_view{display_path_cstr}, path_highlight,
  ...
  if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0)) {
    OpenResultPathWithStaleGuard(*params.state_, result, row, /*open_parent=*/true);
  }

// Right-click context menu (Windows only)
// Use same improved detection as filename column (see above for details)
#ifdef _WIN32
  GuiState& state = *params.state_;
  const NativeWindowHandle native_window = params.native_window_;
  if (ImGui::IsItemHovered() && ImGui::IsMouseDown(ImGuiMouseButton_Right) &&
      !state.ui_visibility.context_menu_open) {
    // Debounce: prevent opening context menu if one was recently opened
    const auto now = std::chrono::steady_clock::now();
    const auto time_since_last =
      std::chrono::duration_cast<std::chrono::milliseconds>(now - state.ui_visibility.last_context_menu_time)
        .count();
    if (time_since_last >= kContextMenuDebounceMs) {
      ...
      POINT pt;
      GetCursorPos(&pt);  // Get screen coordinates directly
      state.ui_visibility.context_menu_open = true;
      state.ui_visibility.last_context_menu_time = now;
      ShowContextMenu(native_window, result.fullPath, pt);
    }
  }
  // Reset context menu flag when right mouse button is released
  if (!ImGui::IsMouseDown(ImGuiMouseButton_Right)) {
    state.ui_visibility.context_menu_open = false;
  }
#endif  // _WIN32
...
void RenderMarkedActionsToolbar(GuiState& state,
...

  {
    ...
    ImGui::Text(ICON_FA_CHECK_DOUBLE " %zu items marked", marked_count);
  }
  ImGui::SameLine();
...
void RenderResultsTableHeaderArea(const IncrementalSearchState& incremental_search,
  ...
  if (incremental_search.IsPromptVisible()) {
    ...
    static const float kFullShortcutsWidth = ImGui::CalcTextSize(k_full_shortcuts_text).x;
    if (kFullShortcutsWidth <= available_width) {
      ImGui::TextUnformatted(k_full_shortcuts_text);
    } else {
      static const float kShortShortcutsWidth = ImGui::CalcTextSize(k_short_shortcuts_text).x;
      if (kShortShortcutsWidth <= available_width) {
  ...
  if (results_shortcuts_active) {
    ...
    static const float kTooltipFullWidth = ImGui::CalcTextSize(k_full_shortcuts_text_tooltip).x;
    if (kTooltipFullWidth > ImGui::GetContentRegionAvail().x &&
...
void ResultsTable::Render(
  ...
  CTRACK_DEV_NAME("ResultsTable::Render");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
  ...
  constexpr float drag_threshold = 5.0F;  // pixels - minimum movement to start drag
  ...
  state.ui_visibility.incremental_search_active = incremental_search.IsPromptVisible();

  // Show results table if search is active OR if we have results to display.
  if (state.search_pipeline.search_active || !state.result_pool_->Results().empty()) {
    // CRITICAL: Check for async sorting completion BEFORE filter/sort so display reflects sorted
    // data
    {
      CTRACK_DEV_NAME("ResultsTable::CheckAndCompleteAsyncSort");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
      search::SearchResultsService::CheckAndCompleteAsyncSort(state, file_index, thread_pool, show_path_hierarchy_indentation);
    }
    ...
    {
      CTRACK_DEV_NAME("ResultsTable::UpdateFilterCaches");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
    ...
    if (constexpr int kResultColumnCount = 7;
        ImGui::BeginTable("SearchResults", kResultColumnCount,
                          ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg |  // NOLINT(hicpp-signed-bitwise,bugprone-signed-bitwise) - ImGui flags bitmask
      ...
      if (ImGui::IsItemHovered(ImGuiHoveredFlags_DelayNormal)) {
        ImGui::SetTooltip(
          "* Recursive non-directory file count under each folder.\n"
          "Computed from cached index state; may lag recent file changes.\n"
          "Refreshes at most every ~5 minutes while the index is changing.");
      }

      // Detect when hierarchy mode is toggled so table sorting updates immediately
      static bool prev_show_path_hierarchy_indentation = show_path_hierarchy_indentation;
      if (ImGuiTableSortSpecs* const sort_specs = ImGui::TableGetSortSpecs();
          sort_specs != nullptr && show_path_hierarchy_indentation != prev_show_path_hierarchy_indentation) {
        sort_specs->SpecsDirty = true;
        prev_show_path_hierarchy_indentation = show_path_hierarchy_indentation;
      }
      // Folder stats handoff runs even during in-flight searches (auto-refresh keeps prior rows).
      // Sorting and displayed-total updates require a complete result set only.
      size_t folder_stats_updated = 0;
      {
        CTRACK_DEV_NAME("ResultsTable::FlushAggregatorFolderStats");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
        folder_stats_updated = FlushAggregatorFolderStats(state, aggregator);
      }

      // Handle sorting (only for complete results). Must run BEFORE GetDisplayResults:
      // HandleTableSorting may rebuild filter caches (filter_caches.time.results/filter_caches.size.results).
      // If we took display_results before this, that pointer would be invalidated after rebuild.
      if (state.search_pipeline.results_complete) {
        CTRACK_DEV_NAME("ResultsTable::HandleTableSorting");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
      ...
      const std::vector<SearchResult>& display_results =
          ResolveIncrementalDisplayResults(base_results, incremental_search, folder_stats_updated);
      ...
      const ColumnHighlightQuery name_highlight =
          BuildNameHighlightQuery(state, incremental_search);
      const ColumnHighlightQuery path_highlight =
          BuildPathHighlightQuery(state, incremental_search);
      // Cached on the input text (see BuildExtensionHighlightQuery); borrowed by the params
      // structs below, so no per-frame / per-row vector copies.
      const ExtensionHighlightQuery& extension_highlight = BuildExtensionHighlightQuery(state);
      ...
      {
        CTRACK_DEV_NAME("ResultsTable::RenderVisibleRows");  // NOLINT(misc-const-correctness) - macro creates a non-const handler object
        RenderVisibleTableRows(visible_rows_params);
      }

      // Update status bar flag: Placed outside resultsComplete guard so the flag clears
      // immediately when CancelPending() is called on a new search.
