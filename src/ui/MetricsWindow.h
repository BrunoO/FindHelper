...
public:
   ...
   *
   * Renders formatted text using printf-style formatting (wrapped to the window
   * width) and displays a tooltip on hover if provided. Public so that
   * implementation helpers in the .cpp (e.g. RenderMonitorProcessingStatistics)
   * can call it.
   *
   * @param tooltip Plain tooltip text on hover (no printf placeholders; drawn
   *                with TextUnformatted). Can be nullptr or empty.
   * @param fmt Printf-style format string for the metric line
...
private:
  ...
  static void RenderSearchPerformanceSection(const SearchWorker* search_worker);
