 ...
 * @param criteria Reference to search criteria (resets item type filter)
...
static void SetQuickFilter(const char* extensions, SearchInputField& extension_input,
  SearchInputField& filename_input, SearchCriteria& criteria,
  ...
  criteria.SetItemTypeFilter(ItemTypeFilter::All);
  mark_input_changed();
}
...
void FilterPanel::RenderQuickFilters(GuiState& state) {
  ...
  const char* active_filter_name = GetActiveQuickFilterName(state.searchCriteria.extension_input);
  ...
  RenderQuickFilterButton(
    ...
    [&state, &mark_input_changed] {  // NOSONAR(cpp:S3608) - Already explicit: captures state and
                                       // mark_input_changed
      SetQuickFilter("rtf;pdf;doc;docx;xls;xlsx;ppt;pptx;odt;ods;odp", state.searchCriteria.extension_input,
                     state.searchCriteria.filename_input, state.searchCriteria, mark_input_changed);
    });
  ImGui::SameLine();
  ...
  RenderQuickFilterButton(
    ...
    [&state, &mark_input_changed] {  // NOSONAR(cpp:S3608) - Already explicit: captures state and
                                       // mark_input_changed
      SetQuickFilter("exe;bat;cmd;ps1;msi;com", state.searchCriteria.extension_input, state.searchCriteria.filename_input,
                     state.searchCriteria, mark_input_changed);
    });
  ImGui::SameLine();
  ...
  RenderQuickFilterButton(
    ...
    [&state, &mark_input_changed] {  // NOSONAR(cpp:S3608) - Already explicit: captures state and
                                       // mark_input_changed
      SetQuickFilter("mp4;avi;mkv;mov;wmv;flv;webm;m4v", state.searchCriteria.extension_input, state.searchCriteria.filename_input,
                     state.searchCriteria, mark_input_changed);
    });
  ImGui::SameLine();
  ...
  RenderQuickFilterButton(
    ...
    [&state, &mark_input_changed] {  // NOSONAR(cpp:S3608) - Already explicit: captures state and
                                       // mark_input_changed
      SetQuickFilter("jpg;jpeg;png;gif;bmp;tif;tiff;svg;webp;heic", state.searchCriteria.extension_input,
                     state.searchCriteria.filename_input, state.searchCriteria, mark_input_changed);
    });
  ImGui::SameLine();
  ...
  if (ImGui::Button("Current User")) {
    ...
    state.searchCriteria.path_input.SetValue(user_home);
    mark_input_changed();
  }
  ...
  if (ImGui::Button("Desktop")) {
    ...
    state.searchCriteria.path_input.SetValue(desktop_path);
    mark_input_changed();
  }
  ...
  if (ImGui::Button("Downloads")) {
    ...
    state.searchCriteria.path_input.SetValue(downloads_path);
    mark_input_changed();
  }
  ...
  if (ImGui::Button("Recycle Bin")) {
    ...
    state.searchCriteria.path_input.SetValue(trash_path);
    mark_input_changed();
  }
...
void FilterPanel::RenderTimeQuickFilters(GuiState& state) {
  ...
  RenderQuickFilterButton("Today", state.searchCriteria.time_filter == TimeFilter::Today, [&state] {
    state.searchCriteria.time_filter = TimeFilter::Today;
  });  // NOSONAR(cpp:S3608) - Already explicit: captures state
  ImGui::SameLine();

  RenderQuickFilterButton("This Week", state.searchCriteria.time_filter == TimeFilter::ThisWeek, [&state] {
    state.searchCriteria.time_filter = TimeFilter::ThisWeek;
  });  // NOSONAR(cpp:S3608) - Already explicit: captures state
  ImGui::SameLine();

  RenderQuickFilterButton("This Month", state.searchCriteria.time_filter == TimeFilter::ThisMonth, [&state] {
    state.searchCriteria.time_filter = TimeFilter::ThisMonth;
  });  // NOSONAR(cpp:S3608) - Already explicit: captures state
  ImGui::SameLine();

  RenderQuickFilterButton("This Year", state.searchCriteria.time_filter == TimeFilter::ThisYear, [&state] {
    state.searchCriteria.time_filter = TimeFilter::ThisYear;
  });  // NOSONAR(cpp:S3608) - Already explicit: captures state
  ImGui::SameLine();

  RenderQuickFilterButton("Older", state.searchCriteria.time_filter == TimeFilter::Older, [&state] {
...
void FilterPanel::RenderSizeQuickFilters(GuiState& state) {
  ...
  RenderQuickFilterButton("Empty", state.searchCriteria.size_filter == SizeFilter::Empty,
                          [&state] { state.searchCriteria.size_filter = SizeFilter::Empty; });
  ImGui::SameLine();

  RenderQuickFilterButton("Tiny", state.searchCriteria.size_filter == SizeFilter::Tiny,
                          [&state] { state.searchCriteria.size_filter = SizeFilter::Tiny; });
  ImGui::SameLine();

  RenderQuickFilterButton("Small", state.searchCriteria.size_filter == SizeFilter::Small,
                          [&state] { state.searchCriteria.size_filter = SizeFilter::Small; });
  ImGui::SameLine();

  RenderQuickFilterButton("Medium", state.searchCriteria.size_filter == SizeFilter::Medium,
                          [&state] { state.searchCriteria.size_filter = SizeFilter::Medium; });
  ImGui::SameLine();

  RenderQuickFilterButton("Large", state.searchCriteria.size_filter == SizeFilter::Large,
                          [&state] { state.searchCriteria.size_filter = SizeFilter::Large; });
  ImGui::SameLine();

  RenderQuickFilterButton("Huge", state.searchCriteria.size_filter == SizeFilter::Huge,
                          [&state] { state.searchCriteria.size_filter = SizeFilter::Huge; });
  ImGui::SameLine();

  RenderQuickFilterButton("Massive", state.searchCriteria.size_filter == SizeFilter::Massive,
                          [&state] { state.searchCriteria.size_filter = SizeFilter::Massive; });
...
void FilterPanel::RenderFilterBadge(
  ...
  std::array<char, 256> label_buf{};
  if (const int written = std::snprintf(label_buf.data(), label_buf.size(), "%s ×", label);
      written < 0) {
    label_buf.front() = '\0';  // Encoding error: fall back to empty label
  }
  if (ImGui::SmallButton(label_buf.data())) {
    on_clear();
  }
...
void FilterPanel::RenderActiveFilterIndicators(GuiState& state) {
  ...
  const auto clear_and_update = [&mark_input_changed](
  ...
  const bool has_active_filters = IsExtensionFilterActive(state) || IsFilenameFilterActive(state) ||
                                  IsPathFilterActive(state) || (state.searchCriteria.item_type_filter != ItemTypeFilter::All) || state.searchCriteria.folders_only ||
                                  state.searchCriteria.time_filter != TimeFilter::None ||
                                  state.searchCriteria.size_filter != SizeFilter::None;
  ...
  if (has_active_filters) {
    ...
    if (IsExtensionFilterActive(state)) {
      RenderFilterBadge("Extensions", true, clear_and_update([&state] {
                          state.searchCriteria.extension_input.SetValue(kDefaultExtensions);
                        }));
    }
    ...
    if (IsFilenameFilterActive(state)) {
      RenderFilterBadge("Name", true,
                        clear_and_update([&state] { state.searchCriteria.filename_input.Clear(); }));
    }

    // Path filter badge
    if (IsPathFilterActive(state)) {
      RenderFilterBadge("Path", true, clear_and_update([&state] { state.searchCriteria.path_input.Clear(); }));
    }

    // Item type badge (Folders Only or Files Only)
    if (state.searchCriteria.FoldersOnly()) {
      RenderFilterBadge("Folders Only", true,
                        clear_and_update([&state] { state.searchCriteria.SetItemTypeFilter(ItemTypeFilter::All); }));
    } else if (state.searchCriteria.FilesOnly()) {
      RenderFilterBadge("Files Only", true,
                        clear_and_update([&state] { state.searchCriteria.SetItemTypeFilter(ItemTypeFilter::All); }));
    }
    ...
    if (state.searchCriteria.time_filter != TimeFilter::None) {
      ...
      const int filter_index = static_cast<int>(state.searchCriteria.time_filter) - 1;  // -1 because None is 0
      if (filter_index >= 0 && filter_index < static_cast<int>(time_filter_labels.size())) {
        ...
        RenderFilterBadge(label, true, [&state] { state.searchCriteria.time_filter = TimeFilter::None; });
      }
    }

    // Size filter badge
    if (state.searchCriteria.size_filter != SizeFilter::None) {
      const char* size_label = SizeFilterDisplayLabel(state.searchCriteria.size_filter);
      // SizeFilterDisplayLabel returns static const char*, no formatting needed
      RenderFilterBadge(size_label, true, [&state] { state.searchCriteria.size_filter = SizeFilter::None; });
    }
...
static bool IsInputFieldActive(const SearchInputField& field, const char* default_value = nullptr) {
  ...
  if (default_value != nullptr) {
    // string_view comparison: AsString() would heap-allocate per call (called per frame).
    return field.AsView() != default_value;
  }
  return true;
}

[[nodiscard]] bool FilterPanel::IsExtensionFilterActive(const GuiState& state) {
  return IsInputFieldActive(state.searchCriteria.extension_input, kDefaultExtensions);
}

[[nodiscard]] bool FilterPanel::IsFilenameFilterActive(const GuiState& state) {
  return IsInputFieldActive(state.searchCriteria.filename_input);
}

[[nodiscard]] bool FilterPanel::IsPathFilterActive(const GuiState& state) {
  return IsInputFieldActive(state.searchCriteria.path_input);
}
