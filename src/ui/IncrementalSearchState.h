...
/**
 * @brief Case-insensitive substring match (recomputes SIMD hints per call).
 *
 * Prefer the overload with precomputed hints in hot paths.
 */
bool IncrementalSearchMatches(std::string_view query, const SearchResult& result);
...
class IncrementalSearchState {
 public:
  ...
  /**
   * @brief Rebuild the filtered result set from updated base results (e.g. after async folder stats write).
   *
   * Preserves query and match index if possible.
   *
   * @param base_results The updated base display results.
   */
  void RefreshFilter(const std::vector<SearchResult>& base_results);
 ...
 private:
  ...
  std::string query_;
  // NOLINTNEXTLINE(readability-identifier-naming) - Private members use snake_case_ per project convention
  string_search::SimdSubstringHints query_hints_{};
  // NOLINTNEXTLINE(readability-identifier-naming) - Private members use snake_case_ per project convention
