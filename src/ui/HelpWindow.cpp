...
// Renders a single shortcut as an ImGui bullet: e.g. "Cmd+P - Pin/unpin current search"
...
void RenderHelpWindowContent(const size_t* memory_bytes_from_state, bool* p_open,
                             ...
                             std::string_view unknown_args_display) {
    ...
    if (ImGui::CollapsingHeader("About", ImGuiTreeNodeFlags_DefaultOpen)) {
      ...
      ImGui::Spacing();
      ImGui::TextColored(Theme::Colors::TextDim, "%s", GetAboutPlatformMonitoringLabel(is_monitoring_active));
      ImGui::Spacing();
      ImGui::Text("System");
      ImGui::Separator();
      ...
      if (startup_args_display.empty()) {
        ImGui::Text("Startup args: <none>");
      } else {
        ImGui::TextWrapped("Startup args: %.*s",
                           static_cast<int>(startup_args_display.size()),
                           startup_args_display.data());
      }
      if (!unknown_args_display.empty()) {
        ImGui::TextColored(Theme::Colors::Warning, "Unknown args (ignored): %.*s",
                           static_cast<int>(unknown_args_display.size()),
                           unknown_args_display.data());
      }
      ImGui::Spacing();
    }
    ...
    if (ImGui::CollapsingHeader("What's new")) {
      BulletWrapped(
          R"fhelp(2026-09-04: Auto-refresh and search handoff: when the result set is unchanged, the table skips redraw; new results land already sorted for most columns (hierarchy-aware), reducing flicker.)fhelp");
      BulletWrapped(
          R"fhelp(2026-09-02: Search as you type: previous results stay visible while a debounced search runs, instead of briefly clearing the table.)fhelp");
      BulletWrapped(
          R"fhelp(2026-08-24: Results table: Path-field matches that appear in the filename are also highlighted in the Name column (e.g. pp:**Monkey*pdf).)fhelp");
      BulletWrapped(
          R"fhelp(2026-08-21: Results table: match highlighting in Name and Full Path — matched characters are tinted so you can see why a row matched (substring, fz:, glob, rs:, pp:/PathPattern, and / Filter in results). Details in Search Help.)fhelp");
      BulletWrapped(
          R"fhelp(2026-07-24: Descendant Files* / folder Size: after index inserts or deletes, aggregated folder stats refresh at most every ~5 minutes (no wait for the index to stop changing). Values can lag during bulk file activity; the column header * marks this cached behavior.)fhelp");
      BulletWrapped(
          R"fhelp(2026-03-27: Results table: "Descendant Files*" column — recursive non-directory file count under each folder (enable via column picker; hidden by default). Shows "…" while computing, "–" for files. Header * indicates cached/snapshot semantics and may lag recent file changes (refreshes at most every ~5 minutes while the index is changing). Export CSV and Ctrl+Shift+X / Cmd+Shift+X clipboard copy include Descendant Files*.)fhelp");
      ...
      BulletWrapped("2026-08-27: Settings: UI Scale is a user preference; monitor DPI applies automatically. Auto resets scale to 1.0 and default font size.");
      ...
      BulletWrapped("2026-02-20: Help window is a normal window (Alt+Tab); Export CSV moved; Ctrl+E shortcut; settings in HOME/.FindHelper.");
    ...
    RenderShortcutBullet(FindShortcut(ShortcutAction::PinSearch));
...
void HelpWindow::Render(bool* p_open, const size_t* memory_bytes_from_state,
  ...
  detail::RenderToolWindow(GetAboutHelpWindowTitle(), p_open, Theme::Colors::WindowBg,
                           [memory_bytes_from_state, p_open, is_monitoring_active,
                            startup_args_display, unknown_args_display] {
    RenderHelpWindowContent(memory_bytes_from_state, p_open, is_monitoring_active,
                            startup_args_display, unknown_args_display);
  });
}
