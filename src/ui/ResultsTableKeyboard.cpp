...
void MarkAllInFolder(GuiState& state,
  ...
  file_index.ForEachEntryWithPath(
    [&state, folder_path, mark](uint64_t id, const FileEntry& /*entry*/, std::string_view path) {
      ...
      if (match) {
        if (mark) {
          state.selection.markedFileIds.insert(id);
        } else {
          state.selection.markedFileIds.erase(id);
        }
      }
      return true;
    });
...
void MarkInvertAll(GuiState& state, const std::vector<SearchResult>& display_results) {
  for (const auto& result : display_results) {
    ...
    if (const auto it = state.selection.markedFileIds.find(file_id);
        it != state.selection.markedFileIds.end()) {
      state.selection.markedFileIds.erase(it);
    } else {
      state.selection.markedFileIds.insert(file_id);
    }
  }
}
...
void MoveSelectionAndFocusInDirection(GuiState& state,
                                      ...
                                      int delta) {
  state.selection.EnsureSomeSelection(display_results);
  const int selected_row = state.selection.GetSelectedRow();
  const int new_row      = selected_row + delta;
  if (new_row < 0 || new_row >= static_cast<int>(display_results.size())) {
    return;
  }
  state.selection.SetSelectedRowAndFocus(new_row);
  state.selection.scrollToSelectedRow = true;
}
...
void ExtendSelectionInDirection(GuiState& state,
  ...
  const int anchor = state.selection.GetPrimarySelectedRow();
  state.selection.SetSelectionRange((std::min)(anchor, new_active), (std::max)(anchor, new_active),
                                    anchor, new_active);
  state.selection.scrollToSelectedRow = true;
}
...
void ApplyMarkToCurrentRow(GuiState& state,
  ...
  if (selected_res.isDirectory) {
    ...
    state.selection.markedFileIds.insert(selected_res.fileId);
  } else {
    state.selection.markedFileIds.erase(selected_res.fileId);
  }
}
...
void MarkToggleCurrentAndMove(GuiState& state,
  ...
  const bool currently_marked =
    state.selection.markedFileIds.find(selected_res.fileId) != state.selection.markedFileIds.end();
...
struct KeyHoldState {
  // NOLINTNEXTLINE(readability-identifier-naming) - local shortcut debounce state; readable field names
  bool handled = false;
  // NOLINTNEXTLINE(readability-identifier-naming) - local shortcut debounce state; readable field names
...
std::vector<int> GetMarkedRowIndices(const GuiState& state,
  ...
  for (int row = 0; row < row_count; ++row) {
    const auto it = state.selection.markedFileIds.find(
    ...
    if (it != state.selection.markedFileIds.end()) {
...
std::vector<int> GetUnmarkedSelectionRowIndices(const GuiState& state,
  ...
  std::vector<int> row_indices;
  const auto row_count = static_cast<int>(display_results.size());
  row_indices.reserve(state.selection.GetSelectedRows().size());
...
std::vector<int> BuildCsvRowIndexList(const GuiState& state,
                                      const std::vector<SearchResult>& display_results) {
  if (!state.selection.markedFileIds.empty()) {
...
struct CsvColumnVisibility {
  // NOLINTNEXTLINE(readability-identifier-naming) - CSV export option flags; names match column labels
  bool name = true;
  // NOLINTNEXTLINE(readability-identifier-naming) - CSV export option flags; names match column labels
  bool size = true;
  // NOLINTNEXTLINE(readability-identifier-naming) - CSV export option flags; names match column labels
...
void HandleResultsTableKeyboardShortcuts(  // NOSONAR(cpp:S3776) - Cohesive shortcut handling; splitting would fragment logic
  ...
  if (incremental_search.IsPromptVisible()) {
    ...
    if (query_changed) {
      int selected_row_for_incremental = state.selection.GetPrimarySelectedRow();
      incremental_search.UpdateQuery(new_query,
                                     base_results,
                                     selected_row_for_incremental,
                                     state.selection.scrollToSelectedRow);
      state.selection.SetSelectedRow(selected_row_for_incremental);
      state.selection.RequestFocusForRow(selected_row_for_incremental);
    }
    ...
    if (const int match_count = incremental_search.MatchCount(); match_count > 0) {
      if (KeyPressedOnce(ImGuiKey_DownArrow) || KeyPressedOnce(ImGuiKey_N)) {
        int selected_row_for_incremental = state.selection.GetPrimarySelectedRow();
        incremental_search.NavigateNext(selected_row_for_incremental, state.selection.scrollToSelectedRow);
        state.selection.SetSelectedRow(selected_row_for_incremental);
        state.selection.RequestFocusForRow(selected_row_for_incremental);
      } else if (KeyPressedOnce(ImGuiKey_UpArrow) ||
                 (KeyPressedOnce(ImGuiKey_P) && !IsPrimaryShortcutModifierDown(io))) {
        int selected_row_for_incremental = state.selection.GetPrimarySelectedRow();
        incremental_search.NavigatePrev(selected_row_for_incremental, state.selection.scrollToSelectedRow);
        state.selection.SetSelectedRow(selected_row_for_incremental);
        state.selection.RequestFocusForRow(selected_row_for_incremental);
      }
    }
    ...
    if (KeyPressedOnce(ImGuiKey_Enter)) {
      ...
      if (cancel_pressed) {
        int selected_row_for_incremental = state.selection.GetPrimarySelectedRow();
        incremental_search.Cancel(selected_row_for_incremental, state.selection.scrollToSelectedRow);
        state.selection.SetSelectedRow(selected_row_for_incremental);
        state.selection.RequestFocusForRow(selected_row_for_incremental);
      }
    }
  ...
  if (activate_search && !display_results.empty()) {
    incremental_search.Begin(display_results,
                             state.selection.GetPrimarySelectedRow(),
                             current_table_scroll_y,
                             state.result_pool_->BatchNumber());
  ...
  if (KeyPressedOnce(ImGuiKey_D) && shift && !state.selection.markedFileIds.empty()) {
    state.selection.showBulkDeletePopup = true;
  }

  const int primary_row = state.selection.GetPrimarySelectedRow(display_results);
  ...
  if (results_table_keyboard_active && KeyPressedOnce(ImGuiKey_Delete) && has_selection) {
    ...
    for (const int sel_row : state.selection.GetSelectedRows()) {
      if (sel_row >= 0 && sel_row < result_count) {
        state.selection.filesToDelete.emplace_back(
    ...
    if (!state.selection.filesToDelete.empty()) {
      state.selection.showDeletePopup = true;
      LOG_INFO_BUILD("Opening delete popup for " << state.selection.filesToDelete.size() << " item(s)");
    }
  }
  ...
  if (has_selection && IsPrimaryShortcutModifierDown(io) && !shift &&
    ...
    state.input_debounce.focus_filename_input = true;
  }
  ...
  if (IsPrimaryShortcutModifierDown(io) && shift && KeyPressedOnce(ImGuiKey_P) && has_selection) {
    if (PinToQuickAccess(native_window, selected_path)) {
      state.export_workflow.notification = "Pinned to Quick Access";
      state.export_workflow.error_message = "";
    } else {
      state.export_workflow.notification = "";
      state.export_workflow.error_message = "Failed to pin to Quick Access (see log for details)";
    }
    state.export_workflow.notification_time = std::chrono::steady_clock::now();
  }
#endif  // _WIN32
  ...
  HandleDebouncedMarkKey(ImGuiKey_M, m_state, time_now,
                         [&state, &file_index, &display_results, shift, has_selection] {
                           if (shift) {
                             for (const auto& result : display_results) {
                               state.selection.markedFileIds.insert(result.fileId);
                             }
                           } else if (has_selection) {
  ...
  // the focus rect in sync via state.selection.SetSelectedRowAndFocus + state.selection.RequestFocusForRow.
  ...
  // state.selection.RequestFocusForRow → SetKeyboardFocusHere fires as plain nav next frame →
  // apply_to_range_src=true → storage->RangeSrcItem drifts → anchor corrupted.
  // Shift+N/P: ExtendSelectionInDirection calls state.selection.SetSelectionRange without state.selection.RequestFocusForRow,
