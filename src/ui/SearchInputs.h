...
/**
 * @struct AISearchArgs
 * @brief Arguments for RenderAISearch
 *
 * Groups parameters to keep the function signature within the 7-param limit
 * (Sonar cpp:S107), same pattern as InputFieldOptions above. All fields have
 * sensible defaults matching the previous default arguments. show_settings /
 * show_metrics / metrics_available are currently unused by the body (reserved
 * for future toggles) — callers still pass them through.
 */
struct AISearchArgs {
 ...
 * - Name: Filter by name pattern (with Ctrl+F focus support)
   ...
   * @param args AI search arguments (window, actions, flags); api_key_set is
   * read fresh by the caller each frame and never cached here — the key can
   * change at runtime
   */
  static void RenderAISearch(GuiState &state, const AISearchArgs& args = {});
