...
static bool IsStatusBarBusy(const GuiState& state, const SearchWorker& search_worker) {
  return state.index_build.in_progress || search_worker.IsBusy() ||
         ...
         state.gemini.api_call_in_progress;
}
...
void StatusBar::Render(GuiState &state,
  ...
  // Cache memory text and its measured width since memory_bytes_ only updates every 10 seconds
  static size_t last_memory_bytes = 0;
  static std::string cached_memory_text;
  static float cached_memory_width = 0.0F;
  ...
  if (state.memory_bytes_ == last_memory_bytes && !cached_memory_text.empty()) {
    ...
    cached_memory_width = ImGui::CalcTextSize(memory_text.c_str()).x;
  }
  ...
  const float right_group_width = status_size.x + item_spacing + kSeparatorWidth +
                                  item_spacing + cached_memory_width;
...
static void RenderLeftGroup(const UsnMonitor *monitor,
  ...
  if (monitor != nullptr) {
    ...
    if (monitor->IsPopulatingIndex()) {
      ...
      const auto get_monitoring_status = [&monitor, &integrity_compromised]() -> std::pair<ImVec4, const char*> {
#ifdef _WIN32
...
static void RenderDisplayedCountAndSize(GuiState& state, const FileIndex& file_index) {
  ...
  const bool has_time_filter = state.searchCriteria.time_filter != TimeFilter::None;
  const bool has_size_filter = state.searchCriteria.size_filter != SizeFilter::None;
  if (has_time_filter || has_size_filter) {  // NOSONAR(cpp:S6004) - Both flags needed in condition and displayed_count
    ...
    const size_t displayed_count = has_size_filter ? state.filter_caches.size.count : state.filter_caches.time.count;
    ImGui::Text("Displayed: %zu", displayed_count);
    if (state.filter_caches.total_size.valid) {
      ImGui::SameLine();
      ImGui::TextDisabled("(file size: %s)", FormatMemory(state.filter_caches.total_size.bytes).c_str());
    }
    ImGui::SameLine();
...
static void RenderQueueAndSearchTime(const UsnMonitor* monitor, const SearchWorker& search_worker) {
  // Show during initial population, or whenever realtime backlog is elevated so
  // USN drain stalls (FPS collapse) are visible without opening Metrics.
  // Threshold matches usn_monitor_constants::kQueueStatusDisplayThreshold (Windows).
  constexpr size_t kQueueStatusDisplayThreshold = 100;
  const size_t queue_size =
      (monitor != nullptr) ? monitor->GetQueueSize() : 0;
  const bool has_queue_info =
      (monitor != nullptr) && queue_size > 0 &&
      (monitor->IsPopulatingIndex() || queue_size >= kQueueStatusDisplayThreshold);
...
static void RenderIndexBuildTiming(const GuiState& state) {
  if (!state.index_build.has_timing) {
    return;
  }
  ...
  } else if (state.index_build.last_duration_ms > 0U) {
    elapsed_ms = state.index_build.last_duration_ms;
  } else {
    return;
  }
...
static void RenderCenterGroup(GuiState& state,
  ...
  if (state.index_build.has_timing &&
      (state.index_build.in_progress || state.index_build.last_duration_ms > 0U)) {
    InlineSeparator();
...
static void RenderRightGroup(const GuiState &state, const SearchWorker &search_worker,
  ...
  if (const bool is_loading_attributes = state.async_sort_.IsLoading();
      state.index_build.in_progress || is_loading_attributes || state.computingFolderSizes ||
      state.gemini.api_call_in_progress || IsCloudFileAttributeLoadActive(state) ||
      IsDisplayedTotalSizeSummationPending(state)) {
    status_color = &Theme::Colors::Warning;
  } else if (state.index_build.failed) {
    ...
    if (!state.export_workflow.error_message.empty()) {
      status_color = &Theme::Colors::Error;
    } else if (!state.export_workflow.notification.empty()) {
