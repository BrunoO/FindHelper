...
void RenderWelcomeSectionTitle(std::string_view label, size_t count, bool with_separator = true) {
...
// Single-line text at cursor with ellipsis when wider than max_width (ImGui has no public
// TextEllipsis).
void TextEllipsisSingleLine(const char* text_begin, const char* text_end, float max_width) {
  ...
  ImGui::RenderTextEllipsis(window->DrawList, pos_min, pos_max, pos_max.x, text_begin, text_end,
                            nullptr);
  ImGui::Dummy(ImVec2(layout_w, line_h));
}
...
[[nodiscard]] bool RenderIconButtonWithTooltip(const char* icon_text, std::string_view tooltip) {
...
static constexpr std::array<ExampleSearch, 10> kExampleSearches = {
  // NOSONAR(cpp:S5945) - Using std::array instead of C-style array
  ExampleSearch{"*.cpp", "*.cpp", "", "", false, false, TimeFilter::None, SizeFilter::None,
                ExampleCategory::ByFilename},
  ExampleSearch{"Makefile", "Makefile", "", "", false, false, TimeFilter::None, SizeFilter::None,
                ExampleCategory::ByFilename},
  ExampleSearch{"^main", "^main", "", "", false, false, TimeFilter::None, SizeFilter::None,
                ExampleCategory::ByPattern},
  ExampleSearch{"*.h", "", "h", "", false, false, TimeFilter::None, SizeFilter::None,
                ExampleCategory::ByFileType},
  ExampleSearch{"*.py", "*.py", "", "", false, false, TimeFilter::None, SizeFilter::None,
                ExampleCategory::ByFileType},
  ExampleSearch{"*.json", "*.json", "", "", false, false, TimeFilter::None, SizeFilter::None,
                ExampleCategory::ByFileType},
  ExampleSearch{"Folders only", "", "", "", true, false, TimeFilter::None, SizeFilter::None,
                ExampleCategory::ByFilters},
  ExampleSearch{"Modified today", "", "", "", false, false, TimeFilter::Today, SizeFilter::None,
                ExampleCategory::ByFilters},
  ExampleSearch{"Empty files", "", "", "", false, false, TimeFilter::None, SizeFilter::Empty,
                ExampleCategory::ByFilters},
  ExampleSearch{"Large files (>10MB)", "", "", "", false, false, TimeFilter::None,
                SizeFilter::Large, ExampleCategory::ByFilters},
};

/**
 * @brief Apply an example search to GuiState (caller should trigger search to match recent-search
 * behavior)
 */
static void ApplyExampleSearch(const ExampleSearch& ex, GuiState& state) {
  ...
  state.gemini.description_input[0] =
    '\0';  // NOLINT(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access)
           // - Fixed-size buffer, index 0 is always valid
...
[[nodiscard]] static size_t CountExamplesInCategory(ExampleCategory cat) {
  size_t n = 0;
  for (const ExampleSearch& ex :
       kExampleSearches) {  // NOLINT(llvm-use-ranges) - C++17; std::ranges requires C++20
    if (ex.category == cat) {
      ++n;
    }
  }
...
static void RenderExamplesPanel(GuiState& state, UIActions* actions, float panel_width,
  ...
  ImGui::BeginChild("empty_state_examples", ImVec2(panel_width, panel_height),
                    ImGuiChildFlags_Borders);

  const ImGuiStyle& style = ImGui::GetStyle();
  RenderWelcomePanelTitle(FormatTitleWithCount("Explore Search Types", kExampleSearches.size()));

  const bool can_show_all = actions != nullptr && !is_index_building && indexed_file_count > 0U;
  if (!can_show_all) {
    ImGui::BeginDisabled();
  }
  ...
  for (size_t i = 0; i < kExampleSearches.size(); ++i) {
    ...
    const float button_width = ImGui::CalcTextSize(ex.label).x + (style.FramePadding.x * 2.0F);
...
static void ApplyHistoryRowPrimaryAction(const SearchHistoryEntry& entry,
                                         const HistoryRowActionsConfig& config, std::int64_t now_ms,
                                         AppSettings& settings, UIActions* actions) {
...
static void RenderHistoryRow(const SearchHistoryEntry* entry,
                             const HistoryRowActionsConfig& actions_config, GuiState& state,
  ...
  if (selected) {
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(
      row_screen_pos, ImVec2(row_screen_pos.x + k_accent_bar_w, row_screen_pos.y + row_height),
      accent_col);
  }
  ...
  const bool show_actions = hovered || state.history.pending_rename_id == entry->id ||
  ...
  const float button_w =
    MaxSmallIconButtonWidth({actions_config.primary_icon, ICON_FA_PEN, ICON_FA_TRASH});
  ...
  if (const bool in_action_zone = mouse_pos.x >= (row_screen_pos.x + avail_w - reserved_right_w);
  ...
  const float headline_max_w =
    ImGui::GetWindowContentRegionMax().x - reserved_right_w - ImGui::GetCursorPosX();
  TextEllipsisSingleLine(info.headline.c_str(), info.headline.c_str() + info.headline.size(),
                         headline_max_w);
  ...
  if (info.subtitle.empty()) {
    ...
    TextEllipsisSingleLine(info.subtitle.c_str(), info.subtitle.c_str() + info.subtitle.size(),
                           subtitle_max_w);
  }
  ImGui::PopFont();
  ImGui::PopStyleColor();
  ...
  if (show_actions) {
    ...
    if (RenderIconButtonWithTooltip(actions_config.primary_icon, actions_config.primary_tooltip)) {
    ...
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                          ImVec4(Theme::Colors::Error.x, Theme::Colors::Error.y,
                                 Theme::Colors::Error.z, kDeleteButtonHoverAlpha));
    ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                          ImVec4(Theme::Colors::Error.x, Theme::Colors::Error.y,
                                 Theme::Colors::Error.z, kDeleteButtonActiveAlpha));
...
static void RenderSearchHistoryPanel(GuiState& state, AppSettings& settings, UIActions* actions,
  ...
  if (!state.history.selected_id.empty()) {
    const bool still_present =
      std::any_of(  // NOLINT(llvm-use-ranges) - C++17; std::ranges requires C++20
        view.begin(), view.end(),
  ...
  const auto pinned_count = static_cast<size_t>(
    std::count_if(  // NOLINT(llvm-use-ranges) - C++17; std::ranges requires C++20
...
static void RenderSplitPanel(GuiState& state, AppSettings& settings, UIActions* actions,
                             float total_width, bool is_index_building, size_t indexed_file_count) {
  constexpr float k_left_ratio = 0.33F;  // Left panel 33%, right 67%
  const float left_width =
    (total_width * k_left_ratio) - (LayoutConstants::kWelcomePanelSplitPanelGutter * 0.5F);
  const float right_width =
    (total_width * (1.0F - k_left_ratio)) - (LayoutConstants::kWelcomePanelSplitPanelGutter * 0.5F);
...
void WelcomePanel::Render(GuiState& state, UIActions* actions, AppSettings& settings,
                          bool is_index_building) {
  ...
  const size_t indexed_file_count = (actions != nullptr) ? actions->GetIndexedFileCount() : 0;
  if (indexed_file_count == 0) {
    ...
    const char* instructions =
      "No files indexed yet. Go to Settings > Index Configuration\nto select a folder to index.";
  ...
  RenderSplitPanel(state, settings, actions, window_width, is_index_building, indexed_file_count);
}
