  ...
  void Start(IndexBuildState& state) override {
    ...
    GetWorkerThread() = std::thread([this]() {
      ...
      try {
        ...
        state->errors.store(metrics.errors.errors_encountered);
        ...
        while (!state->cancel_requested.load()) {
          ...
          if (!monitor_->IsPopulatingIndex()) {
            if (monitor_->InitialPopulationFailed()) {
              ...
              // Wake/join processor (and reader if still running). Population failure
              // runs after Start() already returned true, so nobody else Stop()s.
              monitor_->Stop();
            } else {
