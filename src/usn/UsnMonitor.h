...
// USN reason mask for FSCTL_READ_USN_JOURNAL. Must include USN_REASON_CLOSE when
// ReturnOnlyOnClose is nonzero (MSDN READ_USN_JOURNAL_DATA). CLOSE alone would
// also match no-op close records; callers filter with kActionReasons so only
// closes that carried a real create/delete/rename/data change are applied.
...
// Action bits only (excludes CLOSE). Used to skip close-only journal records.
constexpr DWORD kActionReasons = kInterestingReasons & ~USN_REASON_CLOSE;
