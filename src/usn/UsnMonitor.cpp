...
void ProcessUsnRecordReasons(const UsnRecordContext& record_context,
                             UsnProcessingState& processing_state) {
  if (record_context.record->Reason & USN_REASON_FILE_CREATE) {
  ...
  if (record_context.record->Reason &
    ...
    if (!processing_state.file_index.UpdateSize(record_context.file_ref_num)) {
      // UpdateSize failing means the entry is absent from the index — most
      // likely a race where a DELETE record in the same buffer was processed
      // first. File size is a display attribute only; this does not affect
      // path correctness or the user's ability to find files. Do not set the
      // integrity latch; record for diagnostics only.
      LOG_ERROR_BUILD("USN: UpdateSize failed for ref=" << record_context.file_ref_num
                      << " (entry absent; likely concurrent delete — not a stale-index condition).");
    }
...
void HandleMoveAndOptionalRename(uint64_t file_ref_num, uint64_t parent_ref_num,
  ...
  if (!file_index.Move(file_ref_num, parent_ref_num)) {
    // entry_exists was true moments ago; Move failing means the entry was
    // concurrently removed (e.g. a DELETE in the same buffer). If this fires
    // for any other reason it indicates genuine index divergence.
    integrity_compromised.store(true);
    LOG_ERROR_BUILD("USN: Move failed for ref=" << file_ref_num
  ...
  const std::string current_name_str = (last_sep != std::string_view::npos)
...
void HandleFileRename(const UsnRecordContext& record_context,
  ...
  if (current_parent != record_context.parent_ref_num) {
...
void UsnMonitor::HandleReadJournalError(DWORD err, size_t& consecutive_errors,
  ...
  if (err == ERROR_JOURNAL_ENTRY_DELETED) {
    ...
    // Journal wrapped: events between the last read USN and the new journal start
    // are permanently lost. The in-memory index may now diverge from the volume.
    // Set the integrity latch and stop the reader — the only recovery is an app restart.
    index_integrity_compromised_.store(true);
    LOG_WARNING("USN journal wrapped; updates were lost. Index may be stale. Stopping reader — restart the application.");
    monitoring_active_.store(false);
    should_exit = true;
    return;
  }
