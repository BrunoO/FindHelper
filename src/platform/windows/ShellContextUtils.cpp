...
// Appends one UTF-8 argument to a Windows command-line string, quoting per
// MSVCRT parsing rules when it contains spaces, tabs, quotes, or is empty.
void AppendQuotedArg(std::wstring& out, const char* arg_utf8) {
  const std::wstring warg = Utf8ToWide(arg_utf8);
  bool needs_quotes = warg.empty();
  for (const wchar_t c : warg) {
    if (c == L' ' || c == L'\t' || c == L'"') {
      needs_quotes = true;
      break;
    }
...
// Forwards the original arguments (excluding argv[0]) so the elevated
// process sees the same options. Quote per MSVCRT command-line rules so
// values containing spaces survive as single arguments.
// Extracted from RestartAsAdmin to keep nesting within limits (S134).
void AppendForwardedArgs(std::wstring& parameters, int argc, char* const* argv) {
  if (argv == nullptr) {
    return;
  }
  for (int i = 1; i < argc; ++i) {
    ...
    AppendQuotedArg(parameters, argv[i]);
  }
}
...
bool RestartAsAdmin(HWND hwnd, int argc, char* const* argv) {
  ...
  if (GetModuleFileNameW(nullptr, sz_path, ARRAYSIZE(sz_path))) {
    ...
    AppendForwardedArgs(parameters, argc, argv);
...
bool PromptForElevationAndRestart(HWND hwnd, int argc, char* const* argv) {
  ...
  return RestartAsAdmin(hwnd, argc, argv);
}
