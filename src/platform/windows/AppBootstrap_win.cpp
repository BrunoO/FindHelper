...
void StartUsnMonitor(const CommandLineArgs& cmd_args, const AppSettings& app_settings,
  ...
  if (!cmd_args.crawl_folder.empty()) {
    return;  // Explicit --crawl-folder opts out of USN for this session
  }
...
AppBootstrapResult HandleInitializationException(std::string_view error_message,
  ...
  AppBootstrapCommon::LogInitializationFailureAndCleanup(
      error_message, result, [](AppBootstrapResult& r) { CleanupOnException(r); });
...
AppBootstrapResult Initialize(const CommandLineArgs& cmd_args, FileIndex& file_index,
  ...
  try {
    ...
    return HandleInitializationException(AppBootstrapCommon::BuildSystemErrorMessage(e), result);
  } catch (const std::bad_alloc& e) {
    return HandleInitializationException(AppBootstrapCommon::BuildBadAllocErrorMessage(e), result);
  } catch (const std::invalid_argument& e) {
    return HandleInitializationException(AppBootstrapCommon::BuildInvalidArgumentErrorMessage(e),
                                         result);
  } catch (const std::logic_error& e) {  // NOSONAR(cpp:S1181) - Part of exception hierarchy;
                                         // invalid_argument already caught above
    return HandleInitializationException(AppBootstrapCommon::BuildLogicErrorMessage(e), result);
  } catch (const std::runtime_error& e) {  // NOSONAR(cpp:S1181) - Part of exception hierarchy;
                                           // system_error already caught above
    return HandleInitializationException(AppBootstrapCommon::BuildRuntimeErrorMessage(e), result);
  } catch (const std::exception& e) {  // NOSONAR(cpp:S1181) - Safety net after specific types
    return HandleInitializationException(AppBootstrapCommon::BuildExceptionErrorMessage(e),
                                         result);
