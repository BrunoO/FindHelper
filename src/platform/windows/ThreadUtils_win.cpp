...
void SetWorkerThreadPriorityBelowNormal() {
  if (SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_BELOW_NORMAL) == 0) {
    LOG_WARNING_BUILD("SetWorkerThreadPriorityBelowNormal: SetThreadPriority failed; "
                      "lastError=" << GetLastError());
  } else {
    LOG_DEBUG_BUILD("SetWorkerThreadPriorityBelowNormal: worker thread priority set to BELOW_NORMAL");
  }
}
