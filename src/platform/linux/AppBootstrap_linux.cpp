...
namespace {
  ...
  void HandleInitializationException(std::string_view error_message,
                                     AppBootstrapResultLinux& result) {
    AppBootstrapCommon::LogInitializationFailureAndCleanup(
        error_message, result, [](AppBootstrapResultLinux& r) { CleanupOnException(r); });
  }
...
AppBootstrapResultLinux Initialize(const CommandLineArgs &cmd_args,
  ...
  AppBootstrapResultLinux result = {};  // Value-initialized: suppresses static-analyzer CWE-457/S5274 use-of-uninitialized claims on early-return paths
