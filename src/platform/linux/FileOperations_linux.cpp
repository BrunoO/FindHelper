...
namespace internal {
  ...
  // Drain the stderr pipe to EOF into a bounded buffer (truncated beyond
  // kMaxCapturedStderrLen). Must run BEFORE waitpid: if the child writes more
  // than the pipe capacity (64 KiB on Linux) it blocks until the parent reads;
  // reading after waitpid would therefore deadlock. Draining to EOF (the
  // child's exit closes its stderr copy) also guarantees waitpid never blocks
  // on a pipe-backed child.
  constexpr size_t kMaxCapturedStderr = 255;  // kMaxCapturedStderr chars + '\0'
  [[nodiscard]] std::array<char, kMaxCapturedStderr + 1> DrainStderrPipe(int read_fd) {
    std::array<char, kMaxCapturedStderr + 1> buffer = {};
    size_t stderr_len = 0;
    bool pipe_exhausted = false;
    while (!pipe_exhausted) {
      std::array<char, 256> scratch = {};
      const ssize_t bytes_read = read(read_fd, scratch.data(), scratch.size());
      if (bytes_read < 0) {
        if (errno == EINTR) {
          continue;
        }
        pipe_exhausted = true;  // unrecoverable read error
      }
      if (bytes_read == 0) {
        pipe_exhausted = true;  // EOF: child closed its stderr / exited
      }
      if (pipe_exhausted) {
        break;
      }
      const auto wanted = static_cast<size_t>(bytes_read);
      const size_t capacity_remaining = kMaxCapturedStderr - stderr_len;
      const size_t to_store = (std::min)(wanted, capacity_remaining);
      std::memcpy(buffer.data() + stderr_len, scratch.data(), to_store);
      stderr_len += to_store;
    }
    close(read_fd);
    buffer.at(stderr_len) = '\0';
    return buffer;
  }

  // Wait for child to complete (retry on signal interruption). Returns the
  // waitpid status, or nullopt if waitpid itself failed.
  [[nodiscard]] std::optional<int> WaitForChildExitStatus(pid_t pid) {
    int status = 0;
    do {
      if (const pid_t waited_pid = waitpid(pid, &status, 0); waited_pid == pid) {
        return status;
      }
    } while (errno == EINTR);
    return std::nullopt;
  ...
  // Log the child failure using captured stderr when available (truncates the
  // newline inside the caller's buffer).
  void LogChildFailure(const char* program, std::array<char, kMaxCapturedStderr + 1>& stderr_text,
                       int status) {
    if (stderr_text[0] != '\0') {
      // Truncate at first newline for cleaner log message
      if (char* newline = std::strchr(stderr_text.data(), '\n'); newline != nullptr) {
        *newline = '\0';
      }
      LOG_ERROR(std::string(program) + " failed: " + stderr_text.data());
    } else if (WIFSIGNALED(status)) {
      LOG_ERROR(std::string(program) + " killed by signal "
                + std::to_string(WTERMSIG(status)));
    } else if (WIFEXITED(status)) {
      LOG_ERROR(std::string(program) + " failed with exit code " + std::to_string(WEXITSTATUS(status)));
    } else {
      LOG_ERROR(std::string(program) + " failed with abnormal termination status");
    }
  }
  ...
  bool ExecuteCommand(const char* program, const char* arg, const char* path) {
    ...
    const std::optional<int> exit_status = WaitForChildExitStatus(pid);
    if (!exit_status.has_value()) {
      LOG_ERROR(std::string("Failed to wait for ") + program);
      return false;
    }

    std::array<char, kMaxCapturedStderr + 1> stderr_text =
        DrainStderrPipe(stderr_pipe[0]);

    // Command succeeded?
    if (WIFEXITED(*exit_status) && WEXITSTATUS(*exit_status) == 0) {
      return true;
    }
    LogChildFailure(program, stderr_text, *exit_status);
    return false;
  }
} // namespace internal
