...
enum class ShortcutAction : uint8_t {
  ...
  PinSearch,          // Ctrl/Cmd+P:       pin/unpin current search in Search History
};
...
inline constexpr std::array<ShortcutDef, 6> kShortcuts{{
  ...
  {ShortcutAction::PinSearch,         ImGuiKey_P,      true,  false, ShortcutScope::Global, ShortcutPlatform::All, "P",       "Pin/unpin current search"},
...
inline bool IsTriggered(const ShortcutDef& def, const ImGuiIO& io) {
  ...
  return ImGui::IsKeyPressed(def.key, false);
}
