...
struct SearchCriteria {
  ...
  ItemTypeFilter item_type_filter = ItemTypeFilter::All; // Filter by filesystem item type (All, FilesOnly, FoldersOnly)
  bool folders_only = false;   // Match folders instead of files then filter out files (kept for backward compatibility)
  ...
  void SetItemTypeFilter(ItemTypeFilter filter) noexcept {
    item_type_filter = filter;
    folders_only = (filter == ItemTypeFilter::FoldersOnly);
  }

  [[nodiscard]] ItemTypeFilter GetEffectiveItemTypeFilter() const noexcept {
    return EffectiveItemTypeFilter(item_type_filter, folders_only);
  }

  [[nodiscard]] bool FoldersOnly() const noexcept {
    return GetEffectiveItemTypeFilter() == ItemTypeFilter::FoldersOnly;
  }

  [[nodiscard]] bool FilesOnly() const noexcept {
    return GetEffectiveItemTypeFilter() == ItemTypeFilter::FilesOnly;
  }
  ...
  // item_type_filter, folders_only, case_sensitive). Pure: reads only this struct's fields.
  [[nodiscard]] SearchParams BuildParams() const {
    ...
    params.itemTypeFilter = GetEffectiveItemTypeFilter();
