...
class GuiState {  // NOSONAR(cpp:S1820) - GuiState aggregates all UI state (62 fields): splitting would require passing multiple objects, reducing encapsulation and increasing complexity
 public:
  ...
  // Baseline index size for FolderSizeAggregator cache invalidation. Separate from
  // lastIndexSize (auto-refresh only) so Descendant Files* / folder Size refresh even when
  // auto-refresh is off or a manual re-search reuses CancelPending()'s preserved cache.
  // NOLINTNEXTLINE(readability-identifier-naming) - POD-like struct for UI state, camelCase is intentional
  size_t lastFolderAggregatorIndexSize = 0;
  // Set when index size diverges from lastFolderAggregatorIndexSize; cleared by Reset.
  // NOLINTNEXTLINE(readability-identifier-naming) - POD-like struct for UI state, camelCase is intentional
  bool folderAggregatorCacheStale = false;
  // Latest observed index size while the aggregator cache may be stale (applied on Reset).
  // NOLINTNEXTLINE(readability-identifier-naming) - POD-like struct for UI state, camelCase is intentional
  size_t pendingFolderAggregatorIndexSize = 0;
  // NOLINTNEXTLINE(readability-identifier-naming) - POD-like struct for UI state, camelCase is intentional
  bool folderAggregatorIndexSizeInitialized = false;
  // Last FolderSizeAggregator::Reset() time. Caps recompute to at most once per
  // kFolderAggregatorMinRefreshInterval while the cache is stale (no settle wait).
  // NOLINTNEXTLINE(readability-identifier-naming) - POD-like struct for UI state, camelCase is intentional
  std::chrono::steady_clock::time_point lastFolderAggregatorResetTime;
  ...
  bool deferFilterCacheRebuild =
    false;  // One-frame defer when active filters need unloaded attrs (see ShouldDeferFilterCacheRebuild)
