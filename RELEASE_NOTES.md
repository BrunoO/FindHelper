...
### Performance (Windows indexing groundwork — sub-2-second target)

- **Raw MFT bulk streaming:** Initial population now streams the MFT directly in 4 MB unbuffered chunks (double-buffered I/O overlapping reads with parsing) instead of issuing one `FSCTL_GET_NTFS_FILE_RECORD` per file. Records are parsed in place (idempotent USA fixup, fast-ASCII names, `$DATA`-exclusive sizing) and staged with size/modtime inline — per-file metadata reads are retired on this path. A USN handoff replays journal events that land mid-scan so nothing is missed.
- **Single-pass path indexing (Phase 1B):** `RecomputeAllPaths` now populates the path→id map inline during the topological build instead of re-hashing all entries a second time afterwards (that pass alone cost ~0.7 s Release on ~807k files). Measured Stage 4 baseline: **1.85 s** total recompute → **~1.15 s** projected; baselines recorded in `internal-docs/benchmarks/2026-09-24_STAGE4_RECOMPUTE_BASELINE.md` with per-phase telemetry in the logs.
- **Modular pipeline seams:** Each stage (extents, chunk reading, record parsing, ingest) sits behind a narrow injectable interface with scripted fakes, so future algorithms (SIMD parsing, folder-table ingest) swap in without touching neighbors.

### Fixed

- **Windows indexing / lock discipline:** The indexed-entry count is now logged after the index lock is released instead of inside the critical section.

### Changed

- **Removed:** Obsolete `MftMetadataReader` and dead enumeration code; DOS 8.3 alias names are used only when no Win32/POSIX name exists.
- **CI:** A versioned publish now triggers a single build — the tag run publishes both the versioned release and `latest`, and the duplicate branch run skips.
- **Diagnostics:** New `scripts/parse_stage4_benchmark_log.py` analyzes Stage 4 timing logs (including `--update` of the baseline) and now searches `%TEMP%` / `FindHelper_*.log` patterns for the latest runs.

### Documentation

- **DDD resync:** Glossary and domain map updated for the pipeline restructure (new `PipelineSeams` term, retired parser names repointed, corrected DOS-alias semantics).
- **Specs (pre-implementation):** Sub-2-second indexing plan (`specs/2026-09-24_Sub2s_MFT_Indexing_Plan_Spec.md`) and `FILE_ATTRIBUTE_OFFLINE` reading/usage plan (`specs/2026-09-25_OfflineFileAttribute_Plan_Spec.md`).

## September 24, 2026

### Fixed

- **Results table / silent close on double-click (Debug):** Double-clicking a file or folder to open it could close the app with no error in the logs. WinDbg showed an assert abort (`AssertSelectionInvariant` via `AddSelectionRange`), not a bad path: ImGui's range anchor survives across frames, so after a results refresh it could name a row outside the new selection. `AddSelectionRange` now falls back to the primary row instead of asserting on that external input. Double-click open is additionally guarded against stale result views.
- **CI:** The build workflow now runs automatically on pushes to `main` (and version tags), scoped to the public FindHelper repo only.

## September 18, 2026

### Documentation

- **DDD / ubiquitous language:** `docs/design/USN_MFT_UBIQUITOUS_LANGUAGE.md` and
  `internal-docs/analysis/2026-09-18_USN_MFT_DDD_DOMAIN_MAP.md` are now synced
  with the landed refactors (`NtfsFileReference`, `JournalCursor`, `UsnRecord`,
  `ReasonSet`, `VolumeState`, `IndexDomainEvents`). USN journal cursor checkpoint
  persistence is explicitly documented as rejected by design; restarts re-enumerate
  from FRN 0.

### Code quality

- **Volume-state aggregate:** `VolumeState` is now the single aggregate root per
  USN buffer; all per-buffer mutations route through domain verbs
  (`ApplyFileCreated`, `ApplyFileDeleted`, `ApplyFileRenamed`, `ApplyFileMoved`,
  etc.) instead of calling `FileIndex::*Locked` directly. This eliminates
  per-record aggregate recreation and makes the per-buffer atomicity invariant a
  type-level contract.
- **Domain events:** Integrity-cause events (`JournalWrapped`,
  `JournalIdChanged`, `JournalLost`, `QueueBuffersDropped`, `CorruptBufferTail`,
  `ConsecutiveErrorsExceeded`, `RenameDivergence`) are now published alongside
  every set-once integrity latch, giving forensics and tests the specific cause
  instead of a bare "compromised" flag.

### Changed

- **Build:** Removed the `usn_buffer_pipeline_benchmark` target; measured results
  and decisions are preserved in
  `internal-docs/analysis/2026-09-18_USN_BUFFER_PIPELINE_BENCHMARK_RESULTS.md`.

## September 14, 2026

### Fixed

- **Windows indexing / index drift:** Files whose `CREATE` event was processed before their parent directory (close-time journal reads routinely deliver child records first, e.g. hex-named app staging files) were stored with a bare filename and no `C:\` root — permanently, since live monitoring never re-resolved paths. Such entries are now healed with their full path as soon as the parent directory arrives (including nested chains), so every indexed entry keeps a volume-rooted path.
- **Windows monitoring / filtered directories:** Files created inside system-filtered locations (`$Recycle.Bin`, `$Extend` subtrees) before the parent was recognized as filtered no longer linger in the index; they are evicted when the filtered directory is recorded.
- **Windows indexing / rename-move:** Folder rename/move now updates only true descendants instead of every path sharing a name prefix, so same-named folders can no longer rewrite each other's subtrees.
- **Windows monitoring / never-arriving parents:** Placeholders whose parent never comes (filtered, dropped, or deleted before its `CREATE` was consumed) no longer linger visibly: pending entries are timestamped, the **Metrics** window shows live pending count / oldest age / stale-evicted totals, and a periodic sweep evicts entries waiting longer than **5 minutes** (young entries are healthy — close-time parents arrive milliseconds later).
- **Auto-refresh:** Now triggers on any index change instead of file-count changes only, so healed paths and renames refresh visible results too; a typed query with (still) empty results also arms refresh, so rare or transient files appear without a manual re-search. Also fixed: single-entry indexes silently returned no results (parallel scheduler rounded their only chunk to empty).
...
- **Case-insensitive search:** Removed a redundant ASCII validation pass that re-scanned every filename before matching; matchers now validate the pattern once at creation. Same results, less per-file work.
- **Glob patterns (`*…*`, `prefix*`, `*suffix`):** Replaced the recursive glob matcher with a linear-time matcher plus shape fast paths (contains / prefix / suffix), so patterns like `*nomatch*` skip the general engine entirely. Windows test runs confirm the improvement.
- **Parallel result assembly:** Workers now emit fixed-size result batches straight through futures, merge, and path-pool copy — eliminating per-file vector growth and a second conversion pass over every hit. Result counts are bit-identical (verified by differential tests against the previous pipeline).
- **Benchmarks:** `search_benchmark` now asserts per-case timing parity-or-better against a sequential reference, and ASan differential tests (ASCII vs non-ASCII corpora) guard the SIMD dispatch gates.

### Code quality

- Shared match-rate heuristic between chunking and buffer pre-reserve; merge-size assertion guards the batch protocol; remaining Sonar issues closed (view params, structured bindings).
...
- **Recent Drive Activity window (Windows):** A new tool window (**Recent Drive Activity (USN Journal)**) displaying a live feed of real-time filesystem change events captured by the NTFS USN Journal monitor.
  - **Activity feed:** Ring buffer tracking recent filesystem operations (Created, Deleted, Renamed, and Modified).
  - **Filtering & Search:** Toggle filters by event type and search paths by substring.
  - **Feed controls:** Pause/Resume to freeze the stream for inspection, and Clear to empty the feed.
  - **Context actions:** Right-click entries to copy the file name, copy full path, or open containing folder in Windows Explorer.
  - **Access:** Opened via the **Activity** toolbar button next to Metrics.
...
- **Settings file name:** Preferences are stored as `findhelper_settings.json` under `~/.FindHelper/` (was `settings.json`).
- **Windows monitoring:** Default USN journal queue cap raised from 10000 to **20000** buffers (~1.25 GB), so filesystem bursts absorb more before reader backpressure.

### Fixed

- **Windows monitoring / quit hang:** Closing the app with a full USN queue no longer waits to apply thousands of pending journal buffers. `Stop()` discards the backlog so “Stopping…” returns promptly.
- **Windows monitoring / delete-rename storms:** Each USN buffer now applies create/delete/rename/size-invalidate under one `FileIndex` exclusive lock instead of one lock per event, so the queue can drain faster and search is less starved on Windows SRWLOCK.
...
- **Full Path column:** Shows the complete path again (directory + filename). Leading ellipsis still keeps the trailing name visible when the column is narrow. Path-field and `/` filter match highlighting now covers the filename portion of this column as well as the directory.
- **Extension column match highlighting:** When the Extensions filter is active (`txt;pdf;…`), matching Extension cells are tinted like Name/Path matches so you can see why the row passed the allowlist. Name/Path/`/` queries do not paint the Extension column (those hits already appear in Name).
...
- **Match highlighting:** Path-field queries also highlight matching text in the **Name** column when those literals appear in the filename (for example `pp:**Monkey*pdf` on `SuperMonkeyReport.pdf`). Works for all Path query types (substring, glob, `rs:`, `pp:`, etc.). `/` Filter in results still overrides both columns.
...
- **Results table match highlighting:** Matched text in the **Name** and **Full Path** columns is highlighted so you can see why a row matched. Covers plain substring, `fz:` fuzzy, globs, `rs:` regex, `pp:` / PathPattern literal runs, and `/` “Filter in results”. Highlighting follows the Name or Path field for that column (or the active `/` filter for both). See Search Help for a short guide.
